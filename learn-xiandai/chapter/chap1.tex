...
观察$\bm{AB}$的结果图示：
\[
\bm{AB}=\begin{pmatrix}\bm{\alpha_1}\\\bm{\alpha_2}\\\vdots\\\bm{\alpha_n}\end{pmatrix}(\bm{\beta_1},\bm{\beta_2},\ldots,\bm{\beta_n})\Rightarrow
\begin{pmatrix}
 & \bm{\beta_1}& \bm{\beta_2}& \cdots & \bm{\beta_n}\\
 \bm{\alpha_1}&\bm{\alpha_1\beta_1}&\bm{\alpha_1\beta_2}&\cdots&\bm{\alpha_1\beta_n}\\
 \bm{\alpha_2}&\bm{\alpha_2\beta_1}&\bm{\alpha_2\beta_2}&\cdots&\bm{\alpha_2\beta_n}\\
 \vdots&\vdots&\vdots&&\vdots\\
 \bm{\alpha_n}&\bm{\alpha_n\beta_1}&\bm{\alpha_n\beta_2}&\cdots&\bm{\alpha_n\beta_n}\\
\end{pmatrix}
\]
可以看出，矩阵$\bm{B}$的每一列都被进行了相同的加权求和操作，所以整体上对行向量进行线性变换，同时矩阵$\bm{A}$的每一行都被进行了相同的加权求和操作，所以整体上对列向量进行线性变换
\begin{example}{}{}
    \[D=\begin{vmatrix}1&\cos\theta_1&\cos2\theta_1&\ldots&\cos(n-1)\theta_1\\1&\cos\theta_2&\cos2\theta_2&\ldots&\cos(n-1)\theta_2\\\vdots&\vdots&\vdots&\ddots&\vdots\\1&\cos\theta_n&\cos2\theta_n&\ldots&\cos(n-1)\theta_n\end{vmatrix}\]
\end{example}
\begin{solution}
    设 $x_i = \cos \theta_i$，利用第一类切比雪夫多项式 $T_k(x)$ 满足 $T_k(\cos \theta) = \cos(k\theta)$，
    则行列式可视为矩阵 $\bigl(T_{j-1}(x_i)\bigr)_{i,j=1}^n$ 的行列式。存在上三角矩阵 $C$ 使得
    $(T_0(x), T_1(x), \dots, T_{n-1}(x)) = (1, x, \dots, x^{n-1}) C$，
    且 $C$ 的主对角线元素为 $c_{11}=1, c_{22}=1, c_{jj}=2^{\,j-2} \ (j \ge 3)$，故
    $\det C = 2^{\frac{(n-2)(n-1)}{2}}$。另一方面，Vandermonde 行列式
    $\det \bigl(x_i^{\,j-1}\bigr) = \prod_{1 \le i < j \le n} (x_j - x_i)$。
    因此
    \[
    D = \det C \cdot \prod_{1 \le i < j \le n} (x_j - x_i)
      = 2^{\frac{(n-2)(n-1)}{2}} \prod_{1 \le i < j \le n} (\cos \theta_j - \cos \theta_i).
    \]
    当 $n=1$ 时，右端理解为 $1$。
\end{solution}

\begin{example}{}{}
    \[D=\begin{vmatrix}\sin\theta_{1}&\sin2\theta_{1}&\ldots&\sin n\theta_{1}\\\sin\theta_{2}&\sin2\theta_{2}&\ldots&\sin n\theta_{2}\\\vdots&\vdots&\ddots&\vdots\\\sin\theta_{n}&\sin2\theta_{n}&\ldots&\sin n\theta_{n}\end{vmatrix}\]
\end{example}
\begin{solution}
    利用恒等式 $\sin(k\theta) = \sin \theta \cdot U_{k-1}(\cos \theta)$，其中 $U_{k-1}$ 是第二类切比雪夫多项式。
    设 $x_i = \cos \theta_i$，则
    \[
    D = \left( \prod_{i=1}^n \sin \theta_i \right) \cdot \det \bigl( U_{j-1}(x_i) \bigr)_{i,j=1}^n.
    \]
    存在上三角矩阵 $D$ 使得 $(U_0(x), U_1(x), \dots, U_{n-1}(x)) = (1, x, \dots, x^{n-1}) D$，
    且 $D$ 的主对角线元素为 $d_{jj}=2^{\,j-1}$，故 $\det D = 2^{\frac{n(n-1)}{2}}$。
    而 $\det \bigl(x_i^{\,j-1}\bigr) = \prod_{1 \le i < j \le n} (x_j - x_i)$，所以
    \[
    \det \bigl( U_{j-1}(x_i) \bigr) = 2^{\frac{n(n-1)}{2}} \prod_{1 \le i < j \le n} (\cos \theta_j - \cos \theta_i).
    \]
    因此
    \[
    D = 2^{\frac{n(n-1)}{2}} \left( \prod_{i=1}^n \sin \theta_i \right) \prod_{1 \le i < j \le n} (\cos \theta_j - \cos \theta_i).
    \]
    当 $n=1$ 时，右端空乘积理解为 $1$，指数为 $0$，与 $D=\sin\theta_1$ 一致。
\end{solution}
