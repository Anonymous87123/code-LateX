...
def main():
    ...
    parser.add_argument('--grouping', type=str, default='DG2', choices=['DG2', 'OEDG'],
    ...
    f_obj = bench.get_function(args.func_id)
    info = bench.get_info(args.func_id)
    info['fNum'] = args.func_id
    ...
    out_path = args.out
    if out_path is None:
        out_name = f"cbcco_{args.grouping.upper()}_F{args.func_id}.txt"
        out_path = os.path.join(ROOT, out_name)

    with open(out_path, "w") as f:
        grouping_used = decomp_info.get('method', args.grouping.upper())
        f.write(f"Function: F{args.func_id}\n")
