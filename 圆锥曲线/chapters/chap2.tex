...
\begin{conclusion}{比值代换法}{}
\noindent 定义双线性极化算子：$T(M, N) = \dfrac{x_M x_N}{a^2} + \dfrac{y_M y_N}{b^2} - 1$。\\
若椭圆上两点 $A(x_1,y_1), B(x_2,y_2)$ 与非椭圆上点 $P(x_0,y_0)$ 共线，且满足 $\overrightarrow{AP} = \lambda \overrightarrow{PB}$，则必然成立以下三大完美定律：
\[\begin{cases}
\text{ 比例生成律：} & \lambda = \dfrac{x_1-x_0}{x_0-x_2} = \dfrac{T(P,A)}{T(P,B)} \\[8pt]
\text{ 降维求和律：} & T(A,B) = T(P,A) + T(P,B) \\[8pt]
\text{ 调和极化律：} & T(P,P) \cdot T(A,B) = 2\,T(P,A) \cdot T(P,B)
\end{cases}\]
\textbf{实战推论}：消去 $\lambda$，得到纯粹的调和点位约束：
