...
\begin{solution}
    ...
    \end{align*}
    再把$A_{i+1}^{k}$和${(i+1)}^{k}$联系起来。考虑把$k$个有标号的球放入$i+1$个有标号的盒子中，允许空盒，则放法数为${(i+1)}^{k}$。若按非空盒子的个数分类，则
...
由这两式相除可得$n=-4$，因此直线$MN$过定点$(-4,0)$。也可直接比较$\dfrac{1}{x_1}+\dfrac{1}{x_2}$得到同样的结论。\vspace{10pt}\newline
