...
\end{example}
\begin{solution}
    构造直线系$(y-0)(y-(a-2))=y(y-a+2)$，再与抛物线$x^2+(a+1)x-y+a-2=0$相加，得
...
\end{example}
\begin{solution}
    由斜率互为相反数可用圆系处理，先构造直线系$(x-ty-2)(x-ty+3)=0$，再与椭圆联立：
    \[\begin{cases}(x-ty-2)(x-ty+3)=0\\8x^2+9y^2-72=0\end{cases}\]
    再取原椭圆方程的适当倍数，使 $x^2,y^2$ 的系数相等，于是
...
\begin{solution}
    ...
    两式相加，得直径圆方程
    \[x^2+y^2-(2m+2p^2t^2)x-2pty+m^2-2pm=0\]
    这是因为消去$x,y$后分别得到关于两端点横坐标、纵坐标的二次方程，于是相加后正好得到以$AB$为直径的圆。
\end{solution}
...
\begin{solution}
    ...
    因此圆方程有两个：$x^2+y^2-2x-4y+5=0$和$x^2+y^2-18x+12y+21=0$。
\end{solution}
\begin{example}{（直径圆习题）}{}
...
\begin{solution}
    ...
    将$(1)+(2)+2m(3)$相加，得过$A,B$两点且圆心在$x$轴上的圆\vspace{-5pt}
