...
\end{theorem}
3和4即书上给出的量词辖域扩张与收缩律，2虽然说$B$含$x$，但是由于此时蕴含前件不含$x$，而是含有$y$，且指导变元是$y$，所以此时和4一样。上面的公式利用等价变形都可以解决。
...
\noindent
\textbf{证明 2:} $\forall xA(x)\to \exists xB(x)\Rightarrow \exists x(A(x)\to B(x))$
\vspace{-10pt}
\begin{align*}
...
\noindent
\textbf{证明 7:} $\forall x\forall yA(x,y)\Rightarrow \forall xA(x,x)$
\vspace{-10pt}
\begin{align*}
...
\noindent
\textbf{证明 8:} $\exists xA(x,x)\Rightarrow \exists x\exists yA(x,y)$
\vspace{-10pt}
\begin{align*}
...
\noindent
\textbf{证明 9.1:} $\forall x\forall yA(x,y)\Rightarrow \exists y\forall xA(x,y)$
\vspace{-10pt}
\begin{align*}
&(1)\ \forall x\forall yA(x,y) && \text{前提} \\
...
\noindent
\textbf{证明 9.2:} $\exists y\forall xA(x,y)\Rightarrow \forall x\exists yA(x,y)$
\vspace{-10pt}
\begin{align*}
...
\noindent
\textbf{证明 9.3:} $\forall x\exists yA(x,y)\Rightarrow \exists x\exists yA(x,y)$
\vspace{-10pt}
\begin{align*}
