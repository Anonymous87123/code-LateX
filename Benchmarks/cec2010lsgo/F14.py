...
class F14(Benchmarks):
	...
	def compute(self, x):
		...
		z = x - self.Ovector
		# t = P(z)
		t = super().permute_vector(z, self.Pvector)
		...
		# Σrot_elliptic(t_{(k-1)*m+1},..,t_{k*m}) for k=1,..,D/m
		for k in range(num_groups):
			start_idx = k * m
			end_idx = (k + 1) * m
			group = t[:, start_idx:end_idx]
			rotated_group = super().rotate_vector(group, self.Rmatrix)
			result += super().elliptic(rotated_group)
