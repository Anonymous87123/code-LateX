...
class F11(Benchmarks):
	...
	def compute(self, x):
		...
		# 第一部分：Σrot_ackley(t_{(k-1)*m+1},..,t_{k*m}) (k=1,..,D/2m)
		sum_rot_ackley = torch.zeros(batch_size, device=self.device, dtype=torch.float64)
		...
		for k in range(1, num_groups + 1):
			...
			# 计算旋转后的ackley函数值并累加
			group_result = super().ackley(rotated_group)
			sum_rot_ackley += group_result
		
		# 第二部分：ackley(t_{D/2+1},..,t_{D})
		start_idx_rest = dim // 2
		t_rest = t[:, start_idx_rest:]
		ackley_rest = super().ackley(t_rest)
		
		# 合并结果
		result = sum_rot_ackley + ackley_rest
