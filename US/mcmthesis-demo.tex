...
\mcmsetup{tstyle = \color{red}\bfseries,  % 修改题号,队号的颜色和加粗显示,黑色可以修改为 black
          tcn = 111111, problem = A,     % 修改队号,参赛题号
...
Assessing the endurance of smartphone lithium-ion batteries remains a critical challenge, hindered by dynamic usage patterns, environmental fluctuations, and electrochemical complexities. Traditional empirical models often fail to capture the coupled effects of multi-component power consumption and battery aging, leading to unreliable time-to-empty (TTE) predictions. To address this gap, we proposed a white-box, physics-driven framework that integrates a continuous-time battery model with stochastic scenario analysis. This approach uses coupled ordinary differential equations to describe state-of-charge dynamics, polarization voltage, temperature evolution, and capacity fade, achieving validation accuracy of less than 5\% against public datasets.

Firstly, for the continuous-time battery modeling component, we derived a system of coupled ordinary differential equations that capture the core electro-thermal dynamics of lithium-ion batteries.It incorporates SOC kinetics rooted in energy conservation, polarization dynamics via RC network analogs, and thermal behavior through heat transfer equations.Solved using fourth-order Runge-Kutta methods, the model exhibits exceptional robustness across diverse operating conditions, with high precision in capturing voltage sag under high-current loads. Validation against experimental data confirms voltage-SOC relationship errors of less than 5\%, outperforming empirical alternatives by explicitly accounting for aging effects.

Secondly, to quantify TTE under stochastic user behavior,we extended the model with Monte Carlo simulations incorporating usage pattern stochasticity and implemented a Continuous-Time Markov Chain (CTMC) approach to model transitions between power states.Drawing on 1000 simulations, we generated probabilistic TTE distributions that reflect real-world uncertainty-a capability absent in most existing models.

Thirdly,through Sobol-index-based global sensitivity analysis, the model maintained predictive accuracy despite parameter uncertainties. The analysis identified capacity ($67\%$) and internal resistance ($18\%$) as the principal contributors to output variance, with remaining parameters collectively accounting for the residual $15\%$. Meanwhile, parameter perturbation experiments with $\pm 20\%$ variations demonstrated the model's stability.Furthermore, the framework accurately predicts real-world phenomena like sudden shutdowns triggered by voltage cutoff thresholds, demonstrating its practical utility.

Practical applications include user-centric strategies and operating system-level optimizations. Notably, reducing screen brightness emerges as the most impactful user-level intervention, potentially extending TTE by 15$\sim$20\% due to the display's dominant 40$\sim$50\% share of total power consumption. For system-level implementation, we proposed dynamic power management algorithms that adjust CPU frequencies and network usage based on real-time SOC and temperature. Critically, the framework generalizes seamlessly to other portable devices--including laptops and wearables-via parameter recalibration, delivering consistent performance across platforms and highlighting its scalability.
...
\item {\bf Assumption:} Within typical operating temperatures (e.g., 10$\sim$25$^{\circ}$C), the internal resistance and effective capacity of the battery vary linearly with temperature.
...
\item {\bf Assumption:} Battery aging effects are negligible in short-term simulations (e.g., $\leqslant$ 1 day) but adhere to an exponential decay model over extended periods.
...
\item \textbf{Stopping Criterion}: If $V_{\text{term}}(t_{n+1}) \leqslant V_{\min}$, record $t_{n+1}$ as Time-to-Empty (TTE)
\end{itemize}
...
\begin{equation}
\text{TTE} = \inf\left\{t > 0 : \text{SOC}(t) \leqslant \text{SOC}_{\text{cutoff}} = 0.05 ~ \text{or} ~V_{\text{term}}(t) \leqslant V_{\min} = 3.0\text{V}\right\}
\label{eq:tte_definition}
\end{equation}

The voltage cutoff condition \( V_{\text{term}}(t) \leqslant V_{\min} \) is critical for capturing scenarios where high current draws (e.g., during gaming or navigation) cause rapid Ohmic losses (\(I(t) R_0(T)\)) and polarization effects, leading to a terminal voltage drop below the power management IC's (PMIC) operational limit even at moderate SOC levels (e.g., 10$\sim$20\%). This ensures the model accurately reflects nonlinear battery behavior under heavy loads, beyond a simple SOC-based depletion model.
%电压截止条件 \( V_{\text{term}}(t) \leqslant V_{\min} \) 对于捕捉那些因高电流消耗（例如在游戏或导航过程中）而导致快速的欧姆损耗（\(I(t) R_0(T)\)）和极化效应的情况至关重要,这些情况会导致即使在中等的 SOC 水平（例如 10%-20%）下,电池端电压也会低于电源管理集成电路（PMIC）的运行极限。这确保了该模型能够准确地反映在重负载情况下的非线性电池行为,超越了简单的基于 SOC 的耗尽模型。

\begin{itemize}
\item \textbf{Physical Basis (Peukert Effect):} Under high-current loads (e.g., gaming, navigation), the terminal voltage can drop below the minimum operating voltage ($V_{\min}$) required by the smartphone's power management IC (PMIC) due to significant Ohmic losses ($I \times R_0$) and rapid polarization, even when the SOC is still appreciable (e.g., 10\%$\sim$20\%). This accurately models the "sudden shutdown" phenomenon experienced by users.
...
\subsubsection{Power Decomposition and Stochastic Usage Modeling via CTMC}
...
The state transitions are governed by the intensity matrix below. The state sequence \( s(t) \) is generated using the Gillespie algorithm (Algorithm 2 in Appendix).The random number generation employs Mersenne Twister for reproducibility, with seeds stored for result verification.
...
where $Q_{ij}$ represents the transition rate from state $i$ to state $j$. For example, $Q_{12} =\frac12$ means from Idle to Browsing occurs with rate 0.5 per hour.
 %为了模拟用户行为的随机性,我们采用了具有三个状态（空闲、浏览和游戏）的连续时间马尔可夫链（CTMC）。状态转换由强度矩阵（参见公式\ref{eq：ctmc-matrix}）控制,其中 \( Q_{ij} \) 表示从状态 \( i \) 到状态 \( j \) 的转换速率。状态序列 \( s(t) \) 是通过吉莱斯皮算法（附录中的算法 2）生成的。CTMC 模拟使用吉莱斯皮算法来高效生成状态轨迹。随机数生成采用梅森旋转算法以确保可重复性,并且会保存种子以供结果验证之用。
...
For each Monte Carlo sample, we generated a state sequence $s(t)$ using the CTMC. The power consumption becomes a stochastic process:%\textbf{随机功率过程生成}： 对于每个蒙特卡罗样本,我们使用连续时间马尔可夫链（CTMC）生成了一个状态序列 $s(t)$ 。功率消耗就变成了一个随机过程：
\begin{equation}
...
\subsubsection{Combination of CTMC and ODE}
...
\FOR{$k = 1$ to $N$}
    \STATE Generate state sequence $s^{(k)}(t)$ using CTMC (Algorithm 2 in Appendix A). 
    \STATE Compute $P_{\text{total}}^{(k)}(t)$ from Eq. (13).
    \STATE Initialize $t \gets 0$
    \WHILE{$t \leqslant T_{\max}$}
    ...
    \STATE $t \gets 0$
    \WHILE{$\mathrm{SOC}(t) > 0.05$ \AND $V_{\mathrm{term}}(t) > 3.0\text{V}$ \AND $t \leqslant T_{\max}$}
        ...
        \STATE $t \gets t + \Delta t$
    \ENDWHILE
    \STATE $TTE^{(k)} \gets \min\{t: \mathrm{SOC}(t) \leqslant 0.05 \ \mathbf{or} \ V_{\mathrm{term}}(t) \leqslant 3.0\mathrm{V} \ \mathbf{or} \ t > T_{\max}\}$
\ENDFOR
...
\subsection{Variance Decomposition Analysis via Sobol and Monte Carlo}%偏差分解分析
...
The simulation results shown in Figure \ref{fig:tte_uncertainty} demonstrate the uncertainty in TTE predictions. The shaded region indicates a 95\% confidence interval of [6.92, 9.42] hours, corresponding to a relative uncertainty of $\pm$15.3\% around the mean TTE of 8.17 hours. The approximately normal distribution validates the uncertainty propagation methodology employed in the analysis.%模拟结果表明了 TTE 预测的不确定性。阴影区域表示 95% 的置信区间为 [6.92, 9.42] 小时,这对应于平均 TTE 为 8.17 小时时的相对不确定性为 $\pm$15.3%。近似正态分布验证了分析中所采用的不确定性传播方法的有效性。

Then, these results would be utilized and discussed in subsection \ref{o}.

\section{Further In-Depth Sensitivity Analysis}

\subsection{Further In-Depth Sensitivity Analysis of Model I}

To comprehensively evaluate parameter interactions and their collective impact on TTE predictions, a three-dimensional sensitivity surface visualization was accepted. This advanced analysis extends beyond traditional one-dimensional sensitivity indices by capturing the nonlinear coupling effects between CTMC transition parameters and electrochemical characteristics.%为了全面评估参数之间的相互作用及其对“空电时间”预测的综合影响,我们开发了一种三维敏感性表面可视化工具。这种先进的分析方法超越了传统的单维敏感性指标,能够捕捉到连续时间马尔可夫链（CTMC）转换参数与电池电化学特性之间的非线性耦合效应。

Figure \ref{fig:3d_sensitivity} presents a Gaussian-smoothed sensitivity surface that quantitatively maps the interaction intensity between usage pattern transitions and intrinsic battery properties.The surface reveals that transitions from idle to gaming states ($Q_{13}$) coupled with battery capacity factors ($C_f$) exhibit the highest sensitivity (0.82), while navigation-related parameters show moderate interaction effects. This visualization provides critical insights for targeted parameter optimization in battery management systems.%图 \ref{fig:3d_sensitivity} 展示了一个经过高斯平滑处理的敏感性表面图,该图定量地描绘了使用模式转变与电池固有特性之间的相互作用强度。该图的可视化采用了从 0（影响最小）到 1（影响最大）的标准化敏感性指数,并通过颜色编码来表示敏感性强度等级：高敏感性（红色,>0.7）,中高敏感性（橙色,0.4 - 0.7）,中等敏感性（黄色,0.2 - 0.4）,低敏感性（灰色,<0.2）。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figures/fig5.pdf}
    \caption{Three-dimensional sensitivity surface analysis of CTMC-battery parameter interactions}
...
\begin{itemize}
    ...
    \item \textbf{Nonlinear Coupling Identification}: The curved surface topology reveals significant nonlinear interactions, particularly between thermal parameters ($\tau$) and high-power transitions, validating the need for coupled electro-thermal modeling.
\end{itemize}
...
\end{figure}
These results are in line with the empirical data, with an average error of less than 5\%. The Gaming scenario shows the shortest TTE (8.17 hours), while Standby provides the longest duration (12.81 hours), consistent with expectations based on power consumption levels.%这些结果与实测数据相符,平均误差小于 5%。参数扰动和方差分解有助于揭示不确定性来源。游戏场景下的 TTE 最短（8.17 小时）,而待机场景下的持续时间最长（12.81 小时）,这与基于功耗水平的预期相符。

\subsection{Further In-Depth Sensitivity Analysis of Model II}
\label{o}
...
\begin{itemize}
\item Stochastic mixed usage shows the widest confidence intervals; 
...
This not only reflects real-world usage variability, but also enables clear comparison of prediction reliability under different operational conditions.
\begin{figure}[H]
    \centering
...
\end{equation}
We computed these indices via Saltelli's sampling (N=1000 samples per parameter). The results show that \(Q_n\) dominates the variance (67\%), while \(R_0\) contributes 18\%.

\subsubsection{Interpretation and Application of Sobol Indices}

The global sensitivity analysis quantifies the relative contribution of each model parameter to TTE prediction variance. The 3D visualization (Figure \ref{fig:sobol_3d}) and variance decomposition chart (Figure \ref{fig:variance_pie}) provide complementary perspectives on parameter importance and interaction effects.
...
\caption{3D Visualization of Sobol Indices}
\label{fig:sobol_3d}
\end{subfigure}
\begin{subfigure}{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{fig10.pdf}
\caption{Variance Contribution Pie Chart}
\label{fig:variance_pie}
\end{subfigure}
\end{figure}

\textbf{Figure \ref{fig:sobol_3d}} presents a three-dimensional visualization of Sobol indices, depicting both first-order ($S_i$) and total-order ($S_{Ti}$) sensitivity measures. The vertical height of each bar corresponds to the sensitivity index magnitude, with distinct color coding for parameter types. Key observations from this visualization include:

\begin{itemize}
\item \textbf{Dominant Role of Battery Capacity ($Q_n$)}: The 3D representation clearly shows that $Q_n$ towers over other parameters, with first-order index $S_i = 0.67$ and total-order index $S_{Ti} = 0.72$. This indicates that battery capacity alone explains 67\% of TTE prediction variance, and its interactions with other parameters contribute an additional 5\%.
\item \textbf{Secondary Influence of Internal Resistance ($R_0$)}: Positioned as the second most influential parameter, $R_0$ contributes 18\% of variance through its independent effects ($S_i = 0.18$) and shows moderate interaction effects (increasing to $S_{Ti} = 0.23$). The height difference between $S_i$ and $S_{Ti}$ columns visually represents interaction strength.
\item \textbf{Minimal Impact of Power Coefficients}: Screen ($k_d$) and CPU ($k_c$) power coefficients appear as relatively low bars, confirming their limited influence on TTE uncertainty (combined contribution less than 9\%). The small difference between their first-order and total-order indices suggests negligible interaction effects.
\item \textbf{Interaction Effects Visualization}: The spatial arrangement allows direct comparison between $S_i$ (front bars) and $S_{Ti}$ (rear bars), with the gap representing interaction contributions. This perspective reveals that while $Q_n$ exhibits substantial interactions, power coefficients operate largely independently.
\end{itemize}

\subsubsection{Summary}

As a summary, our three-dimensional violin plot visualization (Figure \ref{fig:3d_violin}) offers a comprehensive probabilistic perspective on TTE predictions across usage scenarios. This innovative representation combines kernel density estimation, measurement uncertainty quantification, and statistical summary metrics in an integrated visualization framework.Each violin shape represents the probability density function of TTE predictions, while the surrounding error clouds quantify measurement and model uncertainties. The visualization captures distribution characteristics including multimodality, skewness, and tail behavior that are obscured in conventional plots.
%虽然传统的箱线图只能提供有限的分布信息,但我们的三维小提琴图可视化方式（图 \ref{fig:3d_violin}）为不同使用场景下的 TTE 预测提供了全面的概率视角。这种创新的表示方法将核密度估计、测量不确定性量化以及统计汇总指标整合在一个综合的可视化框架中。每个小提琴形状代表 TTE 预测的概率密度函数,而周围的误差云则量化了测量和模型的不确定性。该可视化能够捕捉到传统图无法呈现的分布特征,如多峰性、偏度和尾部行为。
\begin{figure}[H]
    \centering
...
\item \textbf{Voltage and SOC Show Moderate Reliability:} Terminal voltage and SOC predictions maintain 3$\sim$5\% error ranges, indicating relatively stable electrochemical state tracking despite varying conditions.
\item \textbf{Thermal and Capacity Metrics Are Stable:} Battery temperature and effective capacity exhibit the lowest uncertainty (2$\sim$4\%), as these parameters are less sensitive to short-term usage fluctuations.
...
\begin{itemize}
\item Adaptive Brightness Control: Enable auto-brightness or manually reduce brightness in low-light conditions to cut display power consumption.
\item Network Optimization: Prefer Wi-Fi over cellular data and disable unused connections (e.g., Bluetooth) to minimize radio power drain.
\item Smart Charging: Maintain State of Charge (SOC) between 20-80\% and avoid using the phone while charging to reduce battery degradation and thermal stress.
\end{itemize}
\subsection{Operating System-Level Strategies}
\begin{itemize}
\item Dynamic Performance Scaling: Implement Model Predictive Control (MPC)-based throttling in low-SOC/high-temperature conditions to balance performance and energy efficiency.
\item Load Scheduling: Defer non-urgent background tasks (e.g., app updates, backups) to charging periods to reduce active drain during battery use.
\item Accurate SOC Display: Use voltage-based Open Circuit Voltage (OCV)-SOC estimation for more reliable state-of-charge readings, avoiding errors from simplified Coulomb counting.
\end{itemize}
\subsection{Extension to Other Portable Devices}
\begin{itemize}
\item Laptops: Prioritize thermal management during intensive tasks (e.g., CPU/GPU workloads) to prevent temperature-induced capacity fade.
\item Wearables: Optimize sensor sampling rates and low-power communication protocols (e.g., Bluetooth Low Energy, BLE) to extend battery life between charges.
\item Cross-Device Ecosystems: Implement energy-aware task allocation to offload work to devices with more available battery capacity.
\end{itemize}

\subsection{Conclusion}

The proposed electro-thermal model and strategies can extend battery life by 15$\sim$30\% through a balance of "energy budget" utilization and cumulative drain minimization. Key innovations include a first-principles model ensuring physical consistency, dual-criterion Time to Empty (TTE) prediction, and a scalable framework for multi-device ecosystems. Future work should focus on real-time parameter estimation to enable personalized battery optimization.
...
\section{Adaptive Battery-Temperature-Aware Power Management Algorithm}
This algorithm is a dedicated dynamic power management solution for embedded/edge devices. Based on dual-dimensional data of battery State of Charge (SOC) and real-time device temperature, it dynamically adjusts CPU operating frequency and network bandwidth limits to achieve intelligent balance between performance and power consumption. It effectively solves problems such as insufficient battery life at low power and device lag/damage due to high-temperature operation.

Core Design Concept: Hierarchical Regulation, On-Demand Energy Saving. Divide 3 levels of power strategies according to the threshold ranges of SOC and temperature to avoid sudden performance drop caused by a single energy-saving strategy, and balance device operation efficiency and power consumption control.

Algorithm Regulation Logic Flow
\begin{itemize}
\item Real-time Collection: Battery SOC (0$\sim$100\%) + Device Temperature ($^{\circ}$C)
\item Threshold Judgment: Match low power/high temperature thresholds to determine power gear
\item Parameter Delivery: Set corresponding CPU frequency + network bandwidth limit
\item Cyclic Execution: Repeat collection-judgment-delivery process at fixed intervals
\end{itemize}
\begin{lstlisting}[language=Python]
import time
from collections import namedtuple
PowerState = namedtuple('PowerState', ['cpu_freq', 'network_limit'])
def read_battery_soc():
    return 75
def read_device_temp():
    return 32
def set_cpu_freq(freq):
    print(f"[Hardware Delivery] Set CPU Operating Frequency $\to$ {freq} MHz")
def set_network_limit(limit):
    print(f"[Hardware Delivery] Set Network Bandwidth Limit $\to$ {limit} Mbps")
...
class PowerManager:
    def __init__(self):
        ...
        self.high_temp_threshold = 45
        self.power_gear = {
            'high': (1800, 100),
            'mid': (1200, 50),
            'low': (600, 10)
        }
    def adjust_power(self, soc, temp):
        if temp >= self.high_temp_threshold or soc <= self.low_soc_threshold:
            return PowerState(*self.power_gear['low'])
        elif 30 <= soc < 80 or 35 <= temp < self.high_temp_threshold:
            return PowerState(*self.power_gear['mid'])
        else:
            return PowerState(*self.power_gear['high'])
if __name__ == "__main__":
    pm = PowerManager()
    cycle_limit = 5
    current_cycle = 0
    print(f"Regulation Parameters: Low SOC Threshold {pm.low_soc_threshold}% | High Temperature Threshold {pm.high_temp_threshold}$^{\circ}$C")
    print(f"Cycle Count: {cycle_limit} times | Collection Interval: 1 second\n")
    while current_cycle < cycle_limit:
        soc, temp = get_hardware_data()
        power_state = pm.adjust_power(soc, temp)
        print(f"[Collection Data] SOC={soc}% | Device Temperature={temp}$^{\circ}$C")
        print(f"[Regulation Result] CPU Frequency={power_state.cpu_freq}MHz | Network Limit={power_state.network_limit}Mbps")
        set_cpu_freq(power_state.cpu_freq)
        set_network_limit(power_state.network_limit)
        time.sleep(1)
        current_cycle += 1
        print("-" * 60 + "\n")
    print(f"Total Execution Times:{current_cycle}|Regulation Logic:Normal")
\end{lstlisting}
\end{appendices}
