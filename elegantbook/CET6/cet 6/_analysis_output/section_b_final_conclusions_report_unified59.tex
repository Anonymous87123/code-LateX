...
旧版本分析里之所以必须区分两层口径，是因为当时有 \CodeInline{3} 篇 2024 年 6 月文章还没有并入标准 \CodeInline{section\_b\_texts} 链路，只保留了人工复核后的答案字母，缺少可直接进入主统计表的标准化段落位置百分比。
...
本轮已经把这 \CodeInline{3} 篇文章补入 \CodeInline{section\_b\_texts} 标准链路，并成功生成完整位置数据，分别是：
...
补齐后，这种“双口径”问题在当前版本中已经消失。现在答案层与位置层都统一为 \CodeInline{59} 套 /\allowbreak{} \CodeInline{590} 题。后文如果引用旧分析草稿中出现的 \CodeInline{56}、\CodeInline{560}、\CodeInline{ocr\_manual} 等字样，应理解为 \textbf{旧阶段工作痕迹}，不再代表当前主口径。
...
\begin{enumerate}
\item 当时已在标准 \CodeInline{section\_b\_texts} 题文包中的 \CodeInline{56} 篇
\item 当时仅存在于 \CodeInline{sample.txt} /\allowbreak{} OCR 复核链路中的 \CodeInline{3} 篇补充标题
\end{enumerate}


但在本轮补齐后，当前正式口径已经统一为：\CodeInline{59} 篇标准化题文包、\CodeInline{590} 题完整位置记录。
