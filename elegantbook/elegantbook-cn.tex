...
\begin{introduction}
  \item 二维随机变量与联合分布函数~\ref{def:2d-random-variable}~\ref{def:joint-df}
  \item 联合分布函数的性质~\ref{prop:joint-df}
  \item 边缘分布函数~\ref{def:marginal-df}
  \item 二维离散型随机变量~\ref{def:2d-discrete}~\ref{prop:joint-pm}~\ref{def:marginal-pm}~\ref{def:conditional-pm}
  \item 二维连续型随机变量~\ref{def:2d-continuous}~\ref{prop:joint-pdf}~\ref{def:marginal-pdf}~\ref{def:conditional-pdf}
  \item 二维均匀分布~\ref{def:2d-uniform}
  \item 二维正态分布~\ref{def:2d-normal}
\end{introduction}
...
\[
F_X(x) = P\{X \leq x\} = F(x, +\infty) = \lim_{y \to +\infty} F(x,y), \quad F_Y(y) = P\{Y \leq y\} = F(+\infty, y) = \lim_{x \to +\infty} F(x,y)
\]
...
\[
P\{X_1=0 \mid X_2=1\} = \frac{p_{01}}{p_{\cdot 1}} = \frac{2/9}{4/9} = \frac{1}{2}, \quad P\{X_1=1 \mid X_2=1\} = \frac{p_{11}}{p_{\cdot 1}} = \frac{2/9}{4/9} = \frac{1}{2}, \quad P\{X_1=2 \mid X_2=1\} = 0.
\]
...
\begin{equation}
   \label{inter}
   \int_{E} f \mathrm{d}x = \sum_{i=1}^k a_i m(A_i) \pi \alpha\beta\sigma\gamma\nu\xi\epsilon\varepsilon. \oint_{a}^b\ointop_{a}^b\prod_{i=1}^n
\end{equation}
一般情况下 $0 \leq \int_{E} f \mathrm{d}x \leq \infty$。若 $\int_{E} f \mathrm{d}x < \infty$，则称 $f$ 在 $E$ 上可积。
\end{definition}

一个自然的问题是，Lebesgue 积分与我们所熟悉的 Riemann 积分有什么联系和区别？在 4.4 在我们将详细讨论 Riemann 积分与 Lebesgue 积分的关系。这里只看一个简单的例子。设 $D(x)$ 是区间 $[0,1]$ 上的 Dirichlet 函数。即 $D(x)=\chi_{Q_0}(x)$，其中 $Q_0$ 表示 $[0,1]$ 中的有理数的全体。根据非负简单函数积分的定义，$D(x)$ 在 $[0,1]$ 上的 Lebesgue 积分为
\begin{equation}
   \label{inter2}
   \int_0^1 D(x)\mathrm{d}x = \int_0^1 \chi_{Q_0} (x) \mathrm{d}x = m(Q_0) = 0
\end{equation}
...
\begin{equation}
   \label{ex:1}
   I(t) = \int_{\mathcal{R}^1} f(x+t)g(x)\mathrm{d}x \quad t \in \mathcal{R}^1
\end{equation}
...
\begin{equation}
   \label{eq:461}
   \int_{\mathcal{R}^p\times\mathcal{R}^q} f(x,y) \mathrm{d}xdy=\int_{\mathcal{R}^p}\left(\int_{\mathcal{R}^q}f(x,y)dy\right)\mathrm{d}x.
\end{equation}
