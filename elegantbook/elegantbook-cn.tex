...
函数 $f(x)$ 为某一随机变量的概率密度的\textbf{充要条件}：
\begin{enumerate}
    \item $f(x) \ge 0$；
...
(1) 由归一性条件：
\[
\int_{0}^{\pi} A\sin x\,\mathrm{d}x = A[-\cos x]_{0}^{\pi} = A(1+1) = 2A = 1,
\]
解得 $A = \dfrac{1}{2}$。\\
...
设 $F_1(x), F_2(x)$ 是随机变量的分布函数，$f_1(x), f_2(x)$ 是相应的概率密度，则下列选项中正确的是（\quad）。\\
...
\begin{enumerate}
    ...
    \item 若 $X \sim N(\mu,\sigma^2)$，则 $F(x) = \Phi\left(\frac{x-\mu}{\sigma}\right)$；
    \item 若 $X \sim N(\mu,\sigma^2)$，则 $\frac{X-\mu}{\sigma} \sim N(0,1)$；
...
\begin{exercise}\label{exer:2-11}
...
\[
\begin{aligned}
F(x) &= P\{X \le 1\} + P\{1 < X \le x\} \\
&= \frac{1}{4} + \frac{x-1}{4-1} \cdot \frac{1}{2} = \frac{5 + 2x}{12};
\end{aligned}
\]
当 $x \ge 4$ 时，$F(x) = 1$。
故
...
\begin{exercise}[提高]\label{exer:2-27}
...
\end{aligned}
\]
答案：$\boldsymbol{1-e^{-1}}$；$\boldsymbol{0}$。
\end{solution}
...
\subsubsection{当 $Y$ 为离散型时} \label{subsubsec:continuous-to-discrete}
...
\subsubsection{当 $Y$ 为连续型时} \label{subsubsec:continuous-to-continuous}
