...
解释：
\begin{itemize}
    \item 当 $x<0$ 时，$F(x)=P\{X\leq x\}=0$；
    \item 当 $0\leq x<1$ 时，$F(x)=P\{X=0\}=1-p$；
    \item 当 $x\geq1$ 时，$F(x)=P\{X=0\}+P\{X=1\}=1-p+p=1$。
\end{itemize}
\end{solution}
...
设 $X\sim B(2,p)$，$Y\sim B(4,p)$，且 $P(X\geq1)=\dfrac{5}{9}$，则 $P(Y\geq1)=$（\quad）。
\begin{tabular}{llll}
A. $\dfrac{65}{81}$ & B. $\dfrac{16}{81}$ & C. 1 & D. $\dfrac{4}{7}$
\end{tabular}
\end{exercise}
\begin{solution}
由题设 $P(X\geq1)=\dfrac{5}{9}=1-P(X=0)=1-\mathrm{C}_2^0p^0(1-p)^2$，即 $(1-p)^2=\dfrac{4}{9}$，解得 $p=\dfrac{1}{3}$，从而有
\[
P(Y\geq1)=1-P(Y=0)=1-\mathrm{C}_4^0p^0(1-p)^4=1-\left(\frac{2}{3}\right)^4=\frac{65}{81},
\]
答案选 \textbf{(A)}。
\end{solution}
...
设随机变量 $X$ 的概率密度为 $f(x)=\begin{cases}2x, & 0<x<1, \\ 0, & \text{其他},\end{cases}$，$Y$ 表示对 $X$ 的 3 次独立重复试验中事件 $\left\{X\leq\dfrac{1}{2}\right\}$ 出现的次数，则 $P\{Y=2\}=\underline{\hspace{2cm}}$。
\end{exercise}
\begin{solution}
首先计算单次试验中事件发生的概率：
\[
p = P\left\{X\leq\frac{1}{2}\right\} = \int_0^{\frac{1}{2}}2x\,\mathrm{d}x = \left[x^2\right]_0^{\frac{1}{2}} = \frac{1}{4}.
\]
...
&= 1-\left[P\{X_1>t\}\right]^4 = 1-\left[1-F_X(t)\right]^4 = 1-\left[e^{-0.2t}\right]^4 = 1-e^{-0.8t},\quad t\geq0.
\end{aligned}
\]
当 $t<0$ 时，$F_T(t)=0$。故
\[
...
\[
P\{Y\geq2\} = P\{Y=2\} + P\{Y=3\} = \mathrm{C}_3^2\left(\frac{2}{3}\right)^2\left(\frac{1}{3}\right) + \mathrm{C}_3^3\left(\frac{2}{3}\right)^3 = 3\cdot\frac{4}{9}\cdot\frac{1}{3} + \frac{8}{27} = \frac{12}{27} + \frac{8}{27} = \frac{20}{27}.
\]
...
\[
\begin{aligned}
P\{\max\{X_1,X_2\}>5\} &= 1-P\{X_1\leq5, X_2\leq5\} \\
...
\[
\begin{aligned}
P\{|X|\geq\lambda\} &= \int_{-\infty}^{-\lambda} f(x)\,dx + \int_{\lambda}^{\infty} f(x)\,dx = 2\int_{\lambda}^{\infty} \frac{1}{2\lambda}e^{-\frac{x}{\lambda}}\,dx \\
...
\[
\begin{aligned}
F_Z(z) &= P\{Z\leq z\} = P\{e^{-|X|}\leq z\} = P\{|X|\geq -\ln z\} \\
&= 2\int_{-\ln z}^{\infty} \frac{1}{2\lambda}e^{-\frac{x}{\lambda}}\,dx = z^{\frac{1}{\lambda}}.
\end{aligned}
\]

当 $z\leq0$ 时，$F_Z(z)=0$；当 $z\geq1$ 时，$F_Z(z)=1$。
...
\end{cases}
\]
则 $P\{0\leq X\leq1\}=\underline{\hspace{2cm}}$，$P\{0<X<1\}=\underline{\hspace{2cm}}$。
\end{exercise}

\begin{solution}
\[
\begin{aligned}
P\{0\leq X\leq1\}&=F(1)-F(0^-)=1-e^{-1}; \\
...
\end{cases}
\]
则 $X$ 的分布列为$\underline{\hspace{2cm}}$，$P\{X\leq3.2\}=\underline{\hspace{2cm}}$，方程 $x^2+Xx+1=0$ 有实根的概率为$\underline{\hspace{2cm}}$。
\end{exercise}
...
$P\{X\leq3.2\}=1$（因为最大取值为3）。

方程有实根 $\Leftrightarrow \Delta=X^2-4\geq0 \Leftrightarrow X\geq2$ 或 $X\leq-2$（后者不可能），
\[
P(X\geq2)=P\{X=2\}+P\{X=3\}=0.4+0.2=0.6.
\]
...
\end{cases}
\]
则 $A=\underline{\hspace{2cm}}$，$P\{100<X\leq150\}=\underline{\hspace{2cm}}$，$P\{X\geq1000\}=\underline{\hspace{2cm}}$，$P\{X=1000\}=\underline{\hspace{2cm}}$，$X$ 的分布函数 $F(x)=\underline{\hspace{2cm}}$。
\end{exercise}
...
相关概率计算：
\[
P\{100<X\leq150\} = \int_{100}^{150}\frac{100}{x^2}\,\mathrm{d}x = 100\left(\frac{1}{100}-\frac{1}{150}\right) = \frac{1}{3},
\]
\[
P\{X\geq1000\} = \int_{1000}^{+\infty}\frac{100}{x^2}\,\mathrm{d}x = 100\cdot\frac{1}{1000} = \frac{1}{10},
\]
\[
...
分布函数：
\[
F(x) = \int_{100}^{x}\frac{100}{t^2}\,dt = 1-\frac{100}{x}, \quad x\geq100,
\]
故
\[
F(x)=
...
\begin{cases}
0, & x<-a, \\[6pt]
A+B\arcsin\dfrac{x}{a}, & -a\leq x\leq a, \\[6pt]
1, & x\geq a,
\end{cases}
\]
...
\begin{cases}
0, & x<0, \\[6pt]
\dfrac{x^2}{2}, & 0\leq x<1, \\[6pt]
-\dfrac{x^2}{2}+2x-1, & 1\leq x<2, \\[6pt]
1, & x\geq2.
\end{cases}
\]
\end{solution}
...
\begin{cases}
0, & x<-1, \\[4pt]
\dfrac{1}{3}, & -1\leq x\leq2, \\[4pt]
1, & x>2,
\end{cases}
&&\text{B. } F(x)=
\begin{cases}
0, & x<0, \\[4pt]
\sin x, & 0\leq x<\pi, \\[4pt]
1, & x\geq\pi,
\end{cases} \\[10pt]
&\text{C. } F(x)=
\begin{cases}
0, & x<0, \\[4pt]
\dfrac{x+2}{5}, & 0\leq x<2, \\[4pt]
1, & x\geq2,
\end{cases}
...
f(x)=F'(x)=
\begin{cases}
\dfrac{2a}{\pi(x^2+a^2)}, & -a\leq x\leq a, \\[6pt]
0, & \text{其他}.
\end{cases}
\]
(3) 方程有两个正根的条件：
\[
\begin{cases}
\Delta=X^2-\dfrac{a^2}{4}\geq0, \\[4pt]
X<0 \quad (\text{由韦达定理，根的和}=-X>0).
\end{cases}
\]
即 $X\leq-\dfrac{a}{2}$。所求概率：
\[
...
\end{exercise}
\begin{solution}
当 $y\leq1$ 时，$F_Y(y)=0$，故 $f_Y(y)=0$。\\
当 $y>1$ 时：
\[
\begin{aligned}
F_Y(y) &= P\{2X^2+1\leq y\} = P\left\{X^2\leq\frac{y-1}{2}\right\} = P\left\{-\sqrt{\frac{y-1}{2}}\leq X\leq\sqrt{\frac{y-1}{2}}\right\} \\
...
\begin{itemize}
    ...
    \]
    故 $f_Y(y)=\dfrac{1}{3}$；
    \item 当 $y\leq0$ 时，$F_Y(y)=0$；当 $y\geq2$ 时，$F_Y(y)=1$。
\end{itemize}
综上，
\[
f_Y(y)=
...
\text{二维离散型}
\begin{cases}
\text{联合分布律：} P\{X=x_i,Y=y_j\}=p_{ij};\ p_{ij}\geq0,\ \sum\limits_{i=1}^{\infty}\sum\limits_{j=1}^{\infty}p_{ij}=1 \\[8pt]
\text{边缘分布律：}
\begin{cases}
...
\text{二维连续型}
\begin{cases}
\text{联合概率密度：} f(x,y)\geq0,\ \displaystyle\int_{-\infty}^{+\infty}\int_{-\infty}^{+\infty}f(u,v)dudv=1 \\[12pt]
\text{边缘分布：}
\begin{cases}
...
\begin{enumerate}
    \item \textbf{单调性}：$F(x,y)$ 关于每个变量单调不减。即对任意固定的 $y$，当 $x_1 < x_2$ 时，$F(x_1,y) \leq F(x_2,y)$；对任意固定的 $x$，当 $y_1 < y_2$ 时，$F(x,y_1) \leq F(x,y_2)$。
    \item \textbf{有界性}：$0 \leq F(x,y) \leq 1$，且
    \[
...
\end{exercise}
\begin{solution}
$f(x,y)$ 为概率密度的充要条件：$f(x,y) \geq 0$ 且 $\displaystyle\iint f(x,y)\,\mathrm{d}x\mathrm{d}y = 1$。由于 $f_1, f_2$ 本身是概率密度（非负、归一），有
\[
\iint f\,\mathrm{d}x\mathrm{d}y = a\cdot 1 + b\cdot 1 = a + b = 1.
\]
又 $f_1, f_2 \geq 0$，故 $f \geq 0$ 要求 $a \geq 0$ 且 $b \geq 0$。答案选 \textbf{D}。
\end{solution}
...
\end{exercise}
\begin{solution}
$X+Y \sim N(0,2)$，$X-Y \sim N(0,2)$，均值为 $0$ 但对称点不在 $1$，故 $P(\cdot \geq 1) \neq 1/2$，排除 A、B。
...
\end{exercise}
\begin{solution}
$P(X=a,Y=b) = P(X=a)\,P(Y=b|X=a) = \dfrac{1}{3}\cdot\dfrac{1}{a}$（$1 \leq b \leq a$）。
逐项列出联合分布律：
\begin{center}
\begin{tabular}{c|ccc|c}
\toprule
...
\]
(2) 当 $0 < y \leq 1$ 时，$f_Y(y) = \displaystyle\int_y^1 \frac{9y^2}{x}\,\mathrm{d}x = 9y^2\bigl[-\ln y\bigr]_y^1 = -9y^2\ln y$；其余范围 $f_Y(y) = 0$。\\
...
设 $(X,Y)$ 服从区域 $G=\{(x,y): 0 \leq y \leq 1-x^2\}$ 上的均匀分布。求：\\
(1) 联合概率密度 $f(x,y)$；\\
...
\]
\[
f_Y(y) = \int_{-\sqrt{1-y}}^{\sqrt{1-y}}\frac{3}{4}\,\mathrm{d}x = \frac{3}{2}\sqrt{1-y},\quad 0 \leq y \leq 1.
\]
条件密度（$|x|<1$ 时）：
\[
f_{Y|X}(y|x) = \frac{f(x,y)}{f_X(x)} = \begin{cases} \dfrac{1}{1-x^2}, & 0 \leq y \leq 1-x^2, \\ 0, & \text{其他}. \end{cases}
\]
...
\[
P\{Y \geq X^2\} = \int_{-1/\sqrt{2}}^{1/\sqrt{2}}\int_{x^2}^{1-x^2}\frac{3}{4}\,\mathrm{d}y\,\mathrm{d}x = \frac{3}{2}\int_0^{1/\sqrt{2}}(1-2x^2)\,\mathrm{d}x
\]
\[
...
\end{exercise}
\begin{solution}
$(X,Y)$ 的联合密度 $f(x,y)=1$（$0\leq x,y\leq 1$），为矩形上的均匀分布。\\
...
\[
P\{X \leq Y/2\} = \int_0^{+\infty}\int_0^{y/2}e^{-y}\,\mathrm{d}x\,\mathrm{d}y = \int_0^{+\infty}\frac{y}{2}e^{-y}\,\mathrm{d}y = \frac{1}{2}.
\]
\end{solution}

\begin{exercise}[混合型：连续+离散] \label{exer:3-c13}
$X \sim E(4)$，$Y$ 为离散型随机变量：$P\{Y=-1\} = 1/4$，$P\{Y=1\} = 3/4$，且 $X$ 与 $Y$ 相互独立。求 $P\{X-Y \leq 1\}$ 和 $P\{XY \leq 2\}$。
\end{exercise}
\begin{solution}
利用全概率公式（对 $Y$ 的取值分解）。
\begin{align*}
...
即
\[
f_{X|Y}(x|y) = \begin{cases} \dfrac{1}{2-2y}, & y \leq x \leq 2-y, \\ 0, & \text{其他}. \end{cases}
\]
\end{solution}
\begin{note}
...
$n$ 维随机变量 $(X_1, X_2, \dots, X_n)$ 中任意 $k$ 个（$1 \leq k < n$）分量所构成的 $k$ 维随机变量的分布函数，称为原 $n$ 维随机变量的\textbf{$k$维边缘分布函数}。特别地，关于第 $i$ 个分量 $X_i$ 的边缘分布函数为
\[
...
若对任意正整数 $k$ 和任意 $1 \leq i_1 < i_2 < \dots < i_k$，随机变量 $X_{i_1}, X_{i_2}, \dots, X_{i_k}$ 都相互独立，则称随机变量序列 $X_1, X_2, \dots$ 是\textbf{相互独立}的。
\end{definition}

\subsection{独立的充要条件}

$n$ 个随机变量 $X_1, X_2, \dots, X_n$ 相互独立
$\iff$ 对任意的 $n$ 个实数 $x_i$（$i=1,2,\dots,n$），$n$ 个事件 $\{X_1 \leq x_1\}, \{X_2 \leq x_2\}, \dots, \{X_n \leq x_n\}$ 相互独立。
...
\[
F(x,y) = \sum_{x_i \leq x}\sum_{y_j \leq y}p_{ij} = \sum_{x_i \leq x}\sum_{y_j \leq y}p_{i\cdot}\,p_{\cdot j} = \left(\sum_{x_i \leq x}p_{i\cdot}\right)\left(\sum_{y_j \leq y}p_{\cdot j}\right) = F_X(x)\,F_Y(y).
\]
\end{proof}
...
随机变量的独立性具有以下重要性质：
\begin{enumerate}
    \item 若 $X_1, X_2, \dots, X_n$ 相互独立，则其中任意 $k$ 个（$2 \leq k \leq n$）随机变量也相互独立。
...
取 $x_0 = 1$，则
\[
P\{X \leq 1\} = \int_{-\infty}^1 \frac{1}{2}e^{-|x|}\,\mathrm{d}x = \frac{1}{2}\int_{-\infty}^0 e^x\,\mathrm{d}x + \frac{1}{2}\int_0^1 e^{-x}\,\mathrm{d}x = \frac{1}{2} + \frac{1}{2}(1 - e^{-1}) = 1 - \frac{1}{2}e^{-1},
\]
\[
P\{|X| \leq 1\} = \frac{1}{2}\int_{-1}^1 e^{-|x|}\,\mathrm{d}x = \int_0^1 e^{-x}\,\mathrm{d}x = 1 - e^{-1}.
\]
...
\begin{note}
本题中 $x_0$ 可取任意正数均可证明不独立。关键在于 $|X| \leq x_0$ 隐含了 $X$ 的信息，因此 $X$ 与 $|X|$ 存在函数关系，不可能独立。
\end{note}
\end{proof}
...
\begin{enumerate}
    \item 先求 $Z$ 的分布函数：
    \[
    F_Z(z) = P\{Z \leq z\} = P\{g(X,Y) \leq z\} = \iint_{\{(x,y): g(x,y) \leq z\}}f(x,y)\,\mathrm{d}x\,\mathrm{d}y;
    \]
...
用分布函数法。当 $z > 0$ 时：
\[
F_Z(z) = P\{X+Y \leq z\} = \iint_{x+y \leq z}e^{-(x+y)}\,\mathrm{d}x\,\mathrm{d}y = \int_0^z\mathrm{d}x\int_0^{z-x}e^{-(x+y)}\,\mathrm{d}y.
\]
...
设二维随机变量 $(X,Y)$ 在矩形区域 $D = \{(x,y) : 0 \leq x \leq 2,\ 0 \leq y \leq 1\}$ 上服从均匀分布，求边长为 $X$ 和 $Y$ 的矩形面积 $Z = XY$ 的概率密度。
\end{exercise}

\begin{solution}
由题设，
\[
f(x,y) = \begin{cases} \dfrac{1}{2}, & 0 \leq x \leq 2,\ 0 \leq y \leq 1, \\ 0, & \text{其他}. \end{cases}
\]
...
\begin{note}
本题亦可用分布函数法验证。$F_Z(z) = P\{XY \leq z\}$ 的积分区域取决于 $z$ 与矩形区域的关系：当 $0 < z < 2$ 时，$F_Z(z) = \dfrac{z}{2}(1 + \ln 2 - \ln z)$；当 $z \geq 2$ 时，$F_Z(z) = 1$。求导后结果一致。
\end{note}
\end{solution}
...
\[
F(x,y) = P\{X_1 \leq x, X_1 X_3^{X_2} \leq y\}
\]
\[
= P\{X_3 = 0\}\,P\{X_1 \leq x, 0 \leq y \mid X_3 = 0\} + P\{X_3 = 1\}\,P\{X_1 \leq x, X_1 \leq y \mid X_3 = 1\}
\]
\[
= \frac{1}{2}\,P\{X_1 \leq x\} + \frac{1}{2}\,P\{X_1 \leq \min(x, y)\} = \frac{1}{2}\Phi(x) + \frac{1}{2}\Phi(\min(x, y)).
\]
...
\begin{remark}
\begin{enumerate}
    \item "$\max\{X_1, \dots, X_n\} \leq z$" 等价于"所有 $X_i \leq z$"，因此取概率时用乘积。
...
\begin{solution}
$X, Y \sim E(1/4)$，分布函数 $F(x) = 1 - e^{-x/4}$（$x \geq 0$）。
...
由独立性，$(X,Y)$ 的概率密度为
\[
f(x,y) = \begin{cases} \dfrac{1}{(b-a)^2}, & a \leq x \leq b,\ a \leq y \leq b, \\ 0, & \text{其他}. \end{cases}
\]

$Z = \max\{X, Y\}$ 的取值范围为 $[a, b]$。当 $a \leq z \leq b$ 时，
\[
F_Z(z) = P\{Z \leq z\} = P\{X \leq z,\ Y \leq z\} = \int_a^z\mathrm{d}x\int_a^z \frac{1}{(b-a)^2}\,\mathrm{d}y = \frac{(z-a)^2}{(b-a)^2}.
\]

求导得概率密度：
\[
f_Z(z) = F_Z'(z) = \begin{cases} \dfrac{2(z-a)}{(b-a)^2}, & a \leq z \leq b, \\ 0, & \text{其他}. \end{cases}
\]
...
\end{exercise}
\begin{solution}
当 $y \leq 1$ 时，$2X^2 + 1 \leq y$ 无解（$X^2 \geq 0$），故 $F_Y(y) = 0$，$f_Y(y) = 0$。\\
当 $y > 1$ 时：
\[
...
综上：
\[
f_Y(y) = \begin{cases} \dfrac{1}{2\sqrt{\pi(y-1)}}\,\exp\!\left\{-\dfrac{y-1}{4}\right\}, & y > 1, \\ 0, & y \leq 1. \end{cases}
\]
\end{solution}
...
\[
f_X(x) = \begin{cases} e^{-x}, & x > 0, \\ 0, & x \leq 0, \end{cases} \quad f_Y(y) = \begin{cases} \dfrac{1}{2}e^{-y/2}, & y > 0, \\ 0, & y \leq 0. \end{cases}
\]
求 $Z = X + Y$ 的概率密度。
\end{exercise}
\begin{solution}
...
\begin{align*}
F_Z(z)& = P\{X+Y \leq z\} = \int_0^z e^{-x}\,\mathrm{d}x\int_0^{z-x}\frac{1}{2}e^{-y/2}\,\mathrm{d}y = \int_0^z e^{-x}\bigl(1-e^{-(z-x)/2}\bigr)\,\mathrm{d}x\\
...
设随机变量 $X, Y$ 相互独立，分布函数分别为 $F_X(x), F_Y(y)$，则对任意实数 $z$，$P\{\max(X,Y) \leq z\}$ 等于（\quad）。\\
\begin{tabular}{ll}
...
答案选 \textbf{A}。
\begin{note}
选项 C 为 $P\{\min(X,Y) \leq z\}$ 的表达式。不要混淆 max 和 min 的公式。
\end{note}
\end{solution}
...
\]
\textbf{方法二（分布函数法）}：当 $z < 0$ 时，$F_Z(z) = 0$；当 $z \geq 0$ 时，积分区域为 $\{0 < x < y,\ x+y \leq z\}$，即 $0 < x < z/2$，$x < y \leq z-x$：
\[
...
求导验证：
\[
f_Z(z) = F_Z'(z) = \left(\frac{z}{2}-1\right)e^{-z/2} + e^{-z}, \quad z \geq 0,
\]
与方法一结果一致。综上：
\[
f_Z(z) = \begin{cases} \left(\dfrac{z}{2}-1\right)e^{-z/2} + e^{-z}, & z \geq 0, \\ 0, & z < 0. \end{cases}
\]
...
\begin{enumerate}
    ...
    \item 第 (4) 问的技巧：在 $\{0 < x < y\}$ 的条件下 $\min(X,Y) = X$，因此 $P\{X \geq 1, Y \geq 1\}$ 的积分区域为 $\{1 \leq x < y\}$。
\end{enumerate}
\end{note}
\end{solution}
...
\[
f_Z(z) = \begin{cases} \dfrac{1}{(z+1)^2}, & z > 0, \\ 0, & z \leq 0. \end{cases}
\]
...
\end{exercise}
\begin{solution}
$X, Y$ 的分布函数 $F(x) = \dfrac{x-a}{b-a}$（$a \leq x \leq b$），密度 $f(x) = \dfrac{1}{b-a}$。
由 $\min$ 分布公式：
\[
...
所以
\[
f_Z(z) = \begin{cases} \dfrac{2(b-z)}{(b-a)^2}, & a \leq z \leq b, \\ 0, & \text{其他}. \end{cases}
\]
\begin{note}
...
$(X,Y)$ 服从区域 $G=\{(x,y):0\leq x\leq 1,\ 0\leq y\leq 2\}$ 上的均匀分布，令 $Z=\min(X,Y)$，$W=\max(X,Y)$，求 $P(Z+W \geq 1)$。
\end{exercise}
\begin{solution}
由于 $Z+W = X+Y$ 恒成立，$P(Z+W \geq 1) = P(X+Y \geq 1)$。$(X,Y)$ 的密度为 $f(x,y) = 1/2$（$0\leq x\leq 1,\ 0\leq y\leq 2$）。
\[
P(X+Y \geq 1) = 1 - P(X+Y < 1) = 1 - \int_0^1\mathrm{d}x\int_0^{1-x}\frac{1}{2}\,\mathrm{d}y = 1 - \frac{1}{2}\int_0^1(1-x)\,\mathrm{d}x = 1-\frac{1}{4} = \frac{3}{4}.
\]
\end{solution}
...
\[
F_Z(z) = \iint_{x^2+y^2 \leq z,\ x>0,\ y>0}\frac{2}{\pi}\,e^{-(x^2+y^2)/2}\,\mathrm{d}x\,\mathrm{d}y = \frac{2}{\pi}\int_0^{\pi/2}\mathrm{d}\theta\int_0^{\sqrt{z}}r\,e^{-r^2/2}\,\mathrm{d}r
\]
\[
...
$Y|X=2 \sim U(0,2)$：$F_2(y) = \begin{cases} 0, & y \leq 0, \\ y/2, & 0 < y < 2, \\ 1, & y \geq 2. \end{cases}$
分区间计算 $F_Y(y)$：
\begin{itemize}
    \item $y \leq 0$：$F_Y(y) = 0$。
    \item $0 < y \leq 1$：$F_Y(y) = \dfrac{1}{2}y + \dfrac{1}{2}\cdot\dfrac{y}{2} = \dfrac{3y}{4}$。
    \item $1 < y \leq 2$：$F_Y(y) = \dfrac{1}{2}\cdot 1 + \dfrac{1}{2}\cdot\dfrac{y}{2} = \dfrac{1}{2} + \dfrac{y}{4}$。
    \item $y \geq 2$：$F_Y(y) = 1$。
\end{itemize}
求导得密度：
\[
F_Y(y) = \begin{cases} 0, & y \leq 0, \\ \dfrac{3y}{4}, & 0 < y \leq 1, \\ \dfrac{1}{2} + \dfrac{y}{4}, & 1 < y \leq 2, \\ 1, & y \geq 2. \end{cases}
\qquad
f_Y(y) = \begin{cases} \dfrac{3}{4}, & 0 < y \leq 1, \\ \dfrac{1}{4}, & 1 < y \leq 2, \\ 0, & \text{其他}. \end{cases}
\]
\end{solution}
...
\text{相关系数}
\begin{cases}
\text{计算公式: } \rho_{XY}=\dfrac{\text{Cov}(X,Y)}{\sqrt{DX}\sqrt{DY}},\ |\rho_{XY}|\leq1 \\
\text{性质}
\begin{cases}
...
已知连续型随机变量 \(Y\) 的概率密度为
\[
f(y) = \begin{cases} \dfrac{y}{a^2}\,e^{-\frac{y^2}{2a^2}}, & y > 0,\ a > 0, \\[4pt] 0, & y \leq 0, \end{cases}
\]
...
\[
\begin{aligned}
EZ &= E\!\left(\frac{1}{Y}\right) = \int_0^{+\infty}\frac{1}{y}\cdot\frac{y}{a^2}\,e^{-\frac{y^2}{2a^2}}\,\mathrm{d}y = \frac{1}{a^2}\int_0^{+\infty}e^{-\frac{y^2}{2a^2}}\,\mathrm{d}y.
\end{aligned}
\]
...
\]
则 \(a =\,\underline{\hspace{2cm}}\)。\par
A. \(\dfrac{2}{3}\)\quad B. \(\dfrac{1}{2}\)\quad C. \(\dfrac{1}{3}\)\quad D. \(\dfrac{1}{6}\)
\end{exercise}
...
\begin{enumerate}
    \item \(DX \geq 0\)，从而 \(E(X^2) = DX + (EX)^2 \geq (EX)^2\)。
    ...
    更一般地，
    \[
    D\!\left(\sum_{i=1}^{n} a_i X_i\right) = \sum_{i=1}^{n} a_i^2 DX_i + 2\sum_{1 \leq i < j \leq n} a_i a_j\,\mathrm{Cov}(X_i,X_j).
    \]
    ...
    \]
    \item 对任意常数 \(c\)，有 \(DX = E[(X - EX)^2] \leq E[(X - c)^2]\)。
\end{enumerate}
\end{property}
...
由于 \(p(1-p) \leq \dfrac{1}{4}\)，当且仅当 \(p = \dfrac{1}{2}\) 时等号成立，故标准差的最大值为
\[
...
设随机变量 \(X\) 的概率密度为
\[
f(x) = \begin{cases} \dfrac{1}{2}\cos x, & 0 \leq x \leq \pi, \\[4pt] 0, & \text{其他}, \end{cases}
\]
...
设随机变量 \(X\) 的概率密度为 \(f(x) = \begin{cases} x, & 0 < x < 2, \\ 0, & \text{其他}, \end{cases}\) \(F(x)\) 为 \(X\) 的分布函数，则 \(P\{F(X) > EX - 1\} =\,\underline{\hspace{2cm}}\)。
\end{exercise}

\begin{solution}
应填 \(\dfrac{2}{3}\)。令 \(Y = F(X)\)，因为 \(f(x)\) 严格单调增加，所以 \(Y \sim U(0,1)\)。又 \(EX = \int_0^2 x^2\,\mathrm{d}x = \dfrac{4}{3}\)，故
\[
...
\[
F(x) = \begin{cases} 0, & x \leq 0, \\ \dfrac{x^2}{4}, & 0 < x < 2, \\ 1, & x \geq 2. \end{cases}
\]
于是 \(P\!\left(F(X) > \dfrac{1}{3}\right) = P\!\left(\dfrac{X^2}{4} > \dfrac{1}{3},\ 0 \leq X < 2\right) = P\!\left(X > \dfrac{2}{\sqrt{3}}\right) = \displaystyle\int_{\frac{2}{\sqrt{3}}}^{2} x\,\mathrm{d}x = \dfrac{2}{3}\)。
\end{note}
...
\begin{enumerate}
    ...
    \]
    \item 若 \(\mathrm{Cov}(X,Y) = 0\)，则 \(D(aX + bY) = a^2 DX + b^2 DY\)。
\end{enumerate}
\end{property}
...
\begin{note}
相关系数 \(\rho_{XY}\) 是刻画 \(X\) 与 \(Y\) 之间\textbf{线性相关程度}的度量。\(\rho_{XY} = 0\) 只表示不存在\textbf{线性}关系，但它们之间仍可能存在非线性关系。因此，\textbf{不相关不等于独立}。

当 \(X\) 与 \(Y\) 相互独立时，可推出 \(X\) 与 \(Y\) 不相关；但当 \(X\) 与 \(Y\) 不相关时，\(X\) 与 \(Y\) 却不一定相互独立。
\end{note}
...
\textbf{充分性}：\(D(X+Y) = DX + DY + 2\mathrm{Cov}(X,Y) = DX + DY \Rightarrow \mathrm{Cov}(X,Y) = 0 \Rightarrow X\) 与 \(Y\) 不相关。

\textbf{必要性}：$X$ 与 \(Y\) 不相关 \(\Rightarrow \mathrm{Cov}(X,Y) = 0 \Rightarrow D(X+Y) = DX + DY\)。
\end{solution}

\begin{exercise}\label{exer:4-13-stick}
将长度为 1\,m 的木棒随机地截成两段，则两段长度的相关系数为（\quad）。\par
A. 1\quad B. \(\dfrac{1}{2}\)\quad C. \(-\dfrac{1}{2}\)\quad D. \(-1\)
\end{exercise}

\begin{solution}
应选 D。设一段长为 \(X\)，另一段为 \(Y\)，则 \(X + Y = 1\)，即 \(Y = -X + 1$（$a = -1 < 0$），故 \(\rho_{XY} = -1\)。
\end{solution}
...
\(X\) 与 \(Y\) 之间存在确定性函数关系 \(Y = X^3\)，故不可能独立，排除 A、C。
...
\end{array}
\]
若事件 \(\{\max(X,Y)=2\}\) 与事件 \(\{\min(X,Y)=1\}\) 相互独立，则 \(\mathrm{Cov}(X,Y) =\,\underline{\hspace{2cm}}\)。\par
A. \(-0.6\)\quad B. \(-0.36\)\quad C. \(0\)\quad D. \(0.48\)
\end{exercise}
...
由此：
\[
P\{X=1,Y=0\} = P\{X=1\} - P\{X=1,Y=1\} = \frac{1}{4},\quad
P\{X=0,Y=1\} = P\{Y=1\} - P\{X=1,Y=1\} = 0,\quad
...
(2) \(Z = XY\) 的可能取值为 \(-1, 0, 1\)：
\[
P\{Z=-1\} = P\{X=1,Y=-1\} = \frac{1}{3},\quad
P\{Z=0\} = P\{X=0,Y=0\} = \frac{1}{3},\quad
P\{Z=1\} = P\{X=1,Y=1\} = \frac{1}{3}.
\]

(3) \(EX = \dfrac{2}{3}\)，\(DX = \dfrac{2}{9}\)；\(EY = 0\)，\(DY = \dfrac{2}{3}\)；\(E(XY) = 0\)。

故 \(\mathrm{Cov}(X,Y) = 0\)，\(\rho_{XY} = 0$。

此例说明 \(X\) 与 \(Y\) 不相关，但由联合分布可知 \(X\) 与 \(Y\) 并不独立。
\end{solution}
...
(3) \(EX_1 = 0\)，\(EX_2 = 0\)，\(DX_1 = DX_2 = 1\)。

\(E(X_1 X_2) = 1 \times \left(\dfrac{1}{6}+\dfrac{1}{6}\right) + (-1) \times \left(\dfrac{1}{3}+\dfrac{1}{3}\right) = -\dfrac{1}{3}\)。

\(\mathrm{Cov}(X_1,X_2) = -\dfrac{1}{3}\)，\(\rho_{X_1X_2} = -\dfrac{1}{3}\)。
\end{solution}

\begin{exercise}\label{exer:4-3-abs}
设随机变量 \((X,Y)\) 服从区域 \(G = \{(x,y) \mid 0 \leq x \leq 2,\ 0 \leq y \leq 2\}\) 上的均匀分布，\(U = |X-Y|\)。\par
(1) 求 \(U\) 的概率密度 \(f_U(u)\)；\par
(2) 求 \(EU\) 和 \(DU\)。
\end{exercise}

\begin{solution}
(1) \(F_U(u) = P(|X-Y| \leq u)\)。当 \(u \leq 0\) 时，\(F_U(u) = 0\)；当 \(u \geq 2\) 时，\(F_U(u) = 1\)。

当 \(0 < u < 2\) 时，区域 \(|x-y| \leq u\) 在 \([0,2]^2\) 内的面积为 \(4 - 2 \times \dfrac{1}{2}(2-u)^2 = 4u - u^2\)，故
\[
...
\begin{solution}
(1) \(F_Z(z) = P(X+Y \leq z) = \displaystyle\int_0^z \mathrm{d}x \int_0^{z-x} \lambda^2 e^{-\lambda x} e^{-\lambda y}\,\mathrm{d}y = 1 - (1+\lambda z)e^{-\lambda z}\)，故
\[
f_Z(z) = \begin{cases} \lambda^2 z\,e^{-\lambda z}, & z > 0, \\[4pt] 0, & z \leq 0. \end{cases}
\]
...
\[
f_Y(y) = \begin{cases}
\displaystyle\int_y^1 \frac{1}{x}\,\mathrm{d}x = -\ln y, & 0.5 \leq y < 1, \\[8pt]
\displaystyle\int_{0.5}^1 \frac{1}{x}\,\mathrm{d}x = \ln 2, & -0.5 \leq y < 0.5, \\[8pt]
\displaystyle\int_{-y}^1 \frac{1}{x}\,\mathrm{d}x = -\ln(-y), & -1 \leq y < -0.5.
\end{cases}
\]
...
设随机变量 \((X,Y)\) 服从二维正态分布，\(X\) 与 \(Y\) 的期望均为 \(\mu\)，方差均为 \(\sigma^2\)，相关系数 \(\rho_{XY} = 0\)。记 \(Z_1 = 2X+Y\)，\(Z_2 = 2X-Y\)，则 \(\rho_{Z_1Z_2} =\,\underline{\hspace{2cm}}\)。\par
A. \(0\)\quad B. \(\dfrac{3}{5}\)\quad C. \(\dfrac{3}{\sqrt{15}}\)\quad D. \(\dfrac{3}{\sqrt{10}}\)
\end{exercise}

\begin{solution}
应选 B。由 \(\rho_{XY} = 0\) 知 \(X\) 与 \(Y\) 相互独立。
\[
...
\begin{note}
切比雪夫不等式说明了方差越小，\(X\) 的取值越集中在 \(EX\) 附近。它在理论上具有重要价值，是大数定律的理论基础，但在具体估计概率时往往比较粗糙。
\end{note}

\begin{exercise}\label{exer:4-s10-cheby}
设 \(X\) 为随机变量，\(P(|X - EX| < \varepsilon) \geq 0.9\)，\(DX = 0.009\)，用切比雪夫不等式估计 \(\varepsilon\) 的取值范围。
\end{exercise}
...
\]
即 \(\dfrac{0.009}{\varepsilon^2} \leq 0.1\)，解得 \(\varepsilon \geq 0.3\)。
\end{solution}
...
\]
\[
P\{X=2\} = \mathrm{C}_5^2 \times 0.04 \times 0.512 = 0.2048,\quad P\{X \geq 3\} = 1 - \sum_{k=0}^{2}P\{X=k\} = 0.05792.
\]
...
\[
Y = \begin{cases} 500n + 300(X-n) = 300X + 200n, & X \geq n, \\[4pt] 500X - 200(n-X) = 700X - 200n, & X < n. \end{cases}
\]
\[
\begin{aligned}
