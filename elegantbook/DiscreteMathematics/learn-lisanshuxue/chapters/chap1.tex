\chapter{命题逻辑}
\section{命题与命题公式}
\begin{definition}{命题}{命题}
    命题是用陈述句表示的、非真即假且二者必居其一的判断语句
\end{definition}
...
除此以外，问句，感叹句，祈使句表示的语义不具备真值，不是命题。比如“多漂亮的花啊！”这句话并不是在断言一个可以判定真假的陈述，因此不是命题。
\begin{definition}{悖论}{}
悖论是表面上看似合理、但按照通常推理会导出矛盾或反直觉结论的命题、论证或语句；像“说谎者悖论”这类情形，甚至会出现若判其为真则推出其为假、若判其为假又推出其为真的现象。
\end{definition}
比如著名的理发师悖论等
...
\begin{align*}
    ...
    P \leftrightarrow Q &\Leftrightarrow \big((P \uparrow (Q \uparrow Q)) \uparrow (Q \uparrow (P \uparrow P))\big) \\
    &\qquad \uparrow \big((P \uparrow (Q \uparrow Q)) \uparrow (Q \uparrow (P \uparrow P))\big)
\end{align*}
类似地：
\begin{align*}
    ...
    P \leftrightarrow Q &\Leftrightarrow ((P \downarrow P) \downarrow Q) \\
    &\qquad \downarrow ((Q \downarrow Q) \downarrow P)
\end{align*}
由上面的等价式可知，单用$\uparrow$或单用$\downarrow$就能表示否定、合取、析取等基本联结词，因此它们各自都是全功能的；而删去这唯一的联结词后只剩空集，显然不再能表示这些联结词，所以$\{\uparrow\}$和$\{\downarrow\}$都是最小全功能联结词集。
\end{solution}
这里补充一下是怎么做的：
...
4. \textbf{对偶式的否定}：若$A=A(p_1,\ldots,p_n)$，则$\neg A(p_1,\ldots,p_n)\Leftrightarrow A^*(\neg p_1,\ldots,\neg p_n)$。也就是说，先将$A$中的原子命题取否定，再求对偶式。\\
...
\textbf{析取范式}是一个或若干个\textbf{简单合取式}（文字的合取）的析取，形如：$(P \land \neg Q) \lor (R \land S) \lor \cdots$\\
\textbf{合取范式}是一个或若干个\textbf{简单析取式}（文字的析取）的合取，形如：$(P \lor \neg Q) \land (R \lor S) \land \cdots$\\
...
\textbf{主析取范式}是由极小项组成的析取范式，且所含极小项全部且仅对应公式的成真赋值。\\
\textbf{主合取范式}是由极大项组成的合取范式，且所含极大项全部且仅对应公式的成假赋值。\\
任何命题公式都存在等价的唯一主析取范式和主合取范式。这里的“唯一”是指在命题变元顺序固定后，不计极小项/极大项的排列顺序和重复项。永真式的主合取范式为空（无极大项），永假式的主析取范式为空（无极小项）。
\end{definition}
所以由上述定义得知，主析取范式或主合取范式中不一定有极小项或者极大项。极小项一般用小写$m_i$表示，极大项一般用大写$M_i$表示；这些下标默认建立在命题变元顺序事先固定的前提下。
...
对析取范式（合取范式）使用分配律（可以理解为多项式乘法）可以得到等价的合取范式（析取范式）；若要求主合取范式或主析取范式，仍应按极大项/极小项重新整理。
\[(P \land Q) \lor (\neg P \land \neg Q) = (P\lor \neg P)\land (P\lor \neg Q)\land (Q\lor \neg P)\land (Q\lor \neg Q)= (P\lor \neg Q)\land (Q\lor \neg P) \]
