...
- 每个可编辑 unit 的 `rewrite_intent_coverage_status=PASS`：standalone unit 使用
  `humanize-unit-rewrite-bundle/v2`；transaction member 使用
...
`voice_profile_sha256` 和 masked text；`scaffold_metadata.json`（`humanize-rewrite-scaffold/v4`）明确
...
仍以每个 bundle 的当前 binding、正文和验证结果裁决，不把 sidecar 当完成台账。
...
```json
{
  "schema_version": "humanize-unit-rewrite-bundle/v2",
...
v2 的 `REWRITE` 必须形成完整 intent 图：每个 source span 和 target signal 都至少被一个 operation 引用；
source span 使用 normalized-LF 后、保留行尾的 frozen masked chunk 行字节计算 SHA-256。每个声明 span
...
无 `schema_version` 的旧 bundle 只读兼容：它仍走全部正文门，但接受后 intent 证据状态为 `REVIEW`，
`rewrite_intent_coverage_status=REVIEW`，不得形成正式交付或 Humanize 完成声明。显式 null、未知 schema、
...
BALANCED 的段落拓扑变化另走声明式窄门，不等于开放任意拆并段。只有 v2 `REWRITE` operation 精确使用
...
复核。结果记录 `process_containment=WINDOWS_JOB_OBJECT` 与
`descendant_cleanup=PASS`。POSIX 使用独立 process group 并在 wrapper 返回后清理同组后代，记录
`POSIX_PROCESS_GROUP`。后代清理失败使 `compile_check=FAIL`。只等待直接 shell、或只在返回前多
算一次 hash 都不合格：后台子进程可以等正式目录发布后再写入。
...
- `paired_quality_review_request_coverage_status=PASS`：只证明所有可编辑 unit 都有当前 request；
  ...
  正式交付；
- `rewrite_intent_coverage_status=PASS`：只证明所有 standalone unit 使用 unit v2、所有 executed
...
- [ ] 每个 `DONE/NO_CHANGE` unit 都有当前 rewrite-intent evidence；standalone v2 span 与 unit diff、
