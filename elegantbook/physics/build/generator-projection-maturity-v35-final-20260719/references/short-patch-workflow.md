...
verifier 重放 bundle application、重算 diff，并核对 validation/result/hash。`VERIFICATION PASS exit=0` 的范围严格为 `SELF_CONSISTENCY_ONLY`：它可以发现本地工件被单独篡改，但没有外部签名或时间戳，不证明历史真实性、文风收益、学术正确性或外部复核。不得从单个 `candidate.review.*`、diff、result 或 manifest 字段推断完成。
