...
def safe_output_path(path: Path, label: str = "output") -> Path:
    ...
    if absolute.exists() or absolute.is_symlink():
        raise ShortPatchError(f"output_exists:{absolute}")
    return absolute
...
def _locate(source: bytes, source_text: str, requested_start: Any, label: str) -> tuple[int, int]:
    ...
    if requested_start is None:
        ...
        if source.find(needle, start + 1) >= 0:
            raise ShortPatchError(f"{label}.source_text is ambiguous; provide start_byte")
    else:
...
def _atomic_write_new(path: Path, raw: bytes) -> None:
    ...
    try:
        ...
        try:
            ...
            raise ShortPatchError(f"output_exists:{path}") from error
    finally:
