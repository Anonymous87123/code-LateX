...
| `STRUCTURAL` | 明确授权后按结构映射跨段重排；长文自动面限同一 unit | 未获授权、无结构 plan、跨 unit/改标题/拆段/整段删除；必须交付 patch/diff |
...
8. 执行 [workflow.md](references/workflow.md) 第 3.5 节的成对质量门：逐段确认职责和论证层级仍在，并逐句核对每处变化是否对应预先登记的病灶；改后不得新增主语错位、硬被动、搭配错误、机械短句或新路径壳。validator `PASS/0` 不能替代这一步；无独立收益的句子先局部回退，整段仍不能稳定优于原文时恢复原段并记 `NO_CHANGE`。
...
STRUCTURAL 必须追加 `--intensity STRUCTURAL`。prepare 会冻结来源段清单；每个 `REWRITE`
bundle 必须按 [structural-rewrite-contract.md](references/structural-rewrite-contract.md) 提交
`structural_plan`。当前自动面固定 `scope=UNIT/title_lock=true/cross_unit=false`；公式只可随
来源段整体移动，不能离段。合法 plan 只证明机械映射；实际移动后
`structural_semantic_mapping=NOT_EVALUATED`，finalizer 生成 hash-bound 外部语义复核请求，顶层固定
`delivery_gate_status=REVIEW/2`，完整候选只写入 `rendered_review/`。本地模型理由、调用方
`HUMAN` 标签和 bundle 自填 clearance 均不能升级状态；禁止全文 Humanize 完成声明。
...
仍为 `NOT_EVALUATED`，同样不能由总体声明覆盖。读取 `candidate_assembly_status=PASS` 时必须同时
读取 `delivery_gate_status`、`publish_state` 与结构语义复核状态；`rendered_review` 不是正式交付目录。
