...
PowerShell 命令先执行 `$skillRoot = Join-Path $HOME '.codex\skills\humanize-academic-chinese'`；下列脚本均从该根调用，不依赖当前工作目录。
...
- [来源锚定的改写候选](#来源锚定的改写候选)
- [长文边界](#长文边界)
- [可执行 Voice Profile](#可执行-voice-profile)
- [完成条件](#完成条件)
...
在交付摘要中声明实际配置。`voice_profile` 是唯一字段；没有作者样本时写 `voice_profile=NONE; voice_disclosure=SCENE_DEFAULT`，不得声称保留了个人声线。短文用户直接粘贴正文时标 `source_kind=INLINE_TEXT`，粘贴检测标红片段时标 `INLINE_SELECTION`。
...
PATCH 必须满足 `patch_hunks_source_partition=NON_OVERLAPPING`：同一 source span 只能属于一个 patch hunk，`REWRITE hunk 不得包住另一个 UNRESOLVED span`。短 PATCH 的 `REWRITE` 还限于不超过 1200 UTF-8 bytes、单一物理行且至多一个 `。！？!?` 句末边界；若有句末标点，其后只能是空白或闭合引号/括号。多句、跨行或超长跨度必须拆分或转入长文 PATCH。先把冲突 span 从可改 hunk 中切出，再对前后安全句壳分别给精确删除/改写块；不得在一个大块中声称已改写，随后又把其中同一句重复列为原样未决。

短文从 CLEAN 降级为 PATCH 或用户直接要求可复核 PATCH 时，读取 [short-patch-workflow.md](references/short-patch-workflow.md)。优先用 `scaffold_humanize_short_patch.py create/finalize` 把 source span 只登记一次并确定性展开为 v2 selection；authoring v4 默认提出 source-bound clause 与完整句候选，并在任何 authoring 动作前解析、冻结 `requested_scene/resolved_scene`、route score/margin/reason、source binding 与 scene policy hash。`--scene AUTO` 必须使用冻结的 `resolved_scene`；用户显式指定场景时固定 `EXPLICIT`，启发式不得覆盖。AUTO 路由歧义固定 `ROUTE_REVIEW/2`，该 scaffold 不得 finalize；弱证据可明确回退 `GENERAL`。可用 `--suggest-spans NONE|CLAUSE|CLAUSE_AND_SENTENCE|SENTENCE|PARAGRAPH|SENTENCE_AND_PARAGRAPH|ALL|FOCUS` 调整。默认建议绑定 candidate high，并为 scanner 的 medium/low candidate 提供不进入 high coverage 的 advisory 边界；连续阅读发现词库外病灶时，不生成全篇候选目录，先写 strict `humanize-short-patch-focus/v1` 精确片段清单。`--focus-spec <json>` 是 additive：可与 `CLAUSE_AND_SENTENCE`、`ALL`、`NONE` 等模式一起传入，保留 scanner/advisory 视图并额外登记调用方已诊断的 FOCUS span；只有在只需要 FOCUS 时才使用 `--suggest-spans FOCUS --focus-spec <json>`。FOCUS 负责唯一定位、UTF-8 registry 和保护区拦截，不判断病灶或填写动作。CLAUSE 同时给 core、左分隔符附着和右分隔符附着边界，完整句用于句级改写；段级建议仍须显式请求。调用方只能显式选用一个边界。与引语、数学、代码、TeX 控制序列等保护内容重叠或超长时固定 SUPPRESSED。建议不自动生成 hunk、replacement、reason、selection、conflict 或用户授权。`.txt` 中实际使用 TeX 语法时，create/finalize/build 均显式传 `--document-format TEX`。再用 `build_humanize_short_patch.py` 冻结精确 UTF-8 source span、`apply_humanize_short_patch.py` 生成 `candidate.review.* + review.md`，最后用 `verify_humanize_short_patch.py` 检查闭集、重算 review 并按当前安装的 validator policy 重放归档 before/after。普通 v1 只证明列出的 hunk；需要机械 coverage 时使用 `humanize-short-patch-selection/v2` 与 `humanize-short-patch-coverage/v2`，按 `AUTO` audit view 把 scanner 的全部 high 命中、调用方绑定 selection 和调用方显式 conflict pair 逐项映射为 hunk、KEEP、PROTECTED、EXCLUDED 或 UNRESOLVED。PROTECTED/EXCLUDED high 和其他保护跨度不得被可变 hunk 改写，selection KEEP 也不得被 hunk 改动。coverage 的 `coverage_completion_claim_allowed=true` 仅限 `ENUMERATED_HIGH_AND_BOUND_DECLARATIONS_ONLY`，并同时披露任务 scene、`scan_scene=AUTO` 和 source kind；advisory/focus suggestion 只有在调用方显式选入后才成为 bound declaration。authoring 固定披露 `CALLER_CONTROLLED_SELF_CONSISTENCY_ONLY`，不能证明 focus 诊断真实、create 后未被同一调用方协同重封或用户已授权。authoring scaffold 和 `review.md` 只减少录入/审阅成本，不证明语义完整性或质量。REPORT_SELECTION 仍必须走 extractor/report-scope 管线，不能在短 PATCH coverage 中自填来源标签。

authoring 对带 finding 的建议只发放完整覆盖该 finding 的 `AVAILABLE` 边界；跨边界窄视图固定 SUPPRESSED。advisory/focus 建议的 `finding_ids=[]`，不得计入 high 处置率。完全相同的可用 byte range 跨 kind 只保留最局部视图，selected spans 两两不得重叠。TeX 控制序列及其嵌套参数在 Markdown/TeX 中均按完整命令调用保护；TeX 模式还把控制序列后由未转义 `%` 注释续接到下一行的参数纳入同一保护跨度。`AUTO` 在 CLI 与 Python API 中都按后缀推断，TeX 风格 `.txt` 仍须显式指定 `TEX`。

需要检查工作文件时可以追加 `--live-source <path>`，但证据包不绑定调用方提供的绝对路径：匹配也只能记为 `CALLER_SUPPLIED_MATCH_UNVERIFIED`，漂移为 `CALLER_SUPPLIED_NOT_CURRENT`，缺失或不可读为 `UNAVAILABLE`，三者均固定 `REVIEW/2`，均不能声称当前工作文件或新鲜度已确认。不得手写一个无 hash/offset 的列表后声称已验证；applicator 必须读取冻结 source 快照运行统一验证器。同 policy 下核心结果、finding、coverage inventory 或 request SHA 重算不一致为 `FAIL/1`，policy drift 为 `REVIEW/2`。合法候选仍固定 `DELIVERY REVIEW exit=2`；verifier `PASS/0` 只证明未传 live source 时闭集工件可在当前 policy 下重放且本地自洽，机械结构或 coverage PASS 不等于文风质量 PASS。

已有 short-PATCH review 只需修改少数既有 hunk 时，使用 `amend_humanize_short_patch.py create/apply`，不要重建 span、selection 和未变 hunk。amend 只开放指定 hunk 的 `decision/replacement/reason`；scene、强度、保护词、source span、hunk 拓扑、coverage declarations、selection 和 conflict 全部冻结。父 review 必须在不传 live source 时通过当前 verifier 与 coverage replay；任何 caller-supplied `--live-source`（即使字节匹配）都使 amend 返回 `REVIEW/2`，并附带 `CALLER_SUPPLIED_MATCH_UNVERIFIED`、`UNVERIFIED_CALLER_PATH` 与 `freshness_claim_allowed=false`。匹配时 create 可以写出 `PENDING_AMENDMENT` scaffold，apply 可以写出派生待审 review，但不得升级为 PASS；漂移或不可用在发布前阻断且不写出新产物。无 live source 时只证明冻结闭集 self-consistency，不暗示工作文件 current。新 v3 bundle 嵌入最多 8 层父血缘并逐层复核，仍只产生待审候选。
...
同一句“下面说明模型如何比较两种方案”不足以定场景。结合所在文档与用户目的；没有专属证据或只有一个场景的唯一弱证据时保守回退 `GENERAL`。只要两个正分场景平局或差距低于 policy margin，无论总分是否达到强路由阈值都标 `AMBIGUOUS/UNRESOLVED`；document prior 不得替低分平局选边。来源角色不明、用户指定场景与明确用途冲突或保护/权限无法裁决时同样用 `UNRESOLVED`。
...
```powershell
python "$skillRoot\scripts\extract_detector_report_scope.py" report.html --source manuscript.md --output report_scope.json
python "$skillRoot\scripts\validate_humanize_output.py" manuscript.md revised.md --scene GENERAL --report-scope report_scope.json --format json
```
...
| 短文 PATCH | `short-patch-workflow.md` + scaffold/build/apply/verify 四个 short-patch 脚本；v2 coverage 由 `humanize_short_patch_coverage.py` 重算；verify 必须重算 `review.md`、检查闭集并执行 `CURRENT_POLICY_REPLAY`；`--live-source` 仅作 caller-supplied 字节比较，匹配也保持 `REVIEW/2`，不能提供新鲜度证明；只发布待审派生目录，verify PASS 仍为 SELF_CONSISTENCY_ONLY |
...
```powershell
python "$skillRoot\scripts\validate_humanize_output.py" <old-or-supplied> <new-or-draft> --mode <REWRITE|DRAFT> --scene <SCENE> --format text
```
...
需要让短文验证结果可复核时，追加 `--evidence-dir <目标目录>`。`humanize-direct-validation-evidence/v3` 证据包使用 `humanize-validation-invocation/v2` 和 `hvr2-*` 内容寻址 run ID，原子保存 `inputs/before.bin`、`inputs/after.bin`、
...
`UNRESOLVED/REVIEW`。BALANCED 只有在 unit v3 intent 以唯一精确 source span 声明
...
文本摘要第一行固定为权威 `DELIVERY <status> exit=<code> publish=<state>`；需要完整审计时省略 `--format text` 读取
...
canonical 证据；每次先读 `latest_attempt_metadata.json` 判断最近尝试，失败时它与去路径化的
`last_failed_attempt_metadata.json` 一致，旧 `finalization_metadata.json` 仅表示被保留的上一候选。
失败回滚头中的命令、stdout/stderr、cwd、源路径和全部绝对路径字符串必须清空；若本轮 ledger 已回滚，
`unresolved_reason_summary.details_artifact` 必须为空并标记 `NOT_RETAINED_AFTER_ROLLBACK`，不得指向旧候选台账。
`finalization_metadata.json`、`last_failed_attempt_metadata.json` 与 `latest_attempt_metadata.json` 必须始终作为同一 authority closure 发布；未变化的第三成员也进入 generation 描述。全部 old/new 字节先写入同目录 transaction，持久化 `PREPARED` journal 后安装目标，最后只以 `.humanize-authority-group-commit.json` 的原子替换作为提交点。重启看到 pointer 仍为 base generation 时统一恢复 old；pointer 已为 next generation 时统一补全 new；pointer、journal、成员 hash 或 generation 落在第三状态时不覆盖目标并硬失败。`KeyboardInterrupt/SystemExit` 发生在事务内也必须先按 pointer 恢复，再保留原中断语义。单个 metadata、mtime、目录存在或 journal 删除均不是权威提交证据。该协议面向受控本地文件系统上的进程崩溃恢复，不证明恶意同权限写者不可伪造整套工件、文件系统整体回退或物理断电下目录项持久顺序。

rendered、validation、diff、ledger、rollback manifest 等非 authority 工件另由
`humanize-run-state-journal/v2` 保护：finalize 在任何发布前封存完整 base image；authority pointer 切换前
再封存完整 next image，闭包同时记录文件字节、空目录和 file/directory 类型。恢复只能按唯一 authority
pointer 选择 base 或 next image 幂等重装；缺 image、hash/类型漂移、第三 pointer、journal 绑定不一致均保留
证据并硬失败。清理阶段使用 pointer 与 live closure 绑定的 cleanup marker；事务目录删除前后被
`KeyboardInterrupt/SystemExit` 打断都必须可重启完成。基础设施前缀只解释 run 根目录的首组件，不得吞掉
普通子目录中的同名前缀工件；无法认证的 staging 前缀碰撞只 fail closed，不自动删除。finalize 的 authority
writer 必须在任何 authority recovery 或写入前确认匹配的 run-state journal，缺失时不得“先修复再报错”。
这套协议证明受控本地文件系统上的工件闭包恢复，不证明 ACL、owner、mtime、xattr、alternate streams、
恶意同权限写者或物理断电持久顺序。

若失败记录本身无法持久化，stdout 必须声明
`attempt_metadata_persistence_status=FAILED` 与 `paths_authoritative=false`，不能读取旧 latest 冒充本次失败。
每个 UNRESOLVED 单元必须携带闭集 `unresolved_codes`；顶层结构化统计只读该数组，不从 notes、用户字段、
validator 正文或异常正文猜测 code；静态测试必须覆盖所有 `unit["status"]="UNRESOLVED"` 赋值分支，并要求同一控制分支在退出前调用 registry 入口。final ledger 以 canonical JSON 写入 `unresolved_codes_json`；所有 CSV 文本单元对 `= + - @ TAB CR` 前缀做 spreadsheet-safe 转义，避免人工用表格软件打开时执行公式或外链。旧 final ledger 仅在明确包含唯一 `unit_id`、合法 `status` 时兼容读取，缺列、重复 ID 或未知状态必须拒绝，不做启发式迁移。
运行时异常只公开固定 error code，不回显绝对路径或异常正文。实际运行
...
骨架使用 `humanize-rewrite-scaffold/v5`，普通 unit 使用
`humanize-unit-rewrite-bundle/v3`，并逐字保留骨架生成的
`humanize-long-authoring-binding/v1`。骨架器只接受 strict schema 2 prepare integrity；合法 v1 历史
run 返回 `legacy_prepare_requires_reprepare / REVIEW/2`，不得签发 v5。v5 把冻结 PENDING 全集、原始模板
SHA、immutable prepare closure、source/scene/Voice 和 live-source 状态绑定到 sidecar；finalizer 从 prepare
工件重建这些值，不信任调用方协调重封。`REWRITE` 必须填写 `rewrite_intent.summary/operations/source_spans/target_signals`；
...
v2 或无 schema 旧 bundle 只读兼容；v2 即使 intent 机械覆盖通过，也因缺少 v3 authoring binding 保持
`authoring_binding_status=REVIEW`，无 schema 另保持 `rewrite_intent_coverage_status=REVIEW`；两者都阻断
正式交付和 Humanize 完成声明。intent PASS 不证明理由真实或改后更自然。
若 v5 scaffold 的某两个 `REWRITE` unit 改由冻结 transaction execution 接管，删除且只删除这两个
standalone JSON，保留 sidecar 原记录，再加入绑定同一 member 的 transaction bundle；不能同时保留
standalone，不能以 `NO_CHANGE` record 替换，也不能用 decline 代替两个 unit 自身的 bundle。
...
```powershell
python "$skillRoot\scripts\build_humanize_voice_profile.py" --sample-spec <samples.spec.json> --allowed-root <root> --profile-id <id> --scene AUTO --manifest-out <voice-manifest.json> --output <voice-profile.json> --format json
python "$skillRoot\scripts\validate_humanize_voice_profile.py" <voice-profile.json> --manifest <voice-manifest.json> --sample-spec <samples.spec.json> --allowed-root <root> --rebuild-evidence --format json
```
...
```powershell
python "$skillRoot\scripts\prepare_humanize_long_document.py" <main.tex|doc.md> --output <empty-run-dir> --scene <SCENE> --voice-profile <voice-profile.json> --voice-profile-sha256 <64-hex> --voice-manifest <voice-manifest.json> --voice-sample-spec <samples.spec.json> --voice-allowed-root <root>
```
...
- `REWRITE`：目标病灶下降，作者声线与保护项未漂移，硬不变量通过；若要声明质量完成，还必须有 `rewrite_intent_coverage_status=PASS`，并有与当前 request 和全部变化绑定的可信外部 paired-quality clearance。本地模型自检、盲读记录、`PASS/0` 或 fresh second pass 都不能单独满足这一条件。
