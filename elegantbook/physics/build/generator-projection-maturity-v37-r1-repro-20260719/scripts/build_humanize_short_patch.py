...
def validate_bundle_payload(
    ...
    *,
    ...
    if schema == BUNDLE_SCHEMA:
        ...
        raise ShortPatchError(
            f"bundle.schema_version must be {BUNDLE_SCHEMA} or {BUNDLE_SCHEMA_V2}"
        )
    ...
    if source is not None:
        ...
        if schema == BUNDLE_SCHEMA_V2:
            try:
                coverage_layer.validate_record_integrity(source, payload["coverage"])
                replay_status = "PASS"
...
def build_bundle(source_path: Path, spec_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    document_format = "tex" if source_path.suffix.casefold() in {".tex", ".ltx"} else "markdown"
