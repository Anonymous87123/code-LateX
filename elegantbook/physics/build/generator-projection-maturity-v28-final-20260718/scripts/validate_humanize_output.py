...
DIRECT_VALIDATION_EVIDENCE_SCHEMA = "humanize-direct-validation-evidence/v2"
DIRECT_VALIDATION_EVIDENCE_ERROR_SCHEMA = "humanize-direct-validation-evidence-error/v1"
VALIDATION_INVOCATION_SCHEMA = "humanize-validation-invocation/v1"
...
def _capture_record(path: Path, raw: bytes, *, archive_path: str) -> dict[str, Any]:
    return {
        ...
        "original_path_sha256": _sha256(str(path).encode("utf-8")),
        "sha256": _sha256(raw),
        "size": len(raw),
    }
...
def _build_invocation_request(
    ...
    *,
    ...
    if report_scope_path is not None:
        ...
        if _sha256(scope_raw) != str(scope_check.get("scope_sha256", "")):
    ...
    warning_review = payload.get("warning_review")
    ...
    if warning_resolutions:
        replay_reasons.append("WARNING_REVIEWER_ID_NOT_ARCHIVED")
    if warning_review.get("reviewer_id_sha256"):
        replay_reasons.append("WARNING_REVIEWER_ID_NOT_ARCHIVED")
    ...
    invocation_body = {
        ...
        "arguments": {
            ...
            "warning_review_request_sha256": str(
                warning_review.get("warning_review_request_sha256") or ""
            ),
            "warning_reviewer_kind": str(
                warning_review.get("reviewer_kind", "NONE")
            ),
            "warning_reviewer_id_sha256": str(
                warning_review.get("reviewer_id_sha256") or ""
            ),
...
def _persist_evidence_bundle(
    ...
    *,
    ...
    if run_id != f"hvr1-{invocation_sha}":
    ...
    if report_scope_path is not None:
        ...
        report_raw = report_path.read_bytes()
        for key, raw in (("report_scope", scope_raw), ("report", report_raw)):
        ...
        input_artifacts["inputs/report-scope.json"] = scope_raw
    ...
    if output_dir.exists() or output_dir.is_symlink():
        ...
        if actual_paths != set(expected_files):
            raise ValueError("evidence run_id conflict: existing file inventory differs")
        for name, expected_raw in expected_files.items():
            ...
            if path.read_bytes() != expected_raw:
                raise ValueError(f"evidence run_id conflict: existing bytes differ: {name}")
...
def _report_scope_invariants(
    ...
    *,
    ...
    semantic_scope_payload = dict(payload)
    semantic_scope_payload["report_path"] = "<ARCHIVED_REPORT>"
    return {
        "status": "PASS" if outside_unchanged else "FAIL",
        "scope_path": str(report_scope_path.resolve()),
        "scope_sha256": _sha256(scope_raw),
        "scope_semantic_sha256": _sha256(
            _canonical_json_bytes(semantic_scope_payload)
        ),
