...
COVERAGE_SCHEMA = "humanize-short-patch-coverage/v1"
...
def _normalized_declarations(source: bytes, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for index, item in enumerate(conflicts_raw):
        ...
        if left == right or left in conflict_members or right in conflict_members:
            raise CoverageError("explicit conflict members must be distinct and non-reused")
        conflict_members.update((left, right))
        conflicts.append(
            {
...
def build_coverage(
    ...
    *,
    ...
    for finding in findings:
        start = byte_offsets[int(finding["start_char"])]
        end = byte_offsets[int(finding["end_char"])]
        role = "PROTECTED" if finding["protected"] else "EXCLUDED" if finding["excluded"] else "CANDIDATE"
        anchor = {
        ...
        if role == "PROTECTED":
            disposition = {
                "decision": "PROTECTED",
                "hunk_id": None,
                "reason": finding["protection"],
            }
        elif role == "EXCLUDED":
            disposition = {
                "decision": "EXCLUDED",
                "hunk_id": None,
                "reason": finding["exclusion"],
            }
        else:
            covering = [
                hunk
...
def validate_record_integrity(
    ...
    if recorded.get("schema_version") != COVERAGE_SCHEMA:
    ...
    _exact_fields(source_record, SOURCE_FIELDS, "coverage source")
    ...
    if not isinstance(policy, dict) or set(policy) != set(current_policy_hashes()):
        raise CoverageError("coverage policy_hashes fields are invalid")
    if any(not isinstance(value, str) or not re.fullmatch(r"[0-9a-f]{64}", value) for value in policy.values()):
...
def replay_coverage(
    ...
    *,
    ...
    validate_record_integrity(source, recorded)
    policy = recorded.get("policy_hashes")
