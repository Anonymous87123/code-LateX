...
短文从 CLEAN 降级为 PATCH 或用户直接要求可复核 PATCH 时，读取 [short-patch-workflow.md](references/short-patch-workflow.md)。优先用 `scaffold_humanize_short_patch.py create/finalize` 把 source span 只登记一次并确定性展开为 v2 selection；authoring v2 默认提出 source-bound clause 与完整句候选，可用 `--suggest-spans NONE|CLAUSE|CLAUSE_AND_SENTENCE|SENTENCE|PARAGRAPH|SENTENCE_AND_PARAGRAPH|ALL` 调整。CLAUSE 同时给 core、左分隔符附着和右分隔符附着边界，完整句用于句级改写；段级建议仍须显式请求。调用方只能显式选用一个边界。建议只覆盖 candidate high 的上下文；与引语、数学、代码、TeX 控制序列等保护内容重叠或超长时固定 SUPPRESSED。它不自动生成 hunk、replacement、reason、selection、conflict 或用户授权。`.txt` 中实际使用 TeX 语法时，create/finalize/build 均显式传 `--document-format TEX`。再用 `build_humanize_short_patch.py` 冻结精确 UTF-8 source span、`apply_humanize_short_patch.py` 生成 `candidate.review.* + review.md`，最后用 `verify_humanize_short_patch.py` 检查闭集、重算 review 并按当前安装的 validator policy 重放归档 before/after。普通 v1 只证明列出的 hunk；需要机械 coverage 时使用 `humanize-short-patch-selection/v2` 与 `humanize-short-patch-coverage/v2`，按 `AUTO` audit view 把 scanner 的全部 high 命中、调用方绑定 selection 和调用方显式 conflict pair 逐项映射为 hunk、KEEP、PROTECTED、EXCLUDED 或 UNRESOLVED。PROTECTED/EXCLUDED high 和其他保护跨度不得被可变 hunk 改写，selection KEEP 也不得被 hunk 改动。coverage 的 `coverage_completion_claim_allowed=true` 仅限 `ENUMERATED_HIGH_AND_BOUND_DECLARATIONS_ONLY`，并同时披露任务 scene、`scan_scene=AUTO` 和 source kind；authoring 固定披露 `CALLER_CONTROLLED_SELF_CONSISTENCY_ONLY`，不能证明 create 后未被同一调用方协同重封。authoring scaffold 和 `review.md` 只减少录入/审阅成本，不证明用户授权、语义完整性或质量。REPORT_SELECTION 仍必须走 extractor/report-scope 管线，不能在短 PATCH coverage 中自填来源标签。

authoring 只发放完整覆盖所绑定 finding 的 `AVAILABLE` 建议；跨边界窄视图固定 SUPPRESSED。完全相同的可用 byte range 跨 kind 只保留最局部视图，selected spans 两两不得重叠。TeX 控制序列及其嵌套参数在 Markdown/TeX 中均按完整命令调用保护；TeX 模式还把控制序列后由未转义 `%` 注释续接到下一行的参数纳入同一保护跨度。`AUTO` 在 CLI 与 Python API 中都按后缀推断，TeX 风格 `.txt` 仍须显式指定 `TEX`。
