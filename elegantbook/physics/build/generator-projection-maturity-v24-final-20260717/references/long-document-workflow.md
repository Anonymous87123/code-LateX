...
不等于执行请求或语义 clearance。transaction bundle 必须以
`humanize-structural-transaction-bundle/v1` 精确回显某个冻结 `STX-*` 的 ID、binding、inventory
...
`delivery_gate_status=REVIEW/2`、`coverage_completion_claim_allowed=false`，不得发布正式
...
`humanize-structural-transaction-review-request/v1`。任一 member、保护、fragment、DOCUMENT 或
...
组装可以是 `candidate_assembly_status=PASS`，但顶层必须为 `delivery_gate_status=REVIEW/2`，候选
