...
只用用户已提供的内容起草。可选择顺序与详略，不补造数据、原因、经历或结论。输出合同与 Rewrite 分开，不假装存在“改前/改后”。把 supplied content 作为验证器的第一个 artifact，使用 `--mode DRAFT`；表面载荷门只检查新增数字、单位、数学、代码、引语、归因和显式保护术语，不证明自然语言蕴含。`semantic_source_check=NOT_EVALUATED` 时只能写“未发现已编码的表面新增载荷，语义来源待复核”，不得写“已证明未补造”。
...
收尾器先校验 prepare 生成的 `prepare_integrity.json`，再逐单元调用统一验证器，只向快照派生目录发布结果，不覆盖源文件。完整性清单缺失或任何 prepare 产物被改动时直接 `FAIL`；它只是审计辅助，收尾器还必须从冻结 `source/`、预算、保护跨度、原始范围和 canonical chunks 独立重建初始状态，拒绝同时伪造 `units.jsonl` 与封条。相同 `run-dir` 的 finalize 通过跨进程锁串行执行；空文件集、零可处理单元或只有保护内容只能是 `REVIEW`。可选 `--check-command` 在一次性检查副本中运行，真实正文 staging、待发布 validation/diff 证据和其他 run 产物的任何新增、删除或修改都会 `FAIL`。`coverage_completion_claim_allowed=true` 只证明覆盖与局部保护闭合；只有 `humanize_completion_claim_allowed=true`（兼容字段 `full_completion_claim_allowed`）时才能声称全文 Humanize 完成。当前 Voice 绑定、全文声线/跨块重复门和 fresh second pass 未实现时，后两字段必须为 false；`rendered_partial`、`PENDING`、`UNRESOLVED`、`SKIPPED_GARBLED`、`CHANGED_AFTER_SNAPSHOT`、验证 `REVIEW/FAIL` 或编译未通过都不得被总体声明覆盖。
