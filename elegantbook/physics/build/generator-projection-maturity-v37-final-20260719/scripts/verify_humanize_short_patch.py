...
OPTIONAL_ARTIFACTS = {"coverage.json"}
...
def verify_directory(
    ...
    *,
    ...
    coverage_present = bundle.get("schema_version") == short_patch.BUNDLE_SCHEMA_V2
    ...
    current_result = result_fields == applicator.CURRENT_RESULT_FIELDS
    evidence = validation.get("evidence")
    if not isinstance(evidence, dict):
        raise ShortPatchError("validation evidence is missing")
    ...
    base = {
        ...
        "coverage_completion_claim_allowed": bool(
            coverage_present and coverage_replay_status == "PASS"
        ),
        "coverage_claim_scope": (
