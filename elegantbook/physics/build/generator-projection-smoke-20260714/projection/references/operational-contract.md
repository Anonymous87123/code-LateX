...
把“保留原意”理解为编辑边界：不要擅自改变输入已经表达的数字、术语、符号、指代、限定、否定、比较方向和言语行为。不要把它扩展为对这些内容作真伪判断。
...
`REPORT_INFORMED` 不改变片段本身的来源角色。报告标红的直接引语仍是 `quoted`，公式仍是 `math`，作者正文才是 `author`。报告提供的“高风险、疑似 AI、82%”等标签全部是 `report-metadata`，不能把保护区降格为可编辑正文。
...
- 长文以 `finalization_metadata.json` 为完成证据。`rendered_partial` 只是派生草稿；只有 `full_completion_claim_allowed=true`、不存在 `PENDING/IN_PROGRESS/UNRESOLVED/SKIPPED_GARBLED/CHANGED_AFTER_SNAPSHOT`，且编译/格式门未失败时，才允许全文完成声明。
