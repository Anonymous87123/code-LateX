...
冲突降级 PATCH 还要记录 `patch_hunks_source_partition=NON_OVERLAPPING`。同一 source span 只能属于一个 patch hunk，`REWRITE hunk 不得包住另一个 UNRESOLVED span`；先切出原样未决句，再分别处理它前后的安全句壳，不能用重叠大块制造“既改写又原样保留”的假补丁。
