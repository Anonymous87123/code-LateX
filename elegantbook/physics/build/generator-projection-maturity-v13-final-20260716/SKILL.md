...
语料动作锚来自 `corpus-action-sources.json` 中已通过来源可读性和行级锚点检查的 MD/TeX 动作卡。`COURSE/MODELING/RESEARCH` 的普通 `REWRITE/DRAFT` 从当前场景文件选择至多两张满足输入条件的正向卡；没有适用卡时明确 `NONE_APPLICABLE`。`GENERAL` 当前固定 `corpus_action_support=NONE`，不列卡、不把 negative guard 当正向证据；它只按原稿内生习惯与 Voice Profile 作局部动作。卡片只影响信息组织，不授权复制来源句子或迁移来源事实。
