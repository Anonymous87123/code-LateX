...
v13 已在 hash-bound Voice Profile、unit/chunk 回显和 FRAGMENT/DOCUMENT 双层结构门上增加两个全文门：注册 Voice 特征非回退门，以及词典修复句壳与真实 MD/TeX 负例卡驱动的跨 unit 新增重复门。它们能拒绝已编码的机械声线回退和跨块修复模板，但不判断完整作者气质。fresh 第二遍生成仍未实现，因此即使两个新门与局部覆盖均 PASS，`humanize_second_pass_convergence=NOT_RUN`，`humanize_completion_claim_allowed=false`，兼容字段 `full_completion_claim_allowed=false`。
...
跨 unit 门先运行 `LEX-REPAIR-01`，再运行当前 `corpus-action-sources.json` 中来源仍可读、
状态为 `AVAILABLE` 且适用于当前场景的 `negative_guard`。`positive_action` 永远不执行为
detector。匹配视图使用 NFKC，删除零宽字符与汉字内部空格，但不跨保护占位符、段落或结构
...
lexicon、action profile、catalog 和 detector 定义 hash。partial 范围或任一适用负例 guard
不可用时只能 `REVIEW`。
