对质点的瞬时角动量表达式 $\vec{L} = \vec{r} \times \vec{p}$ 两边关于时间 $t$ 求一阶导数，应用微分的乘积法则展开：
\[
...
\textbf{积分形式}：在时间区间 $t_1 \to t_2$ 内对等式两边积分，可得角动量定理的冲量矩形式 \(\vec{L}_2 - \vec{L}_1 = \int_{t_1}^{t_2} \vec{M}\mathrm{d}t\) 
当考察由 $n$ 个质点组成的系统时，整个质点系的总角动量定义为各个质点角动量的矢量和：
