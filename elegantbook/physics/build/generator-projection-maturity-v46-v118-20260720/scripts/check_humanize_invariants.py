...
LOGICAL_RELATION_ROUTE_SHELL_RE = re.compile(
    r"^\s*(?:由此可见|由此得到|由此可得|据此可得|因此|所以|于是|从而|因而)"
    r"[，,]?\s*(?:本节|本章|本文|下文)"
    r"(?:首先|随后|接着|将|主要)?"
...
# A registered exclusion describes a whole lexical span, not only the marker
# under which the word was first catalogued.  For example, both "不" and the
# internal "可" in "不可约" are non-speech-act characters.  Applying the
# union to every single-character marker prevents cross-marker leakage while
# keeping the set of exempt words explicit and reviewable.
SINGLE_MARKER_EXCLUSION_WORDS: tuple[str, ...] = tuple(
    sorted(
        {word for words in SINGLE_MARKER_EXCLUSIONS.values() for word in words},
    ...
    for category, markers in SPEECH_ACT_MARKERS.items():
        ...
        for match in pattern.finditer(text):
            ...
            if sentence is None or claim is None:
                continue
            if category == "logical_relation" and LOGICAL_RELATION_ROUTE_SHELL_RE.match(
                str(sentence["text"])
            ):
                continue
...
def _inside_excluded_word(text: str, index: int, _marker: str) -> bool:
    for word in SINGLE_MARKER_EXCLUSION_WORDS:
