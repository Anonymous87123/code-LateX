...
需要让短文验证结果可复核时，追加 `--evidence-dir <空目录路径>`。验证器会在同一原子目录中保存
`validation-result.json`、完整 `paired-quality-review-request.json`（适用时）和
`evidence-manifest.json`；manifest 绑定改前/改后 SHA、request SHA、状态、退出码和每个文件的 SHA-256。
目录已存在、来源文件在验证后变化、路径经过 symlink/reparse point 或写入中断时拒绝覆盖并返回结构化
`FAIL/1`；不得把 stdout 中的临时 JSON 当成唯一质量证据，也不得手动复制 request 后声称它仍与当前
artifact 绑定。
