...
`structural_transaction_candidate_coverage_status` 和 `structural_transaction_scope_complete`。
...
- transaction bundle、prepare/finalizer、validator/invariant/scanner/lexicon 与 transaction policy
  hash。
...
| `READY` 候选有合法 decline，且 member 分别完成 `REWRITE/NO_CHANGE` | disposition `DECLINED` | `PASS`（未发生移动） | 候选覆盖可 PASS；最终交付仍由 unit、Voice、重复和其他门决定 |
...
10. 实际移动时读取 unit 或 transaction review request，披露
