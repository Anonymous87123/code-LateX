...
```powershell
python "$skillRoot\scripts\prepare_humanize_long_document.py" manuscript.tex `
...
正式 transaction 使用 strict JSON schema `humanize-structural-transaction-bundle/v2`：

```json
{
  "schema_version": "humanize-structural-transaction-bundle/v2",
  "transaction_id": "STX-...",
  ...
  "fragments": [
    {
      ...
      "local_rewrite_intent": {
        "decision": "REWRITE",
        "rewrite_intent": {
          "summary": "删除移入片段中的空泛强调壳并保留原有观察对象",
          "operations": [
            {
              "id": "O1",
              "kind": "REWRITE_STYLE_SHELL",
              "source_span_ids": ["S1"],
              "target_signals": ["STYLE-TRANSACTION-LOCAL-1"],
              "summary": "删除空泛强调壳并保留原有观察对象"
            }
          ],
          "source_spans": [
            {
              ...
              "start_line": 3,
              "end_line": 3,
              "sha256": "<结构基线第 3 行（保留行尾）的 SHA-256>"
            }
          ],
          "target_signals": ["STYLE-TRANSACTION-LOCAL-1"]
        }
        ...
        "evidence_spans": [
          {
            ...
            "start_line": 1,
            "end_line": 1,
            "sha256": "<结构基线第 1 行（保留行尾）的 SHA-256>"
          }
        ]
      },
      "target_groups": [
...
`target_groups` 描述跨 member 的结构动作；`local_rewrite_intent` 描述目标 fragment 在结构基线之上
是否另改措辞。finalizer 先按已校验的复合 source refs 生成“只移动、不改字”的目标结构基线，再按
normalized-LF、1-based 行号验证本字段：

- `REWRITE` 必须使用与 unit v3 相同的完整 intent 图；每个 span 必须命中结构基线，且与实际局部
  masked diff 双向覆盖；
- `NO_CHANGE` 必须给具体理由和 evidence span，且 `masked_text` 逐字等于结构基线；它不否认
  transaction 发生了跨 unit 移动，只声明该 fragment 没有额外措辞编辑。

两个 fragment 可以分别选择 REWRITE 或 NO_CHANGE。结构移动理由不能代替局部措辞 intent，局部
intent 也不能代替 `target_groups` 的全局来源覆盖。`humanize-structural-transaction-bundle/v1` 仅保留
只读兼容：可形成待审结构候选，但两个 member 的 intent evidence 固定为 `REVIEW`，不能使全文
`rewrite_intent_coverage_status=PASS`。v2 缺字段、显式 null/未知 schema、空泛 NO_CHANGE、错误 span、
声明外 diff 或 NO_CHANGE 偷改均按整笔 transaction 回滚；不得自动降级成 v1。
...
v2 还要求两个 `fragment_rewrite_intent_status=PASS`。v1 的 intent 状态保持 REVIEW，但这是兼容候选，
不是可贡献全文 intent coverage PASS 的正式路径。
...
相邻 pair 的 v2 bundle 生成一个 transaction-level
`humanize-structural-transaction-review-request/v2`，而不是两份可被分别放行的 unit request；v1
兼容 bundle 仍生成 v1 request。v2 request 还
必须绑定：
...
- transaction bundle、prepare/finalizer、validator/invariant/scanner/lexicon/report-extractor/runtime 与 transaction policy
  hash。
- 两个 `local_rewrite_intent`、fragment canonical hash 与 intent-diff-binding hash。

每个 member 同时生成 `humanize-transaction-fragment-rewrite-intent-evidence/v1`，绑定 transaction
bundle/fragment、chunk/Voice、source member、结构基线、masked/restored candidate、member diff、
paired-quality request 与 transaction review request。final metadata 的 paired-quality record 反向给出
该 evidence path/hash。事务因覆盖缺口或后置 repetition 回滚时，两侧 intent evidence 与两类 request
必须一起撤销，不能让旧 PASS 继续占据 coverage。
...
| v2 transaction 的两个 local intent 均 PASS | intent coverage 可计入两个 member | 不改变结构语义状态 | 仍待 transaction semantic 与 paired-quality 外部复核 |
| v1 transaction 机械 PASS | 两 member intent `REVIEW` | `NOT_EVALUATED` | 只读兼容候选；全文 intent coverage 为 REVIEW，禁止正式完成声明 |
...
8. 生成一个或两个完整 `masked_text` 后计算每个目标段 hash；transaction 两个 fragment 都非空；
   对 transaction v2 再按派生结构基线逐 fragment 填写 `local_rewrite_intent`，纯移动用 NO_CHANGE，
   另有措辞编辑用 REWRITE；
