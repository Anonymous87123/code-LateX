...
resolution 绑定到当前 source/policy。authoring v2 默认同时提出 clause 与完整句候选：clause 包含 core、
左分隔符附着和右分隔符附着三种边界，完整句用于避免调用方为句级改写重复手工登记 span；段级边界仍须
显式请求。可用 `NONE/CLAUSE/CLAUSE_AND_SENTENCE/SENTENCE/PARAGRAPH/SENTENCE_AND_PARAGRAPH/ALL` 调整。
