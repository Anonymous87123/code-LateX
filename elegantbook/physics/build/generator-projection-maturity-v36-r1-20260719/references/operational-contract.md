...
短文可执行 PATCH 固定按 [short-patch-workflow.md](short-patch-workflow.md) 生成 source-byte-bound bundle 和待审派生目录。bundle/application PASS 只证明 span、hash、动作和 candidate 派生一致；发布后 verifier 必须执行闭集核对与 current-policy replay，显式 live source 漂移时保持 `REVIEW/2`。有无 `UNRESOLVED`、current-policy replay 是否 PASS 都不能跳过外部 paired-quality 门。
