...
还要归档 scope/report。完全相同的目录重跑是幂等，不同 run 或字节冲突不得覆盖。安装版独立 replay
只有在记录完整、当前 policy 未漂移且归档输入重算核心结果一致时才返回 `PASS/0`；proposal 无原 reviewer
ID、policy drift 或可选 live-source 检查失败为 `REVIEW/2`，内部篡改/缺件/多件/重算不一致为 `FAIL/1`。
replay 只证明 `SELF_CONSISTENCY_ONLY`，不能清除本节后续的成对质量门。
...
voice=<...>。机械验证=<PASS/FAIL/REVIEW/NOT_RUN>；paired-quality=<PENDING_EXTERNAL_REVIEW/NOT_RUN>。
未运行学术质控。
```
