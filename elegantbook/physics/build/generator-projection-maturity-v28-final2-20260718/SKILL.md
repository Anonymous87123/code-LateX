...
或 warning proposal 因原 reviewer ID 按隐私规则未归档而无法原样复跑时，固定
`REEXECUTION_NOT_SUPPORTED`、`REVIEW/2` 和 `NOT_RUN`；
只有核心状态、退出码、findings 与 request SHA 重算一致时才写 replay `PASS/0`。原始 reviewer ID 永不进入
证据目录，只保存假名化 SHA；这不是匿名身份认证。replay `PASS` 的范围严格是
