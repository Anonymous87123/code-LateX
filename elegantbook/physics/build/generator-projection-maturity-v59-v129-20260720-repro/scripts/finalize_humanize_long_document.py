...
def _unresolved_reason_summary(units: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for unit in units:
        ...
        match = re.search(
            ...
            + "|".join(re.escape(code) for code in fixed_codes)
            + r")(?:\s|;|$)",
            notes,
        )
        if match:
    ...
    return {
        "total": total,
        "classified": sum(codes.values()),
        "unclassified": total - sum(codes.values()),
    ...
    *,
    ...
    with _run_lock(run_dir):
        with tempfile.TemporaryDirectory(
            ...
            _write_json(run_dir / "finalization_metadata.json", metadata)
            _write_json(run_dir / "latest_attempt_metadata.json", metadata)
            return metadata
