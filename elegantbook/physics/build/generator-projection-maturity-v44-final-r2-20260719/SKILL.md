...
需要确认工作文件仍对应记录时追加 `--live-source <path>`；漂移、缺失或不可读固定为 `REVIEW/2`，不能伪装成记录损坏。不得手写一个无 hash/offset 的列表后声称已验证；applicator 必须读取冻结 source 快照运行统一验证器。同 policy 下核心结果、finding、coverage inventory 或 request SHA 重算不一致为 `FAIL/1`，policy drift 为 `REVIEW/2`。合法候选仍固定 `DELIVERY REVIEW exit=2`；verifier `PASS/0` 也只证明闭集工件在当前 policy 下可重放且本地自洽，机械结构或 coverage PASS 不等于文风质量 PASS。

已有 short-PATCH review 只需修改少数既有 hunk 时，使用 `amend_humanize_short_patch.py create/apply`，不要重建 span、selection 和未变 hunk。amend 只开放指定 hunk 的 `decision/replacement/reason`；scene、强度、保护词、source span、hunk 拓扑、coverage declarations、selection 和 conflict 全部冻结。父 review 必须通过当前 verifier 与 coverage replay；policy/live-source 漂移返回 `REVIEW/2` 且不发布。新 v3 bundle 嵌入最多 8 层父血缘并逐层复核，仍只产生待审候选。
...
| 短文 PATCH | `short-patch-workflow.md` + scaffold/build/apply/verify 四个 short-patch 脚本；v2 coverage 由 `humanize_short_patch_coverage.py` 重算；verify 必须重算 `review.md`、检查闭集并执行 `CURRENT_POLICY_REPLAY`，需要新鲜度证明时追加 `--live-source`；只发布待审派生目录，verify PASS 仍为 SELF_CONSISTENCY_ONLY |
