...
显式选入后才成为 bound declaration，不进入 high inventory 或 high 处置率。
...
FOCUS 只把调用方已经诊断的精确片段注册为 suggestion；它不扫描全篇生成候选洪水。与数学、引语、代码、
TeX 命令调用或其他保护跨度重叠时只生成 `SUPPRESSED`，不发放可引用 `span_id`。focus spec、authoring
