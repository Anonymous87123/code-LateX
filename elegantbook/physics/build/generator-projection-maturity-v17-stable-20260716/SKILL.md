...
按 [source-provenance-trust.json](references/source-provenance-trust.json) 裁决来源角色。当前没有代理不可伪造的外部来源证明链，故本地台账中的 `HUMAN_CONFIRMED` 只是“有人作出过人写声明”，与 `UNKNOWN` 一样最高为 `PROVISIONAL`，不得成为 production positive；前者理由码为 `EXTERNAL_ATTESTATION_REQUIRED`，后者为 `ORIGIN_UNKNOWN`，都只允许 `AUDIT` 与 `EXPERIMENTAL_POSITIVE_REVIEW`。`MODEL_GENERATED` 和 `MODEL_ORIGIN_UNRESOLVED` 固定为 `NEGATIVE_ONLY`，只允许 `AUDIT` 与带 detector 的 `NEGATIVE_GUARD`；`OCR_INHERITED`、`THIRD_PARTY` 只允许审计。任何本地 `origin_attestation` 字段、调用方标签、路径相等或重算 SHA-256 都不能升级该矩阵。

普通生成、改写和长文处理一律不依赖正向来源卡；generator projection 删除全部 positive action、候选队列和来源候选审计路由，只保留去除了外部路径及来源台账元数据的 negative detector registry。该边界不证明模型心理上从未受其他材料影响，只限制本工具可声称、可选取和可入队的证据角色。
