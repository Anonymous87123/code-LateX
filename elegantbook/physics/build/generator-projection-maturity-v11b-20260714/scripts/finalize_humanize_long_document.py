...
def _rebuild_prepare_state(
    ...
    ledger_fields = (
        ...
        "scene", "mode", "intensity", "owner_chunk", "context_before_unit", "context_after_unit",
...
def _finalize_locked(
    ...
    *,
    ...
    initial_processable_editable_units = sum(
        1 for chunk in chunks.values() if str(chunk.get("status")) == "PENDING"
    )
    ...
    # The current long-document executor does not yet bind a hash-stable Voice
    # Profile, run a document-level conformance/churn gate, or launch a fresh
    # second Humanize pass. Keep those claims explicit and false rather than
    # promoting local unit validation into a whole-document Humanize claim.
