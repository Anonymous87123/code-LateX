...
使用 `Dominant/Recurring/Local`；默认只列 3-8 个最高价值问题。一个位置只有一个主病灶。
...
严格使用 [operational-contract.md](operational-contract.md) 第 5.1 节的唯一 `DIAGNOSE` schema。用户指定其他展示格式时，语义字段仍须齐全。
...
`LIGHT` 不改段序；`BALANCED` 不跨节重排；`STRUCTURAL` 有明确授权并交付 patch。用户结构锁优先。
...
- 关键取舍连接到模型操作、情景比较或方案后果。
- 结果段回答“变化对选择意味着什么”，但不补造建议。
...
路由仍不清楚时返回 `UNRESOLVED`，不得用一个通用输出冒充场景差异。
...
任何一项未执行都标 `NOT_RUN`。抽样诊断不能通过全文完成门；`rendered_partial` 不能作为全文交付。任何 `UNRESOLVED`、`SKIPPED_GARBLED` 或 `CHANGED_AFTER_SNAPSHOT` 都使 `full_completion_claim_allowed=false`。
