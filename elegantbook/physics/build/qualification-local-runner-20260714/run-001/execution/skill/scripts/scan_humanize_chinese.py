...
def _raw_protected_spans(text: str) -> list[tuple[int, int, str]]:
    patterns = (
        (
...
class ProtectedIndex:
    ...
    def __init__(self, text: str) -> None:
        self.spans = _merge_spans(_raw_protected_spans(text))
...
def scan_text(
    ...
    *,
    ...
    """Return location-rich candidates; protected and excluded hits are opt-in."""
    scene = scene.upper()
    if scene not in SCENE_CHOICES:
        raise ValueError(f"unsupported scene: {scene}")
    lexicon = lexicon or load_lexicon()
    protected_index = ProtectedIndex(text)
    ...
    for signal in lexicon["signals"]:
        ...
        for start, end, matched in matches:
            ...
            protection = protected_index.reason_at(start)
            if protection:
