...
def check_documents(
    ...
    *,
    ...
    result = CheckResult(evidence={"protected_terms": _term_evidence(terms)})
    if terms:
        _compare_sequence(
            result,
    ...
    for side, events in (("before", before_events), ("after", after_events)):
        problems = _validate_environment_nesting(events)
        if problems:
            result.errors.append(
                Diagnostic(
    ...
    for side, text_value, ranges in (
        ("before", before_text, before_code_ranges),
        ("after", after_text, after_code_ranges),
    ):
        problems = _brace_problems(text_value, ranges)
        if problems:
            result.errors.append(
                Diagnostic(
    ...
    before_markers = _marker_counts(_mask_ranges(before_text, before_excluded))
    after_markers = _marker_counts(_mask_ranges(after_text, after_excluded))
