...
def apply_patch(source_path: Path, bundle_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        if output_dir.exists() or output_dir.is_symlink():
            raise ShortPatchError(f"output_exists:{output_dir}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Strictly validate and apply a humanize-short-patch/v1 bundle to a new "
    ...
    parser.add_argument("--bundle", required=True, type=Path, help="humanize-short-patch/v1 bundle")
