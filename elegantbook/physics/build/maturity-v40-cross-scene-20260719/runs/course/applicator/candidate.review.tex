...
在自然坐标系中，速度 $\vec{v}$ 的方向永远沿着轨道切线方向。速度矢量可以写为大小（速率 $v$）与方向（切向单位矢量 $\vec{\tau}$）的乘积：$\vec{v} = v\vec{\tau}$，其中 $\vec{\tau}$ 是关于时间 $t$ 的函数（由于质点曲线运动时，切向单位矢量 $\vec{\tau}$ 的方向一直在改变）。由乘积求导法则可得： \(\vec{a} = \frac{\mathrm{d}\vec{v}}{\mathrm{d}t} = \frac{\mathrm{d}(v\vec{\tau})}{\mathrm{d}t} = \frac{\mathrm{d}v}{\mathrm{d}t}\vec{\tau} + v\frac{\mathrm{d}\vec{\tau}}{\mathrm{d}t}\) 
加速度由两部分组成：
\begin{itemize}
\item 第一项 $\frac{\mathrm{d}v}{\mathrm{d}t}\vec{\tau}$：速度大小随时间的变化率，方向沿切向。
\item 第二项 $v\frac{\mathrm{d}\vec{\tau}}{\mathrm{d}t}$：速度方向随时间的变化率；还需确定 $\frac{\mathrm{d}\vec{\tau}}{\mathrm{d}t}$。
\end{itemize}
考察 $\mathrm{d}\vec{\tau}$ 与路程微元 $\mathrm{d}s$ 的关系，链式展开后可分离出速率 $v$：
...
综合上面的方向（$\vec{n}$）和大小（$\frac{\mathrm{d}s}{\rho}$），$\mathrm{d}\vec{\tau}$ 的严格矢量表达式： \(\mathrm{d}\vec{\tau} = \frac{\mathrm{d}s}{\rho}\vec{n} \implies \frac{\mathrm{d}\vec{\tau}}{\mathrm{d}s} = \frac{1}{\rho}\vec{n}\) 
