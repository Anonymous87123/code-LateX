...
`LIGHT` 不改段序；`BALANCED` 不跨节重排；`STRUCTURAL` 有明确授权并交付 patch。长文自动面还必须提交来源段结构 plan，固定同一 unit、标题锁和保护项随段归属；plan PASS 不替代结构语义复核。实际结构变化必须生成 hash-bound review request，顶层保持 `REVIEW/2`，候选只进 `rendered_review/`。用户结构锁优先。
