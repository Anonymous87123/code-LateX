...
混合文档允许不同 unit 使用不同场景。共享摘要或总引言用途不明时使用 `GENERAL`，不要拼接三种声线。document prior 按 include 根覆盖逻辑文档，而不按物理文件割裂；它只补全与本 unit 弱信号一致的场景，不得把完全中性的“背景/结论”静默改成邻近强场景，也不得替强歧义作隐藏裁决。
...
逐段来源 ID、职责、保护 ID、移动资格和 inventory hash；`REWRITE` 必须提交
`structural_plan`，且每个来源段恰好映射一次。当前自动面不跨 unit、不解锁标题、不拆分或
删除来源段。普通内联数学可随完整来源段移动；陈列数学、正式环境、引语和关键命令所在段锁定。
...
`humanize-structural-semantic-review-request/v1`。请求绑定 before/baseline/after、plan、上下文、
warning 和 policy hash；证据路径使用提交后的 `validation/...` 相对引用。当前不消费本地或模型
自填 clearance。完整候选的机械组装可以是 `candidate_assembly_status=PASS`，但顶层必须为
`delivery_gate_status=REVIEW/2`，候选只进入 `rendered_review/`，不得据此声明全文完成。
...
收尾器从不覆盖原源文件。所有接受的单元先写入 `.rendered_staging`；全文不变量和可选编译门通过后，普通终态发布到 `rendered/`，覆盖未闭合时发布到 `rendered_partial/`。STRUCTURAL 实际变化若只缺外部语义复核，完整候选发布到 `rendered_review/`，顶层保持 `REVIEW/2`。编译失败时 staging 改名为 `failed_staging/`，不创建新的正式输出。`rollback_manifest.json` 明确记录源文件未改、快照依据和丢弃失败 staging 的动作。
...
局部 validator PASS 只进入临时候选，不立即构成最终 `DONE`。finalize 使用冻结 chunk 的
...
lexicon、registry 原始字节、canonical registry 和 detector 定义 hash。registry 缺失、非法
...
- `status/delivery_gate_status=PASS`：候选组装完成，且不存在实际结构变化后的待审语义；
- `status/delivery_gate_status=REVIEW`：仍有 PENDING/UNRESOLVED，或完整 STRUCTURAL 候选仍待外部语义复核；
...
- `publish_state=REVIEW_CANDIDATE` 与 `rendered_review/`：机械完整但语义待审，不能称为正式输出；
