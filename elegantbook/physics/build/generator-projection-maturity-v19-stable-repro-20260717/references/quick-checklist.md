...
扫描器和 validator 都不能证明改后比原文自然。交付前逐段做 A/B 复核：先说出原段落在做什么，再核对改后是否保留该职责和句间层级；最后检查主谓、动宾、修饰和对举两端。只有改后存在具体收益且没有新缺陷时保留修改。若收益只是“更短、更多短句、少了连接词”，或出现信息损失和新模板，继续改；仍不能稳定优于原文就恢复原段并标 `NO_CHANGE`。
