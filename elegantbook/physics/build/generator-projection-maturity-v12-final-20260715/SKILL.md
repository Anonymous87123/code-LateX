...
只有 validator 返回 `PASS/0` 的 Profile 才能进入长文 prepare。低于 300 个去保护、去重后的作者汉字时，builder 必须产出显式 `DEFAULT`；5000 字但不足 3 个完整文本单元时最高仍为 `MEDIUM`。代码、公式、引语、题干、OCR、模板、未知归属和模型生成文本不得进入作者声线证据。乱码样本记录后跳过，不猜测恢复。
...
PERSONAL Profile 必须连同 manifest、sample spec 与 allowed root 提交，由 prepare 当场重建样本证据；只有自哈希而没有可重建证据的 `PERSONAL/PASS` 必须拒绝。builder 因不足 300 字或无合格来源而生成的“证据绑定 DEFAULT”也必须携带同一组证据并写 `REBUILT_DEFAULT_PASS`；只有代码注册表产生的零样本场景 DEFAULT 才可不带 manifest。未提供样本时，prepare 物化对应场景的版本化 `SCENE_DEFAULT`，并要求披露“不声称复现个人文风”。每个 `REWRITE` 或 `NO_CHANGE` JSON bundle 必须回显 chunk 中的 `voice_profile_sha256`；缺失、格式非法或错配分别进入 `voice_profile_hash_missing`、`voice_profile_hash_invalid`、`voice_profile_hash_mismatch`，该 unit 保持 `UNRESOLVED`。hash 一致只证明版本绑定，不证明正文符合声线。
