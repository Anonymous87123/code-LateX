...
`REWRITE` 与 `NO_CHANGE` 都必须从当前 chunk 精确回显 `voice_profile_sha256`。缺失、格式非法或与当前 unit 错配时，在正文 validator 之前分别以 `voice_profile_hash_missing/invalid/mismatch` 拒绝，不得由 finalize 自动补值。旧式 `.txt` bundle 无法承载该字段，因此 Profile-bound run 只接受 JSON。
