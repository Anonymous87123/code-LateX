...
语料动作锚来自 `corpus-action-sources.json` 中已通过来源可读性和行级锚点检查的 MD/TeX 动作卡，但“人工阅读过”不等于“确认由人创作”。只有当前 profile 把场景标为 `SUPPORTED` 时，普通 `REWRITE/DRAFT` 才从当前场景选择至多两张满足输入条件的正向卡；没有适用卡时明确 `NONE_APPLICABLE`。`SUPPORTED_PROVISIONAL` 表示只有 `origin_class=UNKNOWN` 的组织参考：生产改写默认填写 `corpus_action_support=NONE`；实验性候选即使选择卡片也固定 `REVIEW/2`，不得进入 accepted 队列。完整安装版审计 profile 中 `COURSE/MODELING` 为 `SUPPORTED_PROVISIONAL`，`RESEARCH/GENERAL/REPORT` 为 `CORPUS_INSUFFICIENT`；generator projection 会进一步删除 provisional 卡及其来源路径，因此生成能力面内所有场景均按无生产卡处理。当前所有场景都没有 `HUMAN_CONFIRMED` 的生产级正向卡。卡片只影响信息组织，不授权复制来源句子或迁移来源事实。`MODEL_GENERATED`、`MODEL_ORIGIN_UNRESOLVED`、`OCR_INHERITED` 和 `THIRD_PARTY` 均不得成为 `positive_action_reference`；其中模型来源只有带 detector 的负例或排除审计角色。`HUMAN_CONFIRMED` 只表示来源台账中的人写确认，仍不证明作者身份、PERSONAL Voice、事实正确或复制许可。
...
场景只有在足量独立 `HUMAN_CONFIRMED` 正向来源通过行级审计时才返回 `SUPPORTED`。只有
`UNKNOWN` 来源达到数量门槛时返回 `SUPPORTED_PROVISIONAL`：其动作卡保留为可追溯的组织
参考，但选卡候选固定 `REVIEW/2`；改用 `corpus_action_support=NONE` 可继续执行普通文风门。
完整安装版审计 profile 中 `COURSE/MODELING` 为 `SUPPORTED_PROVISIONAL`，
`RESEARCH/GENERAL/REPORT` 为 `CORPUS_INSUFFICIENT`，没有任何生产级 action-card 场景。
generator projection 删除 provisional 卡和对应来源路径，只保留 production 正向卡与负例 guard；
当前 projection 因而没有正向卡。生产任务按原稿与 Voice Profile 改写，不得伪造动作卡、声线
证据或“已由人类语料支撑”的结论。工程治理报告、API 文档、
...
```json
{
  ...
  "before_path": "…",
  "after_path": "…",
  "action_cards": ["COURSE-DECISION-01"],
  "action_evidence": {"COURSE-DECISION-01": ["decision-anchor"]},
  "anchors": [{"id": "decision-anchor", "before_text": "…", "after_text": "…", "role": "decision_chain"}],
  "keep_reasons": {},
  "warning_resolutions": {
...
`corpus_action_support=NONE` 只允许当前 profile 标为 `SUPPORTED_PROVISIONAL` 或 `CORPUS_INSUFFICIENT` 的场景，且必须
不带 `action_cards` 或 `action_evidence`；它不是绕过保护检查的开关。
...
2. 重新构建来源动作 profile，确认动作卡仍为 `AVAILABLE`；源文件不可读或行范围
   ...
   `SUPPORTED`/`SUPPORTED_PROVISIONAL`/`CORPUS_INSUFFICIENT` 状态。选择任何
   `origin_assurance=PROVISIONAL` 的卡固定增加 `provisional_action_card:<id>` 与
   `scene_corpus_origin_unresolved:<scene>`，候选保持 `REVIEW/2`；使用 `NONE` 不继承该 review。
   非当前安装默认路径的 `--catalog` 固定为 `EXTERNAL_UNVERIFIED`；即使其中自填
   `HUMAN_CONFIRMED`，选择卡片仍增加 `external_catalog_not_production_trusted` 并保持 REVIEW。
...
收尾器先校验 prepare 生成的 `prepare_integrity.json`，再逐单元调用统一验证器，只向快照派生目录发布结果，不覆盖源文件。逐单元证据必须标 `document_scope=FRAGMENT`：只容忍改前、改后完全相同的跨块 TeX 边界不平衡，环境/花括号问题一旦变化仍为硬失败；接受的 unit 组装后还要以完整文档范围重验，fragment 通过不能替代全文结构门。完整性清单缺失或任何 prepare 产物被改动时直接 `FAIL`；它只是审计辅助，收尾器还必须从冻结 `source/`、include 关系、预算、保护跨度、当前安装的 routing policy、原始范围和 canonical chunks 独立重建初始状态，并复算 prepare 顶层状态、计数、路由和逐 unit Voice 绑定，拒绝同时伪造 metadata、`units.jsonl`、ledger、chunk 与封条。AUTO 使用四场景 DEFAULT Profile set；set、entry 和 metadata binding 均为精确字段合同，任何 `identity_verified` 或外部 clearance 夹带都必须失败。相同 `run-dir` 的 finalize 通过跨进程锁串行执行；空文件集、零可处理单元或只有保护内容只能是 `REVIEW`。可选 `--check-command` 在一次性检查副本中运行，真实正文 staging、待发布 validation/diff 证据和其他 run 产物的任何新增、删除或修改都会 `FAIL`。局部 PASS 先进入临时接受集：全文 Voice 门只检查 Profile 注册机械特征是否显著回退、负控是否放大；跨 unit 门用 `LEX-REPAIR-01` 和当前可用的真实 MD/TeX `negative_guard` 比较每个 unit 的 masked before/after。新增模板只回退新增 occurrence 所在 unit，原文继承重复不拦截；positive action card 不作 detector。DEFAULT Voice PASS 固定表示场景默认策略闭合，个人声线为 `NOT_APPLICABLE`；PERSONAL PASS 也不证明作者身份或完整气质。`coverage_completion_claim_allowed=true` 只证明覆盖与局部保护闭合；只有 `humanize_completion_claim_allowed=true`（兼容字段 `full_completion_claim_allowed`）时才能声称全文 Humanize 完成。`rendered_partial`、`PENDING`、`UNRESOLVED`、`SKIPPED_GARBLED`、`CHANGED_AFTER_SNAPSHOT`、验证 `REVIEW/FAIL` 或编译未通过都不得被总体声明覆盖。
...
只有 validator 返回生产准入 `PASS/0` 的 Profile 才能进入长文 prepare；PERSONAL 与证据绑定 DEFAULT 必须使用 `--rebuild-evidence`，只验 Profile 自哈希固定为 `REVIEW/2`，确定性零样本场景 DEFAULT 除外。低于 300 个去保护、去重后的作者汉字时，builder 必须产出显式 `DEFAULT`；5000 字但不足 3 个去重完整文本单元时最高仍为 `MEDIUM`。近重复使用顺序无关的连通分量聚类，不能因 sample spec 排序改变置信等级。PERSONAL/PASS 的有效样本场景必须与 `binding_scene` 一致；跨场景重标只能是 `REVIEW`。代码、公式、引语、题干、OCR、模板、未知归属和模型生成文本不得进入作者声线证据。乱码样本记录后跳过，不猜测恢复。
...
PERSONAL Profile 必须连同 manifest、sample spec 与 allowed root 提交，由 prepare 当场重建样本证据；只有自哈希而没有可重建证据的 `PERSONAL/PASS` 必须拒绝。builder 因不足 300 字或无合格来源而生成的“证据绑定 DEFAULT”也必须携带同一组证据并写 `REBUILT_DEFAULT_PASS`；只有代码注册表产生的零样本场景 DEFAULT 才可不带 manifest。manifest 必须绑定 canonical sample-spec hash，finalize 会独立核对冻结 spec；不能只修改来源角色并重算 prepare 封条继续 PASS。上述本地哈希以 allowed root 和当前样本字节为可信输入，只防错配、局部篡改和旧工件重放；若同一主体能同时替换源样本、spec、manifest、Profile 与全部自哈希，本地工具不能证明真人作者身份或历史来源，`claims.identity_verified` 必须保持 false，除非未来接入代理无法访问私钥的外部签名信任根。未提供样本时，prepare 物化对应场景的版本化 `SCENE_DEFAULT`，并要求披露“不声称复现个人文风”。每个 `REWRITE` 或 `NO_CHANGE` JSON bundle 必须使用无重复键的 strict JSON，并同时回显 chunk 中的 `unit_id`、`chunk_binding_sha256` 与 `voice_profile_sha256`；文件改名、错 unit、旧 chunk 或 Voice 错配都在正文验证前拒绝。Voice hash 缺失、格式非法或错配分别进入 `voice_profile_hash_missing`、`voice_profile_hash_invalid`、`voice_profile_hash_mismatch`；unit/chunk 绑定使用独立的 `bundle_unit_id_*` 与 `chunk_binding_hash_*` 理由。重复键直接拒绝，该 unit 不得成为 `DONE`。三个 hash/ID 一致只证明工件与目标块身份绑定，不证明正文符合声线。
