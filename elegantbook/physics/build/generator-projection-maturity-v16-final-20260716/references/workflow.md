...
`SUPPORTED_PROVISIONAL` 和 `CORPUS_INSUFFICIENT` 的生产改写都填写 `corpus_action_support=NONE`，不选择卡片。前者的 `UNKNOWN` 卡只可用于独立候选审计；一旦选择，候选门固定 `REVIEW/2`，不得把它当成正常改写完成。generator projection 会删除 provisional 卡、动作描述及其来源路径，避免正常生成上下文先看到卡片再自报 `NONE`。非安装默认路径的外部 catalog 固定 `EXTERNAL_UNVERIFIED`，其中自填的 `HUMAN_CONFIRMED` 卡也不能进入 accepted。当前所有场景均没有生产级 action-card 支持。
