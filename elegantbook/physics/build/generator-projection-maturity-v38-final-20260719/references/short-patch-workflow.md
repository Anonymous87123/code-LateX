...
resolution 固定下来。编辑者只在 `spans` 中登记一次 source span；`hunks` 与 `selected_spans` 通过
`span_id` 独立引用。填完每个 high 的 `HUNK/KEEP` disposition、hunk、selection 和显式 conflict 后运行：
...
再确定性展开为现有 `humanize-short-patch-selection/v2`。scaffold/finalize 成功都不是 bundle 或交付 PASS。
...
### 4.2 authoring scaffold v1

`humanize-short-patch-selection-authoring/v1` 不是 builder 输入，也不是 evidence。不可编辑的
`source/configuration/policy_hashes/high_findings/inventory_sha256` 绑定当前 source 与 AUTO high universe；
可编辑面只有 span registry、hunk、lexical resolution、selected span 和 explicit conflict。自动生成的
high span 必须保留其 `finding_ids` 精确绑定；需要句级或更大 hunk 时新增一个 `finding_ids=[]` 的 span，
不要改宽原 high span。finalize 输出显式 byte offset 的 v2 selection，再由正式 builder 重验一次完整
coverage。任何本地 authoring 文件都不能证明 selection 来自外部用户请求。
...
authoring scaffold 只降低重复录入和引用错配风险；它不能自动选择 hunk 边界、replacement、KEEP 理由或
conflict pair，也不能把模型填写的 resolution 变成用户授权。`review.md` 只改善可见性，不是人工复核、
外部签名或 paired-quality clearance。
