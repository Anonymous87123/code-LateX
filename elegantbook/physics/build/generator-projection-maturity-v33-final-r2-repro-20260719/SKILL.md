...
`CLEAN` 不能原样带回未决 high span；摘要声明未决不能补救正文中的 high 残留。若 high 句壳承载了用户锁定的主张，而输入又不足以把它改成具体对象、动作或后果，停止生成完整 CLEAN 候选，改用 `UNRESOLVED + 最小 PATCH/ANNOTATED`：只提交其余可安全改动，把冲突 span 原样列为未决并说明需要用户放宽主张锁或补充信息。摘要明确写 `requested_output=CLEAN; effective_output=PATCH`（逐处注释时改为 `ANNOTATED`）；不得把降级后的 PATCH/ANNOTATED 标成 CLEAN，也不得交付“全文候选 + 末句仍 high + 摘要称待审”的假降级。
...
每个 operation 同时引用 source span 与 target signal，span 的 1-based 行范围和 SHA-256 必须命中冻结 masked chunk，
且声明 span 与实际 masked diff 双向覆盖。`NO_CHANGE` 必须给具体 reason 和至少一个同样绑定的
