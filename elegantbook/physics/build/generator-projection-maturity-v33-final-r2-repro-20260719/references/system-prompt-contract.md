...
CLEAN 不能原样带回未决 high span；摘要声明未决不能补救正文中的 high 残留。若 high 句壳承载用户锁定主张且输入不足以具体化，改用 `UNRESOLVED + 最小 PATCH/ANNOTATED`，只交付安全 hunk 和原样未决 span，不生成含 high 残留的完整 CLEAN 候选。声明 `requested_output=CLEAN; effective_output=PATCH`（逐处注释则为 ANNOTATED），不得把降级后的 PATCH/ANNOTATED 标成 CLEAN。
```
