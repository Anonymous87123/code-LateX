...
11. 对每次 `REWRITE/NO_CHANGE` 生成并核对 paired-quality request；没有可信外部 clearance 时保持
    `PENDING_EXTERNAL_REVIEW/REVIEW/2`；
12. 报告 `UNRESOLVED` 和默认声明。
...
- 当前本地工具没有外部信任根，不能创建 `VERIFIED_HUMAN` clearance。真正的 `VERIFIED_HUMAN` 必须由代理不可访问私钥的外部审批服务签发，并在可信边界验证签名、request/artifact 绑定、审批范围和时效；本地调用方标签、文本理由或 agent 不可伪装成该状态。未接入这种服务时，只能通过修改候选稿使 warning 在新 artifact 上消失，从而恢复 speech-act 机械层 `PASS`；`REWRITE` 的顶层交付仍由 paired-quality 门保持 `REVIEW/2`。
...
- 短文需要保留机器证据时使用 `validate_humanize_output.py --evidence-dir <DIR>`。DIR 必须是不存在的
  发布目标；验证器原子写入 `validation-result.json`、完整 paired-quality request（若有）和
  `evidence-manifest.json`，并重新读取改前/改后文件确认 SHA 未漂移。manifest 的自哈希只绑定该目录内
  的字节，不构成外部质量 clearance；目标已存在、来源漂移、reparse point 或 commit 失败均为结构化
  `FAIL/1`，旧目录保持原样。
- 长文以 `finalization_metadata.json` 为完成证据。`rendered_partial` 只是未闭合派生草稿；
  ...
  Voice 绑定、全文声线/跨块重复门、fresh second-pass convergence、paired-quality clearance 与编译/格式门均通过
