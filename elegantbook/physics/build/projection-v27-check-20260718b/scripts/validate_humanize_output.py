...
DIRECT_VALIDATION_EVIDENCE_SCHEMA = "humanize-direct-validation-evidence/v1"
...
def _evidence_bundle_record(payload: dict[str, Any]) -> dict[str, Any]:
...
def _persist_evidence_bundle(
    ...
    *,
    ...
    before_resolved = before.resolve(strict=True)
    after_resolved = after.resolve(strict=True)
    ...
    if output_dir.exists() or output_dir.is_symlink():
        raise ValueError("evidence directory already exists; refusing to overwrite")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        help=(
            "原子保存 validation-result.json、paired-quality request 和 evidence-manifest.json；"
            "目录已存在时拒绝覆盖"
        ),
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.evidence_dir is not None:
        payload["evidence_bundle"] = _evidence_bundle_record(payload)
        try:
    ...
    elif payload.get("schema") == DIRECT_VALIDATION_EVIDENCE_ERROR_SCHEMA:
        sys.stdout.write(
            "status: FAIL\n"
            f"delivery_gate_status: {payload['delivery_gate_status']}\n"
