...
`voice_profile_sha256` 和 masked text；`scaffold_metadata.json`（`humanize-rewrite-scaffold/v2`）明确
...
sidecar 的 `requires_manual_completion=true` 与 `template_sha256` 都描述骨架生成时刻：前者不是动态进度，
后者绑定原始模板字节，人工填写后自然不再等于当前 bundle。finalizer 只把 sidecar 作为严格审计旁证，
