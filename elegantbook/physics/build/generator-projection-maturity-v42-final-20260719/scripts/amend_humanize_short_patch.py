...
def _load_verified_parent(
    ...
    *,
    ...
    try:
        verification = verifier.verify_directory(
            base_review_dir,
...
def _assert_live_source(path: Path | None, expected_sha256: str, *, required: bool) -> None:
    if path is None:
        if required:
    ...
    if status.get("status") != "MATCH":
        reason = (
            ...
            if status.get("status") == "NOT_CURRENT"
            else "LIVE_SOURCE_UNAVAILABLE"
        )
        raise AmendReview(reason, status)
    ...
    try:
        ...
        if args.operation == "create":
            ...
            _print(
                ...
                text=f"PENDING AMENDMENT exit=0 hunks={summary['hunks_total']}",
            )
            return 0
