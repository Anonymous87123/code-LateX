...
读取当前场景文件中的“语料动作锚”。每个可编辑单元最多选择两张 `positive_action` 卡，且卡片列出的对象、条件、证据角色或比较关系必须已经出现在输入中。选择只控制信息组织，不授权复制来源句子、事实、数字、变量或领域对象。
...
4. 对照所选语料动作锚，确认所需对象均来自当前输入；不满足时撤销该卡。
5. 仅在需要范例时读取当前场景的改写范式。
...
3. `COURSE/MODELING/RESEARCH` 可从当前场景文件选择至多两张满足输入锚点的正向
   动作卡；无适用卡写 `NONE_APPLICABLE`。`GENERAL` 固定
   `corpus_action_support=NONE`，不列卡或 source evidence。
