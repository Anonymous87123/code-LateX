...
def _write_json_group_atomic(entries: Sequence[tuple[Path, Any]]) -> None:
    """Publish a small authority-file group or restore every prior byte."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        failed_attempt_path = run_dir / "latest_attempt_metadata.json"
        metadata: dict[str, Any] | None = getattr(
            error, "_humanize_failure_metadata", None
        )
        if metadata is None and failed_attempt_path.is_file():
            try:
                ...
                if (
                    ...
                    and candidate.get("error_code") == _runtime_error_code(error)
                    and candidate.get("attempt_metadata_role")
