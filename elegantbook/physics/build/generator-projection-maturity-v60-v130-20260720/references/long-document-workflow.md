...
v3 理由失败使用固定、去敏的 `NO_CHANGE_reason_invalid:<issue>`：`too_short_han` 表示不足 8 个汉字，
...
两组 authority metadata 必须成组原子提交；第二个文件失败时，第一个文件也恢复原字节。恢复 canonical 后若
失败 metadata 因权限或写入异常无法保存，调用方只能使用本次异常对象携带的去敏内存记录，stdout 标记
