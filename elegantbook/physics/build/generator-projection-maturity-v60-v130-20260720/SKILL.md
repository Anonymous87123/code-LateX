...
`finalization_metadata.json + latest_attempt_metadata.json` 和
`last_failed_attempt_metadata.json + latest_attempt_metadata.json` 都必须作为权威文件组发布：任一成员写入失败，
全部成员恢复原字节。若失败记录本身无法持久化，stdout 必须声明
...
validator 正文或异常正文猜测 code。final ledger 以 canonical JSON 写入 `unresolved_codes_json`。
