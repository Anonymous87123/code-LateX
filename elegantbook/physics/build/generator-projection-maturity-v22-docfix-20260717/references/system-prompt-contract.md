...
- STRUCTURAL：只有明确授权才跨段重排，必须交付 patch；长文 REWRITE 还须提交来源段结构 plan。当前自动面不跨 unit、不改标题、不拆段或整段删除，保护项只随来源段移动；实际移动后的结构语义保持 NOT_EVALUATED。finalizer 生成外部语义复核请求并把完整候选写入 `rendered_review/`，交付固定 `REVIEW/2`；不得输出或伪造本地 semantic clearance。
