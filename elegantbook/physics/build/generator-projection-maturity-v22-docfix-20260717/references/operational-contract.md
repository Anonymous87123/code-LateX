...
长文自动执行先读 `structural-rewrite-contract.md`。当前 finalizer 只证明同一 unit 内的来源段
重排/相邻同职责合并，固定 `title_lock=true` 且禁止跨 unit、拆段、整段删除。下面更宽的
章节级动作只能作为人工审阅 patch 的目标；没有对应结构证据时保持 `REVIEW`，不得把概念权限
误写成工具已经验证的能力。
...
`REVIEW/2`，只发布 `rendered_review/`。本地模型、调用方标签或自填 clearance 不得升级状态。
...
- 长文以 `finalization_metadata.json` 为完成证据。`rendered_partial` 只是未闭合派生草稿；
  `rendered_review` 是结构语义未评估的完整候选，也不是正式交付。
