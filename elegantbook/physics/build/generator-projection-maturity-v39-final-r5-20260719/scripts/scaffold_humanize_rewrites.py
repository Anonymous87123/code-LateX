...
UNIT_BUNDLE_SCHEMA = "humanize-unit-rewrite-bundle/v2"
SCAFFOLD_SCHEMA = "humanize-rewrite-scaffold/v4"
MAX_JSON_DEPTH = 64
...
def _bundle(chunk: dict[str, Any], decision: str) -> dict[str, Any]:
    common: dict[str, Any] = {
        "schema_version": UNIT_BUNDLE_SCHEMA,
    ...
    if decision == "REWRITE":
        ...
        # Both fields are deliberately invalid until the caller supplies a
        # concrete reason and at least one source-bound evidence span.
        common["reason"] = "TODO"
        common["evidence_spans"] = []
    return common
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.decision_map is not None:
            parsed = _strict_json(args.decision_map.read_bytes(), "decision_map")
