...
| 短文 PATCH | `short-patch-workflow.md` + build/apply/verify 三个 short-patch 脚本；v2 coverage 由 `humanize_short_patch_coverage.py` 重算；verify 必须检查闭集并执行 `CURRENT_POLICY_REPLAY`，需要新鲜度证明时追加 `--live-source`；只发布待审派生目录，verify PASS 仍为 SELF_CONSISTENCY_ONLY |
