...
def _possibility_sentence_count(text: str) -> int:
    return sum(
        1
        for sentence in re.split(r"[。！？!?；;\n]+", text)
        if "可能" in sentence or "或许" in sentence
    )


def _local_stacked_possibility(text: str) -> bool:
    for sentence in re.split(r"[。！？!?；;\n]+", text):
        count = sentence.count("可能") + sentence.count("或许")
        if count >= 2:
            return True
    return False


def _safe_possibility_compression(
    before_text: str,
    after_text: str,
    before_counts: Counter[str],
    after_counts: Counter[str],
) -> bool:
    """Allow only a local redundant possibility stack to collapse to one hedge."""
    possibility_markers = {"可能", "或许"}
    before_other = Counter(
        {key: value for key, value in before_counts.items() if key not in possibility_markers}
    )
    after_other = Counter(
        {key: value for key, value in after_counts.items() if key not in possibility_markers}
    )
    before_possibility = sum(before_counts.get(key, 0) for key in possibility_markers)
    after_possibility = sum(after_counts.get(key, 0) for key in possibility_markers)
    if before_other != after_other:
        return False
    if not (before_possibility > after_possibility >= 1):
        return False
    if not _local_stacked_possibility(before_text):
        return False
    if _possibility_sentence_count(before_text) != _possibility_sentence_count(after_text):
        return False
    before_degree_claims = [match.group(0) for match in DEGREE_CLAIM_PATTERN.finditer(before_text)]
    after_degree_claims = [match.group(0) for match in DEGREE_CLAIM_PATTERN.finditer(after_text)]
    return not after_degree_claims or bool(before_degree_claims)
    ...
    *,
    ...
    for category in SPEECH_ACT_MARKERS:
        if before_markers[category] == after_markers[category]:
            continue
        if category == "modality_scope" and _safe_possibility_compression(
            before_speech_text,
            after_speech_text,
            before_markers[category],
            after_markers[category],
        ):
            continue
