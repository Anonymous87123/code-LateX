...
短文从 CLEAN 降级为 PATCH 或用户直接要求可复核 PATCH 时，读取 [short-patch-workflow.md](references/short-patch-workflow.md)，先用 `build_humanize_short_patch.py` 冻结精确 UTF-8 source span，再用 `apply_humanize_short_patch.py` 生成 `candidate.review.*`，最后用 `verify_humanize_short_patch.py` 检查闭集目录。不得手写一个无 hash/offset 的列表后声称已验证；applicator 必须读取冻结 source 快照运行统一验证器。合法候选仍固定 `DELIVERY REVIEW exit=2`；verifier PASS 也只证明本地自洽，机械结构 PASS 不等于文风质量 PASS。
...
| 短文 PATCH | `short-patch-workflow.md` + build/apply/verify 三个 short-patch 脚本；只发布待审派生目录，verify PASS 仅为 SELF_CONSISTENCY_ONLY |
