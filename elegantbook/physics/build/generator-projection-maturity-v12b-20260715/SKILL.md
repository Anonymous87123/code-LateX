...
```powershell
python scripts/prepare_humanize_long_document.py <main.tex|doc.md> --output <empty-run-dir> --scene <SCENE> --voice-profile <voice-profile.json> --voice-profile-sha256 <64-hex>
```

未提供样本时，prepare 物化对应场景的版本化 `SCENE_DEFAULT`，并要求披露“不声称复现个人文风”。每个 `REWRITE` 或 `NO_CHANGE` JSON bundle 必须回显 chunk 中的 `voice_profile_sha256`；缺失、格式非法或错配分别进入 `voice_profile_hash_missing`、`voice_profile_hash_invalid`、`voice_profile_hash_mismatch`，该 unit 保持 `UNRESOLVED`。hash 一致只证明版本绑定，不证明正文符合声线。
