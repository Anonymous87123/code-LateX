...
v12 已实现 hash-bound Voice Profile 的构建、冻结、unit/chunk 贯穿和 rewrite bundle 回显；这只关闭了“版本身份是否一致”一项。全文声线符合性、跨块重复门和 fresh 第二遍生成仍未实现，因此即使局部覆盖与 Voice 绑定闭合，也只允许 `coverage_completion_claim_allowed=true`、`voice_binding_status=PASS`；`voice_conformance_status=NOT_EVALUATED`、`humanize_completion_claim_allowed=false`，兼容字段 `full_completion_claim_allowed=false`。
...
prepare 的有效绑定场景相同。`AUTO` 当前有效绑定到 `GENERAL`，因此不能借 `AUTO`
消费 COURSE、MODELING 或 RESEARCH Profile。PERSONAL 与证据绑定 DEFAULT 缺少上述
任一证据参数时必须拒绝，不得退化成只校验 Profile 自哈希。
...
| `voice_profile.json` | 已校验并按 canonical JSON 物化的 supplied 或场景 DEFAULT Profile |
...
supplied Profile 必须同时提供文件与调用方 pin 的 `profile_sha256`；两者缺一、hash 非 64 位小写十六进制、自哈希无效或实际值不一致都属于全局配置失败。未提供 Profile 时，prepare 物化该场景的版本化 DEFAULT。`AUTO` 因尚无独立长文路由器，临时绑定 GENERAL 保守默认，但 unit 仍保留 `scene=AUTO`，不能把 `scene_routing_status` 升为 `PASS`。Profile 的 ID、revision、confidence、kind、source、hash 与 disclosure 必须同时进入 metadata、unit、chunk 和 ledger。
...
`REWRITE` 与 `NO_CHANGE` 都必须从当前 chunk 精确回显 `voice_profile_sha256`。缺失、格式非法或与当前 unit 错配时，在正文 validator 之前分别以 `voice_profile_hash_missing/invalid/mismatch` 拒绝，不得由 finalize 自动补值。旧式 `.txt` bundle 无法承载该字段，因此 Profile-bound run 只接受 strict JSON；重复 key、浮点数、非有限数字和过深结构直接拒绝，不能依赖解析器“最后一个 key 生效”的覆盖行为。
...
Voice v12 还固定输出 `voice_profile_sha256`、`voice_profile_bindings_total/matched/missing/mismatched`、`voice_profile_default_units/default_scenes` 与 `voice_default_disclosure_required`。只有所有初始 `PENDING` unit 的最终 bundle 均回显精确 hash 时，`voice_binding_status=PASS`；尚未提交、缺失或错配时为 `REVIEW`。该字段只证明 Profile 工件身份闭合，不证明句长、标点、主语、衔接或言语行为符合 Profile。后者必须由独立全文门设置 `voice_conformance_status=PASS`；v12 尚未实现，因此不得据 hash 绑定声称个人拟声完成。
