...
编辑要求的位置范围也属于谓词状态。`正文里既要保留…… -> 既要保留……` 不是单纯删后台词：它删除了“结果应出现在哪里”的约束，却保留其余祈使结构。命中 `SPEECH_ACT_EDITORIAL_SCOPE_DROPPED` 时恢复或明确迁移 `正文里/正文中/文中` 的范围；完整删除编辑要求或把范围移到同句其他位置时，不按该窄门机械拦截，但仍须连续阅读。
