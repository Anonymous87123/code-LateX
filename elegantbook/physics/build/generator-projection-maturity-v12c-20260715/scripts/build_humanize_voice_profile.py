...
MANIFEST_SCHEMA_VERSION = "humanize-voice-sample-manifest/v1"
...
def _read_frozen(path: Path, max_bytes: int) -> bytes:
...
def _near_duplicate_clusters(units: list[dict[str, Any]]) -> None:
    ...
    for index, unit in enumerate(units):
        ...
        if digest in exact:
            rep = exact[digest]
            unit["representative_index"] = rep
            unit["is_representative"] = False
            unit["dedup_relation"] = "EXACT"
            continue
        matched: int | None = None
        if len(value) >= DEDUP_POLICY["near_min_chars"]:
            grams = _gram_set(value)
            shared: Counter[int] = Counter()
            for gram in grams:
                for rep_index in inverted.get(gram, ()):  # candidates only, not all pairs
                    shared[rep_index] += 1
            best: tuple[int, int] | None = None
            for rep_index, intersection in shared.items():
                rep_grams = grams_by_rep[rep_index]
                union = len(grams) + len(rep_grams) - intersection
                jaccard_ppm = intersection * 1_000_000 // max(union, 1)
                containment_ppm = intersection * 1_000_000 // max(min(len(grams), len(rep_grams)), 1)
                if jaccard_ppm >= 900_000 or containment_ppm >= 950_000:
                    score = max(jaccard_ppm, containment_ppm)
                    candidate = (score, -rep_index)
                    if best is None or candidate > best:
                        best = candidate
                        matched = rep_index
        if matched is not None:
            unit["representative_index"] = matched
            unit["is_representative"] = False
            unit["dedup_relation"] = "NEAR"
            continue
        unit["representative_index"] = index
        unit["is_representative"] = True
        unit["dedup_relation"] = "UNIQUE"
        exact[digest] = index
        representatives.append(index)
        if len(value) >= DEDUP_POLICY["near_min_chars"]:
            grams = _gram_set(value)
            grams_by_rep[index] = grams
            for gram in grams:
                inverted[gram].append(index)
    for cluster_number, rep_index in enumerate(representatives, 1):
        units[rep_index]["dedup_cluster_id"] = f"cluster-{cluster_number:06d}"
    for unit in units:
        rep = units[unit["representative_index"]]
        unit["dedup_cluster_id"] = rep["dedup_cluster_id"]
...
def _build_features(units: list[dict[str, Any]], confidence: str) -> list[dict[str, Any]]:
    rules = [
        (
            "syntax.subject_position",
            "TEXT_OR_OBJECT_SUBJECT",
            "subject-position/v1",
            re.compile(r"(?m)^\s*(?:#{1,6}\s*)?(?:本文|本研究|本节|该(?:方法|模型|系统|结果|过程|问题)|这一(?:方法|模型|结果|过程))"),
        ),
        (
            "syntax.subject_position",
            "FIRST_PERSON_SUBJECT",
            "first-person-subject/v1",
            re.compile(r"(?m)^\s*(?:我们|笔者|我)(?:认为|采用|选择|将|把|在|对)"),
        ),
        (
            "syntax.condition_opening",
            "CONDITION_BEFORE_CLAIM",
            "condition-opening/v1",
            re.compile(r"(?m)^\s*(?:如果|若|当|在[^，。]{0,24}(?:条件|情形|情况下))"),
        ),
        (
            "transition.explicit_connector",
            "EXPLICIT_LOGICAL_CONNECTOR",
            "logical-connector/v1",
            re.compile(r"(?:因此|然而|不过|同时|由此|相较之下|换言之)"),
        ),
        (
            "layout.paragraph_terminal",
            "EXPLICIT_TERMINAL_PUNCTUATION",
            "paragraph-terminal/v1",
            re.compile(r"[。！？；]\s*$"),
        ),
        (
            "layout.parenthetical_note",
            "PARENTHETICAL_QUALIFICATION",
            "parenthetical-note/v1",
            re.compile(r"（[^（）\n]{1,80}）"),
        ),
        (
            "layout.semicolon_grouping",
            "SEMICOLON_GROUPING",
            "semicolon-grouping/v1",
            re.compile(r"；"),
        ),
        (
            "layout.paragraph_form",
            "NON_LIST_PROSE",
            "paragraph-form/v1",
            re.compile(r"(?m)^(?!\s*(?:[-*+]\s|\d+[.)、]\s|#{1,6}\s|\\(?:section|subsection|item)\b))\s*[\u3400-\u9fff]"),
        ),
    ]
    features: list[dict[str, Any]] = []
    for feature_type, code, rule_id, pattern in rules:
        feature = _feature_from_rule(feature_type, code, rule_id, pattern, units, confidence)
...
def build_voice_profile(
    *,
    ...
    samples = _validate_spec(_load_json_strict(sample_spec))
    ...
    for sample in samples:
        ...
        raw = _read_frozen(path, max_file_bytes)
        total_bytes += len(raw)
    ...
    complete_hashes = {
        record["complete_unit_sha256"]
        for record in sample_records
        if record["complete_unit"]
        and record["complete_unit_sha256"] != "0" * 64
        and record["origin"] in ELIGIBLE_ORIGINS
    }
    readable_chars = sum(unit["chinese_chars"] for unit in representatives)
    unique_complete_units = len(complete_hashes)
    ...
    validation_status = "PASS" if features and (negative_controls or has_counterexample) else "REVIEW"
    profile = {
        "schema_version": PROFILE_SCHEMA_VERSION,
