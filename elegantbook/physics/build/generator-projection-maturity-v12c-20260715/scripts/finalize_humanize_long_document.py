...
def _validate_voice_profile_binding(
    ...
    if profile.get("profile_kind") == "DEFAULT":
        ...
        if binding.get("voice_evidence_status") != "DETERMINISTIC_DEFAULT":
        ...
        voice_profiles._validate_spec(voice_profiles._load_json_strict(spec_path))
    for unit in units:
...
def _rebuild_prepare_state(
    ...
    ledger_fields = (
        ...
        "voice_profile_kind", "voice_profile_source", "voice_profile_sha256", "voice_default_disclosure",
...
def _finalize_locked(
    ...
    *,
    ...
    _, voice_binding = _validate_voice_profile_binding(run_dir, run_metadata, units)
    ...
    # Hash echo proves artifact identity only.  It does not prove that the
    # generated prose actually conforms to the Voice Profile, so the remaining
    # document-level gates stay explicitly unevaluated.
    voice_conformance_status = "NOT_EVALUATED"
    cross_unit_repetition_status = "NOT_EVALUATED"
    humanize_second_pass_convergence = "NOT_RUN"
    voice_completion_claim_allowed = False
