    ...
    if voice_profile is None:
        ...
        if profile["profile_kind"] == "PERSONAL":
            if any(value is None for value in (voice_manifest, voice_sample_spec, voice_allowed_root)):
                raise ValueError("personal_voice_profile_requires_rebuild_evidence")
            ...
            evidence_status = "REBUILT_PASS"
        else:
            rebuilt = voice_profiles.build_scene_default_profile(str(profile["binding_scene"]))
            if profile != rebuilt:
                raise ValueError("supplied_default_voice_profile_not_registered")
            if any(value is not None for value in (voice_manifest, voice_sample_spec, voice_allowed_root)):
...
def prepare(
    ...
    *,
    ...
    profile, voice_binding, voice_evidence = resolve_voice_binding(
        scene,
        voice_profile,
