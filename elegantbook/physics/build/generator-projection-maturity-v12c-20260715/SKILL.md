...
PERSONAL Profile 必须连同 manifest、sample spec 与 allowed root 提交，由 prepare 当场重建样本证据；只有自哈希而没有可重建证据的 `PERSONAL/PASS` 必须拒绝。未提供样本时，prepare 物化对应场景的版本化 `SCENE_DEFAULT`，并要求披露“不声称复现个人文风”。每个 `REWRITE` 或 `NO_CHANGE` JSON bundle 必须回显 chunk 中的 `voice_profile_sha256`；缺失、格式非法或错配分别进入 `voice_profile_hash_missing`、`voice_profile_hash_invalid`、`voice_profile_hash_mismatch`，该 unit 保持 `UNRESOLVED`。hash 一致只证明版本绑定，不证明正文符合声线。
