...
- `voice_sample_manifest.json`：样本路径、字节 hash、角色范围、去重簇、计数和排除审计；不保存整段样本文本；
...
`locator` 必须相对 `--allowed-root`，不得穿越根目录。角色范围使用原始 UTF-8 字节半开区间，必须有序、无重叠、落在文件长度内。允许角色只有 `author/quoted/exam-original/ocr/code/math/template/unknown`。`UNKNOWN` 与 `MODEL_GENERATED` 来源不进入学习；用户明确确认并实际采用的模型旧稿只能标为 `USER_CONFIRMED_ADOPTED`，仍不得伪称身份已验证。
...
字数、unit 数和 feature 支持只统计去重代表单元。先做 NFC、换行与空白规范化的 exact dedup；对不少于 40 个规范字符的段落再用字符 5-gram 检查近重复。Jaccard 不低于 0.90，或短段对长段 containment 不低于 0.95 时归为同簇。去重视图只用于判断独立性，实际风格测量仍使用未压平标点和节奏的作者视图。
...
退出码：`0=PASS`，`1=FAIL`，`2=REVIEW`。schema、自哈希、路径越界、角色范围伪造、manifest/Profile 错配属于 `FAIL`；来源归属或模板边界不能可靠裁决时为 `REVIEW`。样本确实不足而生成合法 DEFAULT 可以是 `PASS`，但必须披露默认声线。

### 14.5 长文消费规则

prepare 不接受“只给路径、由工具自己猜当前版本”。supplied Profile 必须同时提供路径和调用方 pin 的 `profile_sha256`。若为 PERSONAL，还必须提供 manifest、sample spec 与 allowed root；prepare 当场重建全部证据，只有 `voice_evidence_status=REBUILT_PASS` 才能冻结。只有自哈希、没有可重建 manifest/spec 的 PERSONAL 工件必须拒绝。prepare 把经验证的 Profile 物化为 `voice_profile.json`，把 PERSONAL 的 manifest/spec 一并冻结，并把 `profile_id/revision/confidence/kind/source/hash/default_disclosure` 写入 run metadata、unit、chunk、ledger 和完整性封条。

rewrite bundle 只回显 `voice_profile_sha256`，不得自行提交 Profile 正文、ID 覆盖值或版本覆盖值。finalize 先重建 Profile 与 unit/chunk 的绑定，再检查 bundle：

- 缺 hash：`voice_profile_hash_missing`；
- 非 64 位小写十六进制：`voice_profile_hash_invalid`；
- 与该 unit 的冻结值不同：`voice_profile_hash_mismatch`。

三种情况都在正文 validator 前拒绝该 bundle，并把 unit 留在 `UNRESOLVED`。`voice_binding_status=PASS` 仅表示所有可处理 unit 的最终 bundle 精确回显了冻结 hash；`voice_conformance_status` 仍须由独立全文声线门产生。在该门未实现前，`voice_completion_claim_allowed`、`humanize_completion_claim_allowed` 与 `full_completion_claim_allowed` 必须保持 false。
