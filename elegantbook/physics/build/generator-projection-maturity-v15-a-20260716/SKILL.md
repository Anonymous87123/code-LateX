...
语料动作锚来自 `corpus-action-sources.json` 中已通过来源可读性和行级锚点检查的 MD/TeX 动作卡。只有当前 profile 把场景标为 `SUPPORTED` 时，普通 `REWRITE/DRAFT` 才从当前场景选择至多两张满足输入条件的正向卡；没有适用卡时明确 `NONE_APPLICABLE`。当前 `COURSE/MODELING` 为 `SUPPORTED`，`RESEARCH/GENERAL/REPORT` 为 `CORPUS_INSUFFICIENT`，后三者固定 `corpus_action_support=NONE`，不列卡、不把 negative guard 当正向证据；只按原稿内生习惯与 Voice Profile 作局部动作。卡片只影响信息组织，不授权复制来源句子或迁移来源事实。来源 `origin_class=MODEL_GENERATED` 时只能承担带 detector 的负例或审计角色，构建器必须拒绝其成为 `positive_action_reference`。
...
场景只有在 profile 达到其独立正向来源门槛时才称为有语料支持。当前登记语料对
`RESEARCH/GENERAL/REPORT` 返回 `CORPUS_INSUFFICIENT`，因此这些场景的候选必须显式填写
`corpus_action_support=NONE`，按原稿与 Voice Profile 改写即可，但不得伪造动作卡、声线证据
或“已由语料支撑”的结论。工程治理报告、API 文档、
...
`corpus_action_support=NONE` 只允许当前 profile 标为 `CORPUS_INSUFFICIENT` 的场景，且必须
