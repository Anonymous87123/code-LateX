...
收尾器先校验 prepare 生成的 `prepare_integrity.json`，再逐单元调用统一验证器，只向快照派生目录发布结果，不覆盖源文件。逐单元证据必须标 `document_scope=FRAGMENT`：只容忍改前、改后完全相同的跨块 TeX 边界不平衡，环境/花括号问题一旦变化仍为硬失败；接受的 unit 组装后还要以完整文档范围重验，fragment 通过不能替代全文结构门。完整性清单缺失或任何 prepare 产物被改动时直接 `FAIL`；它只是审计辅助，收尾器还必须从冻结 `source/`、预算、保护跨度、原始范围和 canonical chunks 独立重建初始状态，拒绝同时伪造 `units.jsonl` 与封条。相同 `run-dir` 的 finalize 通过跨进程锁串行执行；空文件集、零可处理单元或只有保护内容只能是 `REVIEW`。可选 `--check-command` 在一次性检查副本中运行，真实正文 staging、待发布 validation/diff 证据和其他 run 产物的任何新增、删除或修改都会 `FAIL`。`coverage_completion_claim_allowed=true` 只证明覆盖与局部保护闭合；只有 `humanize_completion_claim_allowed=true`（兼容字段 `full_completion_claim_allowed`）时才能声称全文 Humanize 完成。v12 已实现 Voice Profile 工件的自哈希、prepare 冻结和 rewrite bundle 精确回显，但尚未实现全文声线符合性、跨块重复门和 fresh second pass，因此 `voice_conformance_status` 仍为 `NOT_EVALUATED`，Humanize 两个完成字段仍必须为 false；`rendered_partial`、`PENDING`、`UNRESOLVED`、`SKIPPED_GARBLED`、`CHANGED_AFTER_SNAPSHOT`、验证 `REVIEW/FAIL` 或编译未通过都不得被总体声明覆盖。
