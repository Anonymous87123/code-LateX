...
短文可执行 PATCH 固定按 [short-patch-workflow.md](short-patch-workflow.md) 生成 source-byte-bound bundle 和待审派生目录。bundle/application PASS 只证明 span、hash、动作和 candidate 派生一致；有无 `UNRESOLVED` 都不能跳过统一验证器或外部 paired-quality 门。
