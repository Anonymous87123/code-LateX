...
本文档不是实验流水账，而是给论文写作使用的“结果边界说明书”。它专门回答六个问题：

1. 我们现在最强的主张到底是什么。
2. 哪些结果已经足够进入论文主表与主图。
3. 哪些结果只能作为第二梯队支持或边界条件。
4. method-line（`CURV_SORT vs DG2`）到底证明了什么、没证明什么。
5. 审稿人可能怎么质疑，我们该如何回应。
6. 论文 Results/Discussion 现在最合理的写法是什么。

---

## 1. 当前一句话主张

当前数据支持一个分层、谨慎但已经相当有力的主张：
...
更进一步，在 `CURV_SORT vs DG2` 的独立预算 method-line 实验里，我们已经看到：即使一个分解器在隐藏结构恢复上更“正确”（`weighted_purity` 更高），如果它没有给后端提供更可优化的子空间几何，它依然可能在最终 `best_so_far_y` 上输给一个 purity 很低、但 geometry-first 的便宜分解器。

更谨慎地说，这仍然不是一个“所有 CEC 函数、所有优化器、所有指标都服从同一规律”的万能定理。F2/F3/F6/F10 说明，多峰结构、平台区、预算规模和后端动态会调制最终性能；F8 的 method-line 结果也说明，不同过程指标可能强调收敛的不同阶段。因此论文必须采用“函数族分层 + 指标分层”的叙事，而不是一刀切。
...
5. **Method-line 的 independent paper 已经足够进入一个单独 Results 小节。**
...
3. **证据链派生表和图已经足以把几何、性能、统计归因连起来。**
   - 主线 analysis：`outputs/tables/paper_analysis/2026-04-26_19-15-41_paper_analysis/`
   - 主线图：`outputs/plot/paper_figures_v2/`
