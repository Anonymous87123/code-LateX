...
\end{exercise}
\begin{solution}
在圆环上取一个小电荷元 \(dq\)。由于圆环上每个点到 \(P\) 的距离相等\(s=\sqrt{x^2+R^2}\)，电荷元产生的电场大小为\(dE=k\frac{dq}{s^2}\)，由于圆环具有旋转对称性，垂直于 \(x\) 轴的分量全部抵消，沿 \(x\) 轴的分量相加。因此\[dE_x=dE\cos\theta=k\frac{dq}{s^2}\frac{x}{s}=k\frac{x\,dq}{s^3}=k\frac{x\,dq}{(x^2+R^2)^{3/2}}\]因为分母和 \(x\) 对整个圆环都是相同的：\[E_x=k\frac{x}{(x^2+R^2)^{3/2}}\int dq
=k\frac{qx}{(x^2+R^2)^{3/2}},~~\boxed{\mathbf E=\frac{1}{4\pi\varepsilon_0}\frac{qx}{(x^2+R^2)^{3/2}}\mathbf i}\]当\(x=0\)有\(E=0\)，这是因为圆环上各个方向的电场完全抵消。当\(x\gg R\)时，有\((x^2+R^2)^{3/2}=x^3\left(1+\frac{R^2}{x^2}\right)^{3/2}\approx x^3\)，因此\(E\approx
\frac{1}{4\pi\varepsilon_0}\frac{q}{x^2}\)，远处看圆环，就像一个总电荷为 \(q\) 的点电荷。
求导：
\[E_x=k\frac{qx}{(x^2+R^2)^{3/2}}\implies\frac{dE_x}{dx}=kq\frac{R^2-2x^2}{(x^2+R^2)^{5/2}}
\implies R^2-2x^2=0,x=\pm\frac{\sqrt2}{2}R\]
\end{solution}

\begin{exercise}
设圆盘半径为 \(R_0\)；面电荷密度为 \(\sigma\)；场点 \(P\) 位于圆盘轴线上；场点距离圆心为 \(x\)。求点$P$处的场强。
\end{exercise}
\begin{solution}
取半径为 \(R\)、宽度为 \(dR\) 的细圆环。该细圆环的面积为\(dS=2\pi R\,dR\)，所以电荷量为\(dq=\sigma dS
=\sigma 2\pi R\,dR\)，场点到这个细圆环上任一点的距离为\(s=\sqrt{x^2+R^2}\)，根据圆环电场公式，该细圆环在轴向产生的电场为\[dE_x=k\frac{x\,dq}{(x^2+R^2)^{3/2}}=\frac{1}{4\pi\varepsilon_0}\frac{x(2\pi\sigma R\,dR)}{(x^2+R^2)^{3/2}}=\frac{\sigma x}{2\varepsilon_0}\frac{R\,dR}{(x^2+R^2)^{3/2}}
\]从圆心处的 \(R=0\) 积分到圆盘边缘 \(R=R_0\)：
\[E=\frac{\sigma x}{2\varepsilon_0}\int_0^{R_0}\frac{R\,dR}{(x^2+R^2)^{3/2}}=\frac{\sigma}{2\varepsilon_0}\left(1-\frac{x}{\sqrt{x^2+R_0^2}}\right)\]这个表达式默认场点在圆盘的 \(x>0\) 一侧。若在另一侧，方向相反。当\(x\ll R_0\)时有\(\sqrt{x^2+R_0^2}\approx R_0\)，于是\(\frac{x}{\sqrt{x^2+R_0^2}}\approx0\)，所以\(E\approx\frac{\sigma}{2\varepsilon_0}\)，当圆盘很小、场点很远时，即当\(x\gg R_0\)时，有\(\frac{x}{\sqrt{x^2+R_0^2}}=\frac{1}{\sqrt{1+R_0^2/x^2}}\)，利用近似：
\[(1+u)^{-1/2}\approx1-\frac u2\implies 1-\frac{x}{\sqrt{x^2+R_0^2}}\approx\frac{R_0^2}{2x^2}
\implies E\approx\frac{\sigma R_0^2}{4\varepsilon_0x^2}\]
圆盘总电荷为\(q=\sigma\pi R_0^2\)，所以\[
\boxed{
E\approx
\frac{1}{4\pi\varepsilon_0}
\frac{q}{x^2}
}
\]也就是说，远处的带电圆盘也可以近似成一个点电荷。
\end{solution}

\section{电通量与高斯定理}

\begin{definition}[面积矢量]
  为了处理方向问题，物理学中不仅把面积看作一个数 \(S\)，还定义面积矢量：
\[
\boxed{\mathbf S=S\mathbf e_n}
\]其中：
\(S\) 是面的面积；\(\mathbf e_n\) 是面的法向单位矢量；\(\mathbf e_n\) 的方向垂直于这个面；\(\mathbf e_n\) 的大小为 1。因此$\vec{S}$的大小是面积 \(S\)，方向是面的法线方向。
对于一个平面，法线方向有两个相反的选择，所以面积矢量也有两个可能方向。
\end{definition}

\begin{definition}[电场线与电通量]
  穿过电场中某一个面的电场线数，叫做这个面的电场强度通量，简称电通量（这里的“电场线数”是一种直观说法。电场线越密的地方，电场越强；穿过一个面的电场线越多，电通量越大）。电通量通常用希腊字母表示\(\Phi_e\)，下标 \(e\) 表示 electric，也就是电的。
  \begin{enumerate}
    \item 若电场 \(\mathbf E\) 垂直穿过这个平面，则均匀电场穿过平面的电通量\(\Phi_e=ES\)，如果电场不再垂直于平面，而是与平面的法线方向有夹角 \(\theta\)，那么真正穿过平面的电场，是电场在法线方向上的分量\(E_n=E\cos\theta\)，因此电通量为\[\Phi_e=E_n\vec{S}=ES\cos\theta\]当\(\theta<90^\circ\)时\(\cos\theta>0\)，所以\(\cos\theta>0\)；当\(\theta>90^\circ\)时此时\(\cos\theta<0\)，所以\(\Phi_e<0\)，表示电场穿过曲面的方向与面积法向量相反。如果题目给的是电场与“平面本身”的夹角 \(\alpha\)，那么由于 \(\alpha\) 和 \(\theta\) 互余有\(\theta=\frac{\pi}{2}-\alpha\)，因此\[\Phi_e=ES\cos\theta=ES\sin\alpha\]
    \item 任意曲面的电通量：把任意曲面分割成许多很小的面元\(dS\)，每一个很小的面元都可以近似看成平面。对这个小面元定义面积矢量\(d\mathbf S=dS\mathbf e_n\)（其中 \(\mathbf e_n\) 是该面元的法向单位矢量），如果这个面元处的电场为 \(\mathbf E\)，电场和法向量的夹角为 \(\theta\)，那么穿过这个面元的电通量为：\[d\Phi_e=E\cos\theta\,dS=\mathbf E\cdot d\mathbf S\]把所有面元的电通量加起来，就得到整个曲面的电通量：\[\Phi_e=\int_S d\Phi_e=\int_S E\cos\theta\,dS=\int_S\mathbf E\cdot d\mathbf S\]注意：这时一般不能把 \(E\)、\(\theta\) 和 \(S\) 直接提出积分号，因为曲面上不同位置的电场强度可能不同；曲面的法线方向不断变化；电场与法线的夹角也随位置变化。
  \end{enumerate}
\end{definition}

\begin{theorem}[高斯定理]
在真空中，通过任意闭合曲面的电场强度通量，等于该闭合曲面所包围的全部电荷的代数和除以真空介电常数
\[\boxed{\oint_S\mathbf E\cdot d\mathbf S=\frac{Q_{\mathrm{in}}}{\varepsilon_0}}\]如果闭合曲面内有多个电荷$Q_{\mathrm{in}}=\sum_iq_i$，于是：\[\boxed{\oint_S\mathbf E\cdot d\mathbf S=\frac1{\varepsilon_0}\sum_iq_i^{\mathrm{in}}}\]这里的电荷必须带符号：正电荷取正；负电荷取负。
\begin{enumerate}
  \item 如果点电荷 \(q\) 在闭合曲面 \(S\) 内：\[\boxed{\Phi_e=\frac q{\varepsilon_0}}\]无论闭合曲面是球面、立方体还是不规则形状，结果都一样。
  \item 如果点电荷在闭合曲面外面，电场线会穿过曲面两次：先进入曲面；后离开曲面。一次通量为负，一次通量为正，二者抵消$\Phi_e=0$。
\end{enumerate}
如果系统中有多个点电荷：
\[
...
\oint_S(\mathbf E_1+\mathbf E_2+\cdots+\mathbf E_n)\cdot d\mathbf S=
\Phi_{e1}+\Phi_{e2}+\cdots+\Phi_{en}
\]对于每个点电荷：
在闭合曲面外部，通量为零；
在闭合曲面内部，通量为 \(q_i/\varepsilon_0\)。
因此：
\[
\boxed{
\Phi_e=
...
\]这就是高斯定理。闭合面外的电荷对通过闭合面的电场强度通量没有贡献，但是对闭合面上各点的电场强度是有贡献的，即闭合面上各点的电场强度是由闭合面内、外所有电荷共同激发的。
\end{theorem}
\begin{note}
高斯定理为什么成立？先考虑一个点电荷 \(q\) 位于球面中心。半径为 \(r\) 的球面上，电场大小处处相同：\(E=\frac{1}{4\pi\varepsilon_0}\frac{q}{r^2}\)，球面上任意面积元的法线方向都沿半径方向，因此
\[\Phi_e=\oint_S\mathbf E\cdot d\mathbf S
=E\oint_SdS=
\frac{1}{4\pi\varepsilon_0}
\frac{q}{r^2}\cdot4\pi r^2=
\frac{q}{\varepsilon_0}
\]注意这里 \(r^2\) 和球面积中的 \(r^2\) 恰好抵消，所以只要点电荷被闭合曲面包围，无论曲面多大，通量都不变。如果把球面变形成任意闭合曲面，只要电荷始终在曲面内部，电场线穿出曲面的总数仍然不会改变，因此通量仍然是\(\frac q{\varepsilon_0}\)。
\end{note}

\begin{exercise}
给出一个三棱柱体，处在均匀电场中，例如：
\[
\mathbf E=200\mathbf i\ \mathrm{N/C}
\]要求通过这个三棱柱表面的总电通量。
\begin{figure}[htbp]
\centering
\includegraphics[width=0.3\textwidth]{pdf-exercises/23.png}
\end{figure}
\end{exercise}
\begin{solution}
由于电场是均匀的，并且三棱柱内部没有电荷，根据高斯定理，最终总电通量应该为零。
\end{solution}

\begin{conclusion}[用高斯定理求电场强度]
高斯定理本身永远成立，但它并不是对所有问题都方便。要用高斯定理直接求出 \(E\)，必须具有足够的对称性。
\begin{enumerate}
  \item 判断电荷分布具有哪种对称性：球对称；柱对称；平面对称。
  \item 根据对称性判断电场：是沿半径方向；还是垂直于带电直线；还是垂直于带电平面。
  \item 选择一个合适的假想闭合曲面，使得高斯面上电场大小相同或者某些面的电通量为零。常见选择：球对称：球面；柱对称：同轴圆柱面；平面对称：圆柱形“药盒”高斯面。
  \item 求高斯面内的电荷：确定\(Q_{\mathrm{in}}\)，应用高斯定理\[\oint_S\mathbf E\cdot d\mathbf S
=\frac{Q_{\mathrm{in}}}{\varepsilon_0}\]再解出电场强度。
\end{enumerate}
\end{conclusion}

\begin{exercise}
设一个半径为 \(R\) 的薄球壳，均匀带电，总电荷为 \(Q\)。
要求求球壳内外的电场。
\end{exercise}
\begin{solution}
\begin{enumerate}
  \item 球壳内部：\(0<r<R\)。取半径为 \(r\) 的球形高斯面，且\(r<R\)，这个高斯面完全位于球壳内部，因此没有包围任何电荷，所以\(Q_{\mathrm{in}}=0\)，所以根据高斯定理\[\Phi_e=\frac1{\varepsilon_0}\sum_iq_i^{\mathrm{in}}=\oint_{S}E\cdot d\vec{S}=0\]由于球对称性，半径为 \(r\) 的球面上电场大小处处相同，并且沿径向，\(E\cdot4\pi r^2=0\)，所以\(E=0\)。这就是均匀带电薄球壳的一个重要结论：球壳内部电场为零。
  \item 球壳外部：\(r>R\)。取半径 \(r\) 的球形高斯面，且\(r>R\)，此时高斯面包围了整个球壳，内部电荷为 \(Q\)。因此\[E\cdot4\pi r^2=\frac Q{\varepsilon_0}\implies \boxed{E=\frac{Q}{4\pi\varepsilon_0r^2},\qquad r>R}\]这说明在球壳外部，整个球壳的电场等效于一个位于球心、带电量为 \(Q\) 的点电荷。
\end{enumerate}
\end{solution}

\begin{exercise}
设一个半径为 \(R\) 的实心球，均匀带电，总电荷为 \(q\)。体电荷密度为 \(\rho\)。要求求球体内外的电场。
\end{exercise}
\begin{solution}
\begin{enumerate}
  \item 球外电场：\(r\ge R\)。取半径为 \(r\) 的球面，且 \(r\ge R\)。高斯面包围整个球体，所以\(Q_{\mathrm{in}}=q\)，由高斯定理\(E4\pi r^2=\frac q{\varepsilon_0}\)，因此
\[\boxed{E=\frac q{4\pi\varepsilon_0r^2},\quad r\ge R}\]与带电球壳外部的结果完全一样。
\item 球内电场：\(r<R\)。取半径为 \(r\) 的球形高斯面。由于电荷均匀分布，半径 \(r\) 内部所包含的电荷为：\(q_{\mathrm{in}}=\rho\frac43\pi r^3\)，应用高斯定理：
\[E4\pi r^2=\frac1{\varepsilon_0}\rho\frac43\pi r^3\implies E=\frac{\rho r}{3\varepsilon_0}=\frac{qr}{4\pi\varepsilon_0R^3}\]
\end{enumerate}
所以均匀带电实心球的电场图像是球内从 0 开始，随 \(r\) 线性增加；到表面达到最大；球外按 \(1/r^2\) 衰减。
\end{solution}


\begin{exercise}
设无限长直线的线电荷密度为\(\lambda\)，求距离直线 \(r\) 处的电场。
\end{exercise}
\begin{solution}
选择一个与带电直线同轴的圆柱面，半径为 \(r\)，高度为 \(h\)，由圆柱侧面，上下底面组成。在圆柱侧面上，电场方向沿圆柱半径方向，正好与侧面的外法线同向。而且侧面上电场大小处处相同，因此：
\[
\Phi_{\text{侧}}=ES_{\text{侧}}=E(2\pi rh)
\]长度为 \(h\) 的带电直线所包含的电荷为\(Q_{\mathrm{in}}=\lambda h\)，应用高斯定理\[E2\pi rh
=\frac{\lambda h}{\varepsilon_0}\implies E=\frac{\lambda}{2\pi\varepsilon_0 r}\]所以无限长均匀带电直线的电场与距离成反比\[E\propto\frac1r\]正电荷时电场向外，负电荷时电场向内。
\end{solution}

\begin{exercise}
设无限大平面上的面电荷密度为$\sigma$，求距平面为$r$处的电场强度。
\end{exercise}
\begin{solution}
选择一个薄圆柱形高斯面垂直穿过带电平面。圆柱侧面的法线方向平行于带电平面，而电场垂直于带电平面，所以\(\Phi_{\text{侧}}=0\)，在两个底面上，电场都与外法线方向一致。因此每个底面的通量为\(ES'\)，两个底面总通量\(\Phi_e=2ES'\)，药盒内部包围的电荷\(Q_{\mathrm{in}}=\sigma S'\)，应用高斯定理：
\[
2ES'=\frac{\sigma S'}{\varepsilon_0}\implies E=\frac{\sigma}{2\varepsilon_0}
\]无论离平面多远，只要仍然处在理想无限平面的两侧，电场大小都相同。对于正电荷面，\(\mathbf E\)从平面向外。
对于负电荷面\(\mathbf E\)从两侧指向平面。电场大小始终为\(\frac{|\sigma|}{2\varepsilon_0}\)
\end{solution}

\begin{note}[两个平行带电平面的电场叠加]
每一个无限大带电平面单独产生的电场大小都是：
\[
E_0=\frac{\sigma}{2\varepsilon_0}
\]然后利用电场叠加原理，把两个平面的电场相加。
\begin{enumerate}
  \item 两个都带正电：\(+\sigma\) 和 \(+\sigma\)：左边正电荷面产生的电场向左；右边正电荷面在其左侧产生的电场向左。两个电场同向：
\[E_{\text{左}}=\frac{\sigma}{2\varepsilon_0}+\frac{\sigma}{2\varepsilon_0}=\frac{\sigma}{\varepsilon_0}\]左平面的电场向右，右平面的电场向左，方向相反、大小相等：
\[
E_{\text{中}}=0
\]两个电场都向右：
\[
...
E=
\begin{cases}
\sigma/\varepsilon_0,&\text{外侧}\\
0,&\text{中间}
\end{cases}
}
\]
\item 一个正平面和一个负平面：\(+\sigma\) 和 \(-\sigma\)：左边正电荷面产生的电场向左；右边负电荷面在其左侧产生的电场指向右。两者抵消：
\[
E_{\text{左}}=0
\]左边正平面的电场向右；右边负平面的电场也指向右。
两者相加：
\[E_{\text{中}}=\frac{\sigma}{2\varepsilon_0}+\frac{\sigma}{2\varepsilon_0}=\frac{\sigma}{\varepsilon_0}\]
右侧区域两个电场方向相反，彼此抵消：
\[
E_{\text{右}}=0
\]因此：
\[\boxed{E=\begin{cases}0,&\text{外侧}\\\sigma/\varepsilon_0,&\text{中间}\end{cases}}\]
\end{enumerate}
\end{note}

\section{静电场的环路定理与电势}

设点电荷 \(q\) 固定在原点附近，试验电荷 \(q_0\) 从 A 沿任意曲线移动到 B。某一小段位移为 \(d\mathbf l\)，它与电场 \(\mathbf E\) 的夹角为 \(\theta\)。这小段路程上电场力做的功是：
\[
dA=\mathbf F\cdot d\mathbf l
=q_0\mathbf E\cdot d\mathbf l
=q_0E\,dl\cos\theta=
\frac{q_0q}{4\pi\varepsilon_0r^2}dr
\]
从 A 积分到 B：
\[A_{AB}=\frac{q_0q}{4\pi\varepsilon_0}\int_{r_A}^{r_B}\frac{dr}{r^2}=\frac{q_0q}{4\pi\varepsilon_0}\left(\frac{1}{r_A}-\frac{1}{r_B}\right)\]
例如，若 \(q>0,q_0>0\)，把 \(q_0\) 从靠近 \(q\) 的 A 点移到较远的 B 点，电场力方向向外，与运动方向一致，所以电场力做正功。反过来，若把正试验电荷推向正场源电荷，电场力做负功，外力必须做正功。
\begin{theorem}[静电场是保守场，满足环路定理]
对于多个点电荷组成的系统\(\mathbf E=\sum_i\mathbf E_i\)，电场力做功为：
\[A=\int_l q_0\mathbf E\cdot d\mathbf l=\sum_i q_0\int_l\mathbf E_i\cdot d\mathbf l\]
每一个点电荷产生的静电场做功都只和起止位置有关，因此它们相加后的总电场做功也只和起止位置有关。
这类力叫保守力，对应的场叫保守场。
如果从 A 出发，沿任意闭合路径最后又回到 A，那么初、末位置相同，静电场力总功必为零：
\[
\boxed{
\oint_l \mathbf E\cdot d\mathbf l=0
}
\]这就是静电场的环路定理。假如从 A 到 C 有两条路径：ABC 和 ADC。因为两条路径起点、终点都相同，所以：
\[\int_{ABC}\mathbf E\cdot d\mathbf l=\int_{ADC}\mathbf E\cdot d\mathbf l\]将第二条路径反向，就构成闭合回路 ABCDA，于是闭合积分为零。
\end{theorem}

\begin{definition}[电势与电势差]
电势能与试验电荷 \(q_0\) 成正比。为了描述电场本身在某点的性质，我们把电势能除以试验电荷：
\[\boxed{U_A=\frac{W_{pA}}{q_0}}\]
这就是 A 点的电势。单位是伏特\(1\ \mathrm V=1\ \mathrm{J/C}\)，A、B 两点的电势差定义为：
\[
\boxed{
U_{AB}=U_A-U_B=\int_A^B\mathbf E\cdot d\mathbf l,~~~~
A_{AB}=q_0U_{AB}
}
\]
电场方向总是指向电势降低最快的方向。更一般地说：
\[
\boxed{\mathbf E=-\nabla U}
\]在一维情况下：
\[
E_x=-\frac{dU}{dx}
\]对于有限范围内的带电体，距离足够远时电场趋于零，因此通常取\(U_\infty=0\)，于是某点 A 的电势为：
\[
\boxed{
U_A=\int_A^\infty\mathbf E\cdot d\mathbf l
}
\]
\end{definition}

\begin{conclusion}[电偶极子在匀强电场中的电势能]
电偶极子由 \(-q\) 与 \(+q\) 构成，电偶极矩\(\vec{p}=q\vec{l}\)，方向从负电荷指向正电荷。设外电场为匀强电场 \(\vec{E}\)，偶极矩与电场夹角为 \(\theta\)。正、负电荷所在位置的电势分别为 \(U_+\) 和 \(U_-\)。偶极子的总电势能等于两个电荷电势能之和：
\[W_p=qU_++(-q)U_-=qU_+-qU_-=-q(U_--U_+)\]
而从负电荷位置沿 \(\mathbf l\) 移到正电荷位置：
\[U_--U_+=\int_-\!^+\mathbf E\cdot d\mathbf l=El\cos\theta\implies W_p=-qEl\cos\theta=-\vec{p}\cdot\vec{E}=-qE\cos\theta\]
\begin{itemize}
\item \(\theta=0\)，即 \(\mathbf p\) 与 \(\mathbf E\) 同向：\(W_p=-pE\)，电势能最低，是稳定平衡。
\item \(\theta=\pi/2\)：\(W_p=0\)
\item \(\theta=\pi\)，即 \(\mathbf p\) 与 \(\mathbf E\) 反向：\(W_p=+pE\)，电势能最高，是不稳定平衡。
\end{itemize}
所以，“偶极矩会趋向与外电场同向”，从能量角度说，就是系统总会趋向更低的电势能。
\end{conclusion}

\begin{definition}[点电荷的电势]
  取无穷远为零电势。点电荷 \(q\) 在距离 \(r\) 处产生的电场为\(\vec{E}=\frac{1}{4\pi\varepsilon_0}\frac{q}{r^2}\mathbf e_r\)，电势定义为\[U(r)=\int_r^\infty\mathbf E\cdot d\mathbf l=\int_r^\infty\frac{1}{4\pi\varepsilon_0}\frac{q}{r'^2}\,dr'=
\boxed{\frac{q}{4\pi\varepsilon_0r}}\]注意：电势是标量，带正负号。若 \(q>0\)，则 \(U>0\)；若 \(q<0\)，则 \(U<0\)。离正电荷越近，电势越高；离负电荷越近，电势越低。
\end{definition}

\begin{theorem}[电势的叠加原理]
电势是标量叠加：
\[
\boxed{
U_A=
\sum_{i=1}^n
\frac{q_i}{4\pi\varepsilon_0r_i}
}
\]其中 \(r_i\) 是第 \(i\) 个点电荷到场点 A 的距离。对于连续带电体：
\[
dq=\rho\,dV
\]每个电荷元在场点产生：
\[
...
\frac{1}{4\pi\varepsilon_0}
\int_V\frac{\rho\,dV}{r}
\]同理，也可用线电荷密度 \(\lambda\) 或面电荷密度 \(\sigma\) 表示。计算电势通常有两种方法：
\begin{enumerate}
  \item 已知电场，利用
\[
...
\]
\end{enumerate}第二种方法往往更简洁，因为电势不需要处理方向。
\end{theorem}

\begin{exercise}
半径为 \(R\) 的细圆环带总电荷 \(q\)，场点 P 在圆环轴线上，离圆心距离为 \(x\)。求电势？
\end{exercise}
\begin{solution}
圆环上任意电荷元 \(dq\) 到 P 的距离都相同：\(r=\sqrt{x^2+R^2}\)，因此\(dU_P=
\frac{1}{4\pi\varepsilon_0}\frac{dq}{r}\)，因为 \(r\) 对整条圆环都是常数：
\[U_P=\frac{1}{4\pi\varepsilon_0r}\int dq=\frac{q}{4\pi\varepsilon_0\sqrt{x^2+R^2}}\]
当 \(x=0\)，即在圆心处\(U_0=\frac{q}{4\pi\varepsilon_0R}\)，它不等于零，可是圆心处电场却为零。当 \(x\gg R\)，\(\sqrt{x^2+R^2}\approx x\)，所以\(U_P\approx
\frac{q}{4\pi\varepsilon_0x}\)，远处看，圆环的电势就像一个总电荷集中在圆心的点电荷。另外，\(E_x=-\frac{dU}{dx}\)，可以从圆环电势反推出圆环轴线电场：
\[
E_x=
\frac{1}{4\pi\varepsilon_0}
\frac{qx}{(x^2+R^2)^{3/2}}
