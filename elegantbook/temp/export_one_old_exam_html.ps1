param(
  ...
  [Parameter(Mandatory=$true)][string]$OutStem
)
...
$outDir = Join-Path (Join-Path (Join-Path $base 'tmp') 'past-exams-dochtml') $OutStem
...
$html = Join-Path $outDir ($OutStem + '.htm')
$txt = Join-Path $outDir ($OutStem + '.word.txt')
...
try {
  ...
  Write-Host "SAVEHTML $html"
  $doc.SaveAs2($html, 10)
  Write-Host "DONE $OutStem"
}
finally {
  if ($null -ne $doc) {
