...
\end{definition}
可以看出，闭式谓词公式在给定解释下可以判断真值；含自由变元的命题函数需要给自由变元赋值后才有真值。个体域可以是无限或者有限的；没有特别说明时，通常先固定当前讨论语境下的共同论域，再在这个论域内解释各个谓词。所以当题目没有明说个体域时，最好显式说明你采用的论域，或者用谓词把讨论范围写出来，比如设置命题函数“$M(x):x$是人”，而不要默认跳过这一步。
\begin{definition}{量词}{}
...
\end{definition}
注意，当当前论域比实际讨论对象更大时，常把表示“对象范围”的谓词和表示“对象性质”的谓词分开写。此时常见的表达分别是：$\forall x (P(x) \rightarrow Q(x))$ 与 $\exists x (P(x) \land Q(x))$。
...
4. 多个量词约束的同名变元可以按各自辖域分别换名，但每次换名必须只在对应量词的辖域内一致进行
...
\end{align*}
这里$B(x,y)$中的$x$是自由变元，不受前件中原$\exists x$的约束；换名为$z$只是为了避免把两个不同角色的$x$混淆。
...
2. 需要注意在这个公式中，$B(x)$中的变元是自由变元（没有被任何量词约束），因此两边都表示关于某个特定个体$x$的陈述。左边说"对任何一个$y$，如果$y$具有性质$A$，那么$x$具有性质$B$"；右边说"如果存在某个个体具有性质$A$，那么$x$具有性质$B$"。例如，"如果任何人都是医生，那么张三健康"等价于"如果有医生存在，那么张三健康"。由于$x$是自由变元，两边讨论的都是同一个特定的$x$，$B(x)$描述的是这个特定个体的性质，不受前面的$y$量词影响。
...
\begin{example}{}{}
    0元谓词公式是命题吗？
\end{example}
是。0元谓词公式没有自由变元，在给定解释下它是真或假确定的闭式，因此可以视为命题。
