...
以上等价在通常离散数学课程默认的非空集合语境下成立；其中“满射当且仅当有右逆”以及本节后面若干无限集结论，默认采用承认选择公理（至少可数选择）的常见教材口径。
\end{definition}
...
3.\textbf{极限情况}：自然数集 $\mathbb{N}$ 是包含 $0$ 且在后继运算下封闭的最小归纳集\\
...
\textbf{性质}：每个自然数都是它前面所有自然数的集合；$n = \{0, 1, 2, \ldots, n-1\}$；这种定义方式使得 $m < n$ 当且仅当 $m \in n$；自然数集 $\mathbb{N}$ 是一个归纳集；并且每个自然数都是传递集，即若$x\in n$，则$x\subseteq n$
\end{definition}
...
反设$n+1 \in n+1$。那么由于 $n+1 = n \cup \{n\}$，我们有两种可能：$n+1 \in n$，或者 $n+1 = n$。考虑第一种情况：如果 $n+1 \in n$，由于每个自然数都是传递集，所以 $n+1 \subseteq n$。但 $n \in n+1$（由定义），因此 $n \in n$，这与归纳假设 $n \notin n$ 矛盾。
...
\textbf{注}：可数无限集的基数记为 $\aleph_0$（阿列夫零），实数集的基数常记为 $\mathfrak{c}=2^{\aleph_0}$（连续统基数）。
\end{definition}
\noindent\textbf{说明：} 以下关于“无限集与真子集等势”“无限集含可数子集”以及“可数个可数集的并仍可数”的结论，按通常离散数学教材的叙述习惯，默认使用选择公理或至少可数选择。
