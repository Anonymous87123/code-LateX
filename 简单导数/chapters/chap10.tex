...
\]
直接计算 \(\frac{X-Y}{1+XY}\)。分子与分母通分后都出现公共因子
\[
...
令 \( X_n = \sqrt{\frac{3}{2}}F_{2n} \)，则每一项等于 \(\arctan X_n - \arctan X_{n-1}\)。对对称部分和求极限，得该和为 \(\frac{\pi}{2}\)。同理，另一和也为 \(\frac{\pi}{2}\)。
\end{solution}
...
\]
直接计算 \(\frac{X-Y}{1+XY}\) 即可得到题中的分式，因此原级数可以写成反正切差。
...
\end{example}
\begin{solution}
把 \(n^4 + n^2 + 1\) 写成 \((n^2+1)^2 - n^2 = (n^2-n+1)(n^2+n+1)\)。再利用 \((n+1)^2 - (n+1) + 1 = n^2+n+1\)，即可得到
\[
...
\end{example}
\begin{solution}
由三倍角公式 \(\cos 3\alpha = 4\cos^3\alpha - 3\cos\alpha\)，令 \(a_n = 2\cos(2\cdot 3^{n-1} x)\)，可得所需递推。再取 \(x=\pi/8\)，便有 \(a_n + 1 = \frac{\sin(3^n x)}{\sin(3^{n-1} x)}\)。
...
\end{example}
\begin{solution}
令 \(A=y^2-4\)。在立方递推下可得到 \(A(A+3)^2\) 的分母结构，再由常数项消去确定系数 \(9\)。
...
\]
再把 \(x\) 替换为 \(2^{n-1}\theta\)，即可得到所需裂项。

原项可写为 \(\cot(2^{n-1}\theta) - \cot(2^n\theta)\)。求和得：
\[
