...
插值多项式的次数等于各点插值阶数（含 $0$ 阶）之和减 $1$，余项阶数比它高 $1$。再结合余项阶数与原函数的光滑性条件即可判断题型。
\section{插值法}
...
\text{证明：}
\begin{enumerate}
    \item[(1)] $f(x) \leqslant \pi$.
    \item[(2)] $f'(x) > \sin\left(\sqrt{2}\right) \ \left(x \in \left[\sqrt{2},\ \frac{\pi}{\sqrt{2}}\right]\right)$
\end{enumerate}
\end{example}
\begin{solution}
...
既然在整个区间 $(0, \frac{\pi}{\sqrt{2}}]$ 内都有 $f(x) < \pi$，则 $\sin f(x)$ 不会降为 $0$，$f'(x)$ 也不会变为非正。故该区间内 $f(x)$ 单调递增。又因为 $\sin y$ 在 $[0, \pi]$ 上是向下凹的，故 $f'(x) = \sqrt{2\sin f(x)}$ 的最小值取在两端点。下面估计 $f'(\sqrt{2})$ 和 $f'\left(\frac{\pi}{\sqrt{2}}\right)$ 的下界。\\
...
\end{example}
\begin{solution}
    这里只用到端点函数值，不必加入导数条件。分别在区间$[0,1]$与$[1,2]$上对靠近的端点作一次展开即可。
...
\end{example}
\begin{solution}
这里只需满足$f(0),f(1)$两个函数值条件，不需加入导数条件，因此插值多项式取一次即可。由余项公式，存在$\theta(x)\in(0,1)$使得
...
g(x)=\int_0^xF(t)\dd t,
\]
则$g'(0)=0,g'(1)=\dfrac52$，且$g(0)=0,g(1)=1$。于是只需证明：$g(x)$在$(0,1)$上三阶可导，且存在$\xi\in(0,1)$使得$g'''(\xi)=3$。按这四个条件作三次插值，
...
\end{example}
\begin{solution}
构造$G(x)=f(x)-p(x)$，则$G(0)=G(1)=0,G'(0)=G'(1)=0$。继续应用 Rolle 定理，只能推出存在$\xi\in(0,1)$使得$G'''(\xi)=0$。若要进一步得到四阶导数的信息，还需额外添入一个零点。于是只需证明
\[
...
\end{example}
\begin{solution}
取$c\in(0,1)$使得$f(c)=-1$。于是插值条件为$f(0)=f(1)=0,f(c)=-1,f'(c)=0$。下面分别在区间$[0,c]$和$[c,1]$上作插值。
...
\end{example}
\begin{solution}
    现有微分条件不足以直接同时处理四个插值点，因此把区间分成左右两段，并取公共点$x=0$来连接两边的插值式。
...
\begin{solution}
    ...
    \]
    设 $\displaystyle M = \max_{x\in[a,b]} |f(x)|$，则 $M>0$（否则 $f\equiv0$，不等式成立）。取 $c\in(a,b)$ 使 $|f(c)|=M$。在上式中令 $x=c$，并注意到 $|f(y)|\le M$，$y-a\le c-a$（当 $y\in[a,c]$），$b-y\le b-c$（当 $y\in[c,b]$），故
    \begin{align*}
...
\end{align*}
(2) $F(x)$两阶可导，$|F''(x)|\leqslant M,F(a)=F(b)=F'(a)=F'(b)=0$。取$x_0\in(a,b)$ 为 $|F(x)|$ 的极大值点；若极值在端点取得，则结论直接成立。分别在区间$[a,x_0]$和$(x_0,b)$上作展开：
\begin{align*}
