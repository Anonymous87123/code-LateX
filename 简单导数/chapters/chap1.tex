\chapter{微分中值定理}
本章整理微分中值定理的几类题目，主要用埃米特插值、常数 $K$ 值法和罗尔定理。

题目通常给出函数 $f(x)$ 的若干值、若干阶导数值以及待证结论。证明时先构造插值多项式 $p(x)$，再令 $G(x)=f(x)-p(x)$，对 $G$ 反复应用罗尔中值定理。

插值多项式的次数等于各点插值阶数（含 $0$ 阶）之和减 $1$，余项阶数比它高 $1$。余项阶数与原函数的光滑性条件合在一起，便能确定应当使用的证明模型。
\section{插值法}

\begin{example}
$f(x)$满足$f(0) = f'(0) = 0, f''(x) = \cos f(x)\ (x \in \mathbb{R})$，
\text{证明：}\\
（1）$f(x) \leqslant \pi$.\\
（2）$f'(x) > \sin\left(\sqrt{2}\right) \ \left(x \in \left[\sqrt{2},\ \frac{\pi}{\sqrt{2}}\right]\right)$.
\end{example}
\begin{solution}
（1）由$f''(x) = \cos f(x)$得$2f'(x)f''(x) = 2f'(x)\cos f(x)$，两边积分：
$$\int 2f'(x)f''(x) dx = \int 2f'(x)\cos f(x) dx\implies(f'(x))^2 = 2\sin f(x) + C$$
代入初始条件 $f(0) = 0, f'(0) = 0$得到$ C = 0$，因此$(f'(x))^2 = 2\sin f(x)$。
由$(f'(x))^2 \geqslant 0$ 恒成立，必须满足$\sin f(x) \geqslant 0$，因为 $f(0) = 0$，且函数 $f(x)$ 在 $\mathbb{R}$ 上可导连续，它的值域必须是一个连续的区间。在包含 0 的情况下，满足 $\sin y \geqslant 0$ 的最大连通区间是 $[0, \pi]$。因此，对于任意 $x \in \mathbb{R}$，都有 $f(x) \in [0, \pi]$，即 $f(x) \leqslant \pi$ 成立。\\
（2）对于$x \in \left[\sqrt{2},\ \frac{\pi}{\sqrt{2}}\right]$，
由 $f(0) = 0$、$f''(0) = 1 > 0$ 可知，从 $x=0$ 开始的一段区间内，$f(x)$ 单调递增，此时 $f'(x) = \sqrt{2\sin f(x)}>0$。
利用 $f''(x) = \cos f(x) \leqslant 1$ 积分两次得到的上限：$$f(x) \leqslant \frac{x^2}{2}$$
把区间的最右端 $x = \frac{\pi}{\sqrt{2}}$ 代入这个上限
$$f\left(\frac{\pi}{\sqrt{2}}\right) \leqslant \frac{\left(\frac{\pi}{\sqrt{2}}\right)^2}{2} = \frac{\pi^2}{4}<\pi$$
既然在整个区间 $(0, \frac{\pi}{\sqrt{2}}]$ 内都有 $f(x) < \pi$，则 $\sin f(x)$ 不会降为 $0$，$f'(x)$ 也不会变为非正。故该区间内 $f(x)$ 单调递增。又因为 $\sin y$ 在 $[0, \pi]$ 上是向下凹的，故 $f'(x) = \sqrt{2\sin f(x)}$ 的最小值取在两端点。为了得到区间内的统一下界，分别控制 $f'(\sqrt{2})$ 和 $f'\left(\frac{\pi}{\sqrt{2}}\right)$。\\
先估计 $f\left(\frac{\pi}{\sqrt{2}}\right)$。已知对于 $u \in \left[0, \frac{\pi}{2}\right]$，有 $\sin u \geqslant \dfrac{2u}{\pi}$。由
\[\dd x = \dfrac{\dd f}{\sqrt{2\sin f}}\implies x = \int_0^{f(x)} \frac{\dd u}{\sqrt{2\sin u}}\]
假设 $f\left(\frac{\pi}{\sqrt{2}}\right) < \frac{\pi}{2}$，则：
$$\frac{\pi}{\sqrt{2}} = \int_0^{f(\frac{\pi}{\sqrt{2}})} \frac{\dd u}{\sqrt{2\sin u}} < \int_0^{\frac{\pi}{2}} \frac{\dd u}{\sqrt{2\sin u}} \leqslant \int_0^{\frac{\pi}{2}} \frac{\dd u}{\sqrt{4u/\pi}} = \frac{\sqrt{\pi}}{2} \left[ 2u^{\frac{1}{2}} \right]_0^{\frac{\pi}{2}} = \frac{\pi}{\sqrt{2}}$$
这就导出矛盾 $\frac{\pi}{\sqrt{2}} < \frac{\pi}{\sqrt{2}}$。因此 $f\left(\frac{\pi}{\sqrt{2}}\right) \geqslant \frac{\pi}{2}$。再由 $f(x) \leqslant \frac{x^2}{2}$ 得
\[
f\left(\frac{\pi}{\sqrt{2}}\right) \leqslant \frac{\pi^2}{4},
\]
从而
\[
\sin f\left(\frac{\pi}{\sqrt{2}}\right) \geqslant \sin\left(\frac{\pi^2}{4}\right) > \sin\left(\frac{5\pi}{6}\right) = \frac{1}{2}.
\]
于是
$$f'\left(\frac{\pi}{\sqrt{2}}\right) = \sqrt{2\sin f\left(\frac{\pi}{\sqrt{2}}\right)} > \sqrt{2 \times\sin\left(\frac{\pi^2}{4}\right)}>\sqrt{2 \times\sin\frac{5\pi}{6}} = 1$$
再估计左端点处的导数。利用 $\cos y \geqslant 1 - \frac{y^2}{2}$，并结合 $f(x) \leqslant \frac{x^2}{2}$，可得：
$$f''(x) = \cos f(x) \geqslant 1 - \frac{f(x)^2}{2} \geqslant 1 - \frac{x^4}{8}\implies f'(x) \geqslant x - \frac{x^5}{40}$$
将 $x = \sqrt{2}$ 代入上式：$$f'(\sqrt{2}) \geqslant \sqrt{2} - \frac{(\sqrt{2})^5}{40} = \sqrt{2} - \frac{4\sqrt{2}}{40} = \frac{9\sqrt{2}}{10}$$因为 $\frac{9\sqrt{2}}{10} = \sqrt{1.62} > 1$，所以 $f'(\sqrt{2}) > 1$。
最终得出：$$f'(x) > 1 > \sin(\sqrt{2}) \quad \left(x \in \left[\sqrt{2},\ \frac{\pi}{\sqrt{2}}\right]\right)$$证毕。
\end{solution}



\begin{example}[$\int_a^bf\left(x\right)dx=\left(b-a\right)f\left(\frac{a+b}{2}\right)+\frac{1}{24}\left(b-a\right)^3f''\left(\xi\right)$]
...
\end{example}
\begin{solution}
取$f(x)$的原函数$F(x)$，则$F(x)\in C^3[a,b]$，且$F'(x)=f(x)$。原结论等价于证明存在$\xi\in(a,b)$使得
\[
\frac{F(b)-F(a)}{b-a}=F'\left(\frac{a+b}{2}\right)+\frac{(b-a)^2F'''(\xi)}{24}.
\]
按这四个条件作三次插值，多项式可写成
\begin{align*}
...
它满足
\[
p(a)=F(a),\quad p(b)=F(b),\quad p\left(\frac{a+b}2\right)=F\left(\frac{a+b}2\right).
\]
对它求导，并令$p'\left(\frac{a+b}2\right)=F'\left(\frac{a+b}2\right)$：
\begin{align*}
...
\end{align*}
构造$G(x)=f(x)-p(x)$，则$G(a)=G(b)=G\left(\frac{a+b}2\right)=0$。由 Rolle 定理，存在$\xi_1\in(a,\frac{a+b}2)$与$\xi_2\in(\frac{a+b}2,b)$使得$G'(\xi_1)=G'(\xi_2)=0$。再结合$G'\left(\frac{a+b}2\right)=0$，继续应用 Rolle 定理，可得
\[
G''(\xi_3)=0,\qquad G''(\xi_4)=0,
\]
其中$\xi_3\in(\xi_1,\frac{a+b}2)$，$\xi_4\in(\frac{a+b}2,\xi_2)$。最后再用一次 Rolle 定理，存在$\xi\in(\xi_3,\xi_4)\subset(a,b)$使得$G'''(\xi)=0$，于是$F'''(\xi)=p'''(\xi)=6c$，即
\[F'''(\xi)=\frac{24}{(b-a)^3}(F(b)-F(a))-\frac{24}{(b-a)^2}F'\left(\frac{a+b}2\right)\]
\end{solution}

\begin{example}[$\frac{f(b)-f(a)}{b-a}=\frac{f'(a)+f'(b)}{2}-\frac{f'''(\xi)}{12}(b-a)^2$]
...
此时不必显式求出$m$。
设 $G(x)=f(x)-p(x)$，则 $G(a)=G(b)=0,\ G'(a)=G'(b)=0$。由罗尔中值定理，先有 $\xi_1\in(a,b)$ 使 $G'(\xi_1)=0$；再有 $\xi_2,\xi_3\in(a,b)$ 使 $G''(\xi_2)=0,\ G''(\xi_3)=0$；于是存在 $\xi\in(a,b)$ 满足 $G'''(\xi)=0$。因此
\[
f'''(\xi)=p'''(\xi)=6k=\frac{6(f'(b)+f'(a))}{(b-a)^2}-\frac{12(f(b)-f(a))}{(b-a)^3}.
\]
\end{solution}
\newpage
\begin{example}[$\left|\int_0^2f(x)\dd x\right|\leqslant M$]
设$f\in C[0,2]\cap D(0,2)$，满足$f(0)=f(2)=0,|f'(x)|\leqslant M,\forall x\in(0,2)$，证明\[\left|\int_0^2f(x)\dd x\right|\leqslant M\]
\end{example}
\begin{solution}
    端点条件给出两侧的线性控制：在左半区间从 $0$ 展开，在右半区间从 $2$ 展开。
    
    当$x\in[0,1]$时，由拉格朗日中值定理，
    \[f(x)=f(0)+f'(\xi(x))x=f'(\xi(x))x,\]
    故 \(|f(x)|\leqslant Mx\)。
    当$x\in[1,2]$时，由拉格朗日中值定理，
    \[f(x)=f(2)+f'(\eta(x))(x-2),\]
    故 \(|f(x)|\leqslant M(2-x)\)。
    所以
    \begin{align*}
...
\end{solution}
\newpage
\begin{example}[$ \left|\int_0^1f(x)\dd x\right|\leqslant \dfrac{M}{12}$]
设$f\in D^2[0,1],f(0)=f(1)=0,|f''(x)|\leqslant M$，证明$\displaystyle \left|\int_0^1f(x)\dd x\right|\leqslant \dfrac{M}{12}$
\end{example}
\begin{solution}
由 $f(0)=f(1)=0$ 可取一次插值余项表示 $f(x)$，从而把积分估计转化为二阶导数的界。由余项公式，存在$\theta(x)\in(0,1)$使得
\[f(x)=\frac{f''(\theta(x))}{2}x(x-1),\forall x\in[0,1]\]
这一式子也可用 $K$ 值法推出。令
\[
F(y)=f(y)-Ky(y-1),
\]
其中 $K$ 依赖于固定的 $x$。由$F(0)=F(1)=F(x)=0$，两次应用 Rolle 定理可得存在$\theta(x)\in(0,1)$使得$F''(\theta(x))=0$。而
\[
F''(\theta(x))=f''(\theta(x))-2K,
\]
故
\[
K=\frac{f''(\theta(x))}{2},
\qquad
f(x)=\frac{f''(\theta(x))}{2}x(x-1).
\]
于是
\begin{align*}
...
\end{solution}
\newpage
\begin{example}[$f'''(\theta)=0$]
    设$f\in C^3[0,2]$满足\[f(0)=f'(0)=0,\int_0^2f(x)\dd x=8\int_0^1f(x)\dd x\]求证存在$\theta\in(0,2)$使得$f'''(\theta)=0$
\end{example}
\begin{solution}
设$\displaystyle F(x)=\int_0^x f(y)\dd y$，则$F(x)\in C^4[0,2]$，且$F(0)=F'(0)=F''(0)=0,\ F(2)=8F(1)$。取插值多项式
\[
p(x)=x^3(ax+b).
\]
由 $p(2)=8p(1)$ 得 $a=0$，再由 $p(1)=F(1)$ 得 $b=F(1)$，故
\[
p(x)=F(1)x^3.
\]
令
...
则
\[
G(0)=G(1)=G(2)=0,\qquad G'(0)=0,\qquad G''(0)=0.
\]
由罗尔定理，存在 $\xi_1\in(0,1),\xi_2\in(1,2)$ 使得
\[
G'(\xi_1)=G'(\xi_2)=0.
\]
再由罗尔定理，存在 $\theta_1,\theta_2$ 使得
\[
G''(\theta_1)=G''(\theta_2)=0.
\]
结合 $G''(0)=0$，再应用罗尔定理，存在 $\eta_1\in(0,1),\eta_2\in(1,2)$ 使得
\[
G'''(\eta_1)=G'''(\eta_2)=0.
\]
再由罗尔定理，存在 $\theta\in(0,2)$ 使得
\[
G''''(\theta)=0.
\]
而
...
\newpage
\begin{example}[$f'(\xi)=3$]
    $f(x)$在$[0,1]$可导，且$\displaystyle \int_0^1f(x)\dd x=\dfrac52,\int_0^1 xf(x)\dd x=\dfrac32$,求证在$(0,1)$存在$\xi$使$f'(\xi)=3$.
\end{example}
\begin{solution}
设\[F(x)=\displaystyle\int_0^xf(t)\dd t,\qquad G(x)=\displaystyle\int_0^xtf(t)\dd t\]则有\[F(0)=G(0)=0,\qquad F(1)=\dfrac52,\qquad G(1)=\dfrac32\]。对 $\int_0^1xf(x)\dd x$ 作分部积分：
\[\int_0^1xf(x)\dd x=\int_0^1x\dd F(x)=\left.xF(x)\right|_0^1-\int_0^1F(x)\dd x\Rightarrow \int_0^1F(x)\dd x=1\]
则问题转化为：$F(x)$在$(0,1)$上二阶可导，且$F(0)=0,F(1)=\dfrac52,\displaystyle\int_0^1F(x)\dd x=1$，证明存在$\xi\in(0,1)$使得$F''(\xi)=3$。
再取
\[
g(x)=\int_0^xF(t)\dd t,
\]
则$g'(0)=0,g'(1)=\dfrac52$，且$g(0)=0,g(1)=1$。于是问题落到函数 $g$ 上；若能得到某点 $g'''(\xi)=3$，便由 $g'''=F''=f'$ 得到原结论。按这四个条件作三次插值，
...
\end{align*}
构造$W(x)=g(x)-p(x)$，则$W(0)=W(1)=W'(0)=W'(1)=0$。由 Rolle 定理，先有$\xi_1\in(0,1)$使得$W'(\xi_1)=0$；再有$\xi_2\in(0,\xi_1)$与$\xi_3\in(\xi_1,1)$使得$W''(\xi_2)=W''(\xi_3)=0$；最后存在$\xi\in(\xi_2,\xi_3)\subset(0,1)$使得$W'''(\xi)=0$。因此
\[
g'''(\xi)=p'''(\xi)=6k=3.
\]
\end{solution}
\newpage
\begin{example}[$f(a)+f(b)-2f\left(\frac{a+b}{2}\right)=\frac14(b-a)^2f''(\xi)$]
$f(x)$在$[a,b]$上连续，在$(a,b)$上二次可微，证明存在$\xi\in(a,b)$使得
\[f(a)+f(b)-2f\left(\frac{a+b}{2}\right)=\frac14(b-a)^2f''(\xi)\]
\end{example}
\begin{solution}
在$f(a),f(b),f\left(\frac{a+b}{2}\right)$处插值，设
...
\end{solution}
\newpage
\def\exampletitle{$|f(x)-p(x)|\leqslant \frac1{384}\max_{x\in[0,1]}\{f''''(x)\}$}
\begin{example}[\exampletitle]
    $f(x)\in C^4[0,1]$，三次多项式$p(x)$满足$p(0)=f(0),p(1)=f(1),p'(0)=f'(0),p'(1)=f'(1)$，证明\[|f(x)-p(x)|\leqslant \frac1{384}\max_{x\in[0,1]}\{f''''(x)\}\]
\end{example}
\begin{solution}
构造$G(x)=f(x)-p(x)$，则$G(0)=G(1)=0,G'(0)=G'(1)=0$。继续应用 Rolle 定理可推出存在$\xi\in(0,1)$使得$G'''(\xi)=0$。四阶信息需要再引入一个随 $x$ 变化的零点，因此把 $G(x)$ 与 $x^2(x-1)^2$ 的比例联系起来，转为证明
\[
-\frac1{384}\max_{x\in[0,1]}\{f''''(x)\}\leqslant G(x)\leqslant \frac1{384}\max_{x\in[0,1]}\{f''''(x)\}.
\]
设 $G(x)=f(x)-p(x)$。取 $K$ 使得 $G(x)=K\dfrac{x^2(x-1)^2}{4!}$，再令
\[
H(y)=G(y)-K\dfrac{y^2(y-1)^2}{4!}.
\]
则 $H(0)=H(1)=H(x)=0$，$H'(0)=H'(1)=0$。重复应用罗尔定理，可得存在 $\xi\in(0,1)$ 使得 $H^{(4)}(\xi)=0$。因此
\[
f''''(\xi)=K\frac{\dd^4}{\dd y^4}\left(\dfrac{y^2(y-1)^2}{4!}\right).
\]
而 $\dfrac{y^2(y-1)^2}{4!}$ 的四阶导数恒为 1，故 $K=f''''(\xi)$，从而
\[
f(x)-p(x)=\dfrac{f''''(\xi)x^2(x-1)^2}{4!}\leqslant \frac{1}{384}f''''(\xi).
\]
取绝对值得到\[|f(x)-p(x)|\leqslant \frac1{384}\max_{x\in[0,1]}\{f''''(x)\}\]
\end{solution}
\newpage
\begin{example}[$f''''(\xi)=0$]
...
\end{example}
\begin{solution}
取原函数$F(x)$，则$F(x)\in C^5[0,1]$，且$F'(x)=f(x)$。原条件可写为
\[
F(1)-F(0)+3F'\left(\frac12\right)=8\left(F\left(\frac34\right)-F\left(\frac14\right)\right).
\]
按 $F(0),F(1),F\left(\frac12\right),F'\left(\frac12\right),F\left(\frac34\right),F\left(\frac14\right)$ 这六个条件作五次插值，
\begin{align*}
...
\def\exampletitle{$\displaystyle\max_{x\in[0,1]}f''(x)\geqslant8$}
\begin{example}[\exampletitle]
    设$f\in C^2[0,1]$，满足$f(0)=f(1)=0,\displaystyle\min_{x\in[0,1]}f(x)=-1$，证明$\displaystyle\max_{x\in[0,1]}f''(x)\geqslant8$
\end{example}
\begin{solution}
取$c\in(0,1)$使得$f(c)=-1$。于是插值条件为$f(0)=f(1)=0,f(c)=-1,f'(c)=0$。点 $c$ 将区间分为两段；左段由 $0,c$ 的条件给出一条估计，右段由 $c,1$ 的条件给出另一条估计。
...
因此在区间$(0,c)$和$(c,1)$上分别有$\xi_2,\xi_4$满足
\[
f''(\xi_2)=\frac{2}{c^2},\qquad f''(\xi_4)=\frac{2}{(1-c)^2}.
\]
若$c\in(0,\frac12]$，则$\frac{2}{c^2}\geqslant8$；若$c\in(\frac12,1)$，则$\frac{2}{(1-c)^2}\geqslant8$。于是
\[
\max_{x\in[0,1]}f''(x)\geqslant8.
\]
\end{solution}
\newpage
\begin{example}[$a\geqslant\sqrt2$]
...
\end{example}
\begin{solution}
    现有条件自然分布在两端，取公共点 $x=0$ 连接左右两段的插值式。
...
\newpage
\begin{example}[$\int_a^b\left|\frac{f''(x)}{f(x)}\right|\dd x\geqslant \frac{4}{b-a}$]
...
\begin{solution}
    ...
    \]
    设 $\displaystyle M = \max_{x\in[a,b]} |f(x)|$，则 $M>0$（否则 $f\equiv0$，不等式成立）。取 $c\in(a,b)$ 使 $|f(c)|=M$。在上式中令 $x=c$。由于 $|f(y)|\le M$，且左、右两段分别有 $y-a\le c-a$、$b-y\le b-c$，故
    \begin{align*}
    ...
    \]
    等号情形不影响本题所需下界，估计至此完成。
\end{solution}
\newpage
\begin{example}[（2004年全国II卷）]
    函数$f(x)=x\ln x$，求证：对于$a>b>0$有
...
\newpage
\begin{example}[$\displaystyle |F(x)|\leqslant \frac{M(b-a)^2}{16}$]
...
\end{align*}
(2) $F(x)$两阶可导，$|F''(x)|\leqslant M,F(a)=F(b)=F'(a)=F'(b)=0$。取$x_0\in(a,b)$ 为 $|F(x)|$ 的极大值点；若极值在端点取得，则由 $F(a)=F(b)=0$ 已满足估计。否则 $x_0$ 为内部极大点，从两侧 Taylor 关系同时夹住 $F(x_0)$：
\begin{align*}
    F(x)=F(a)+F'(a)(x-a)+\frac{(x-a)^2}{2!}F''(\xi_1)=\frac{(x-a)^2}{2!}F''(\xi_1)\\
    F(x)=F(x_0)+F'(x_0)(x-x_0)+\frac{(x-x_0)^2}{2!}F''(\xi_2)=\frac{(x-x_0)^2}{2!}F''(\xi_2)
\end{align*}
相减得到\[F(x_0)=\frac{(x_0-a)^2}{2!}F''(\xi_1)-\frac{(x-x_0)^2}{2!}F''(\xi_2)\]，取绝对值得
\[|F(x_0)|\leqslant \frac{1}{2!}\left(\frac{(x_0-a)^2}{2!}+\frac{(x-x_0)^2}{2!}\right)M=\frac{M}{4}\left((x_0-a)^2+(x-x_0)^2\right)\]
...
\end{solution}
\newpage
\begin{example}[$f'\left(\xi\right)=\int_0^1\left(12x-6\right)f\left(x\right)\dd x$]
