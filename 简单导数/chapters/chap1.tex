...
我们知道 $f(0) = 0, f''(0) = 1 > 0$，所以从$x=0$开始的一段区间内，$f(x)$ 是单调递增的，此时 $f'(x) = \sqrt{2\sin f(x)}>0$。
...
既然在整个区间 $(0, \frac{\pi}{\sqrt{2}}]$ 内都有 $f(x) < \pi$，那么 $\sin f(x)$ 就不会降为 $0$，$f'(x)$ 就不会等于 $0$ 或变成负数。所以这段区间内恒有$f(x)$单增。又因为 $\sin y$ 在 $[0, \pi]$ 上是向下凸的，它在两端较低，在中间 $y = \frac{\pi}{2}$ 时取得最高点。这意味着 $f'(x) = \sqrt{2\sin f(x)}$ 在该区间的最小值，必然在其两端点处取得。所以下面来估计一下 $f'(\sqrt{2}) $和 $f'\left(\frac{\pi}{\sqrt{2}}\right)$ 的下界。\\
由于$\sin\sqrt2$很接近1，先看$f'\left(\frac{\pi}{\sqrt{2}}\right)$的估值，不妨再直接考虑估计$f\left(\frac{\pi}{\sqrt{2}}\right)$，已知对于 $u \in \left[0, \frac{\pi}{2}\right]$，有 $\sin u \geqslant \dfrac{2u}{\pi}$。由 
...
这就导出了矛盾 $\frac{\pi}{\sqrt{2}} < \frac{\pi}{\sqrt{2}}$。因此 $f\left(\frac{\pi}{\sqrt{2}}\right) \geqslant \frac{\pi}{2}$。所以当 $x = \frac{\pi}{\sqrt{2}}$ 时，$f(x)$ 落在了$\sin x$的递减区间内，如果我们需要找到$\sin f\left(\frac{\pi}{\sqrt{2}}\right)$的下界，就要找到$f\left(\frac{\pi}{\sqrt{2}}\right)$的上界，利用$f(x) \leqslant \frac{x^2}{2}$得到上限为：$f\left(\frac{\pi}{\sqrt{2}}\right) \leqslant \frac{\pi^2}{4}$。因此：$$\sin f\left(\frac{\pi}{\sqrt{2}}\right) \geqslant \sin\left(\frac{\pi^2}{4}\right) > \sin\left(\frac{5\pi}{6}\right) = \frac{1}{2}$$
所以不难放缩得到
...
然后证明在区间左端点处 $f'(x) > 1$利用$\cos y \geqslant 1 - \frac{y^2}{2}$，并结合$f(x) \leqslant \frac{x^2}{2}$，可得：
