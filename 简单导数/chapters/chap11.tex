...
\end{align*}
所以关于 $(x-1)$ 的部分只剩 $\frac{3/2}{(x-1)^2}$。从原式中减去这项，记剩余部分为 $H(x)$，则
\begin{align*}
...
所以只需分别分解 $\frac{1}{AB}$ 与 $\frac{1}{AB^2}$。由 $A-B=x$，再利用带余除法 $B \div x$ 得 $B = x \cdot x + 1$，把 $x=A-B$ 代入余式关系便有
$$ 1 = B - x(A-B) = (x+1)B - xA, $$
从而
...
\end{example}
\begin{solution}
对一次因式 $(x-1)$ 和 $(x+1)$ 可以直接赋值。令 $x=1$，掩去分母中的 $(x-1)$，代入剩余部分得
...
参数微元也可以直接从几何看出。对实圆，$\dd\theta$ 是位置向量绕原点逆时针转过的有向小角，此时指针尖端的位移大小等于弧长 $a\dd\theta$，方向正好是 $\vec{r}$ 逆时针旋转 $90^\circ$ 后得到的向量 $(-y, x)$，模仍为 $a$，所以
\[(\dd x, \dd y)=\dd\theta(-y,x)\]
对虚圆，$\dd\phi$ 是位置向量绕原点逆时针转过的有向小角，此时指针尖端的位移大小等于 $a\dd\phi$，方向正好与 $\vec{r}$ 沿 $y=x$ 对称的向量 $(y,x)$ 一致，所以
\[(\dd x, \dd y)=\dd\theta(y,x)\]
