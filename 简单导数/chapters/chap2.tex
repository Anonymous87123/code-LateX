...
\section{一阶构造类}
\begin{example}[$f'(\xi)=\frac{2\xi-f(\xi)}{\xi}$]
...
\end{solution}
\newpage
\begin{example}[$f'(\xi)=\frac{\xi f(\xi)}{1-\xi}$]
...
\begin{solution}
    ...
    即证明存在$\xi\in(0,1)$使得$c'(\xi)=0$。又由于$c(0)=c(1)=0$，由罗尔定理可知这样的 $\xi$ 必存在，命题得证.
\end{solution}
\newpage
\def\exampletitle{$f'(\xi) = \lambda \left[ f(\xi) - \frac{\xi}{2} \right] + \frac{1}{2}$}
...
\end{solution}
\newpage
\begin{example}[$f'\left(\xi\right)+3f^3\left(\xi\right)=0$]
...
\end{example}
\begin{solution}
把 \(3f^2\) 视为已知函数，考虑方程 \(y'+3f^2y=0\)。解得 \(y=ce^{-\int_0^x3f^2(t)\dd t}\)，于是构造
\[
c\left(x\right)=f\left(x\right)e^{\int_0^x3f^2(t)\dd t}.
\]
对 $c(x)$ 求导，得到
...
若 \(f\) 只有一个零点，则由 \(f\left(0\right)>0,f\left(1\right)>0\) 可知
\[
f\left(x\right)>0,\qquad x\in[0,\theta)\cup(\theta,1],
\]
从而 \(\int_0^1f\left(x\right)\dd x>0\)，与题设矛盾。故存在 $\theta_1\neq\theta_2$，使得 $f\left(\theta_1\right)=f\left(\theta_2\right)=0$。于是
\[
c\left(\theta_1\right)=c\left(\theta_2\right)=0.
\]
由罗尔中值定理，存在 $\xi\in\left(0,1\right)$ 使得 $c'(\xi)=0$，即
$$f'\left(\xi\right)+3f^3\left(\xi\right)=0.$$
\end{solution}
\section{二阶构造类}
\subsection{\texorpdfstring{视为关于$f'$的一阶构造类}{视为关于f'的一阶构造类}}
\begin{example}[$f''(\eta) = \frac{2f'(\eta)}{1 - \eta}$]
...
\end{align*}
即证存在$\xi\in(0,1)$使得$c'(\xi)=0$。由已知 $c(1)=0$，再由 Rolle 定理可知存在$\xi_1\in(0,1)$使得$f'(\xi_1)=0$，从而$c(\xi_1)=0$。再对$c$应用 Rolle 定理，存在$\xi\in(\xi_1,1)\subset(0,1)$使得$c'(\xi)=0$，得证.
\end{solution}
\begin{example}[$f''\left(\xi\right)\left(1+\xi^2\right)+2f'\left(\xi\right)\xi=0$]
...
c'(x)=2xf'(x)+(1+x^2)f''(x).
\]
三个给定函数值与 $\arctan x$ 在对应点一致，即
\[
f(0)=\arctan 0,\qquad f\!\left(\frac{1}{\sqrt{3}}\right)=\arctan\frac{1}{\sqrt{3}},\qquad f(1)=\arctan 1,
\]
因此令
\[
...
\begin{example}[$f''\left(\xi\right)-2f'\left(\xi\right)\cot\xi+f\left(\xi\right)\left(1+2\cot^2\xi\right)=0$]
...
\]
由于 $\sin x>0$ 在 $(0,\pi)$ 内恒成立，原式为零等价于 $g''(\xi)=0$。

由 $f(0)f(\pi)<0$ 知 $f(0)$ 与 $f(\pi)$ 异号且均非零。设 $f(0)>0$，则 $f(\pi)<0$。考虑
\[
g'(x)=\dfrac{f'(x)\sin x-f(x)\cos x}{\sin^2 x}.
\]
当 $x\to0^+$ 时，$\sin x\sim x$，$\cos x\sim 1$，故
\[
...
\]
因此 $g'$ 在 $(0,\pi)$ 内连续，且两端趋于 $-\infty$，从而 $g'$ 在 $(0,\pi)$ 内必取得最大值（例如，取 $\delta>0$ 足够小，使在 $(0,\delta)$ 和 $(\pi-\delta,\pi)$ 上 $g'(x)<g'(\pi/2)$，则 $g'$ 在 $[\delta,\pi-\delta]$ 上有最大值，该最大值即为整个区间上的最大值）。设最大值点为 $\xi\in(0,\pi)$，则 $g'(\xi)$ 为极大值，故 $g''(\xi)=0$。若 $f(0)<0$，则上述两端极限同时改为 $+\infty$，改取内部最小值点，同样得到 $g''(\xi)=0$。因此存在 $\xi\in(0,\pi)$ 使 $g''(\xi)=0$，代入即得
\[
f''(\xi)-2f'(\xi)\cot\xi+f(\xi)(1+2\cot^2\xi)=0.
\]
\end{solution}
\begin{example}[$f''\left(\xi\right)=f\left(\xi\right)\left(1+2\tan^{2}\xi\right)$]
...
\end{solution}
\newpage
\begin{example}[$f''\left(\zeta\right)=3f'\left(\zeta\right)\tan\zeta+2f\left(\zeta\right)$]
...
代入方程可得
\[
y_1''-3y_1'\tan x-2y_1=0,\qquad
y_2''-3y_2'\tan x-2y_2=0,
\]
其中 $y_1(x)=\dfrac{1}{\cos^2 x}$，$y_2(x)=\dfrac{\sin x}{\cos^2 x}$，且二者线性无关，故通解为
\[
...
\end{solution}
\begin{example}[$f''\left(\xi\right)+2\xi^2f'\left(\xi\right)+\left(\xi^4+2\xi\right)f\left(\xi\right)=0$]
...
g'(\xi_1)=0,\quad g'(\xi_2)=0.
\]
再对 $g'$ 在 $[\xi_1,\xi_2]$ 上应用罗尔定理，存在 $\xi\in(\xi_1,\xi_2)$ 使得 $g''(\xi)=0$。由 $e^{\xi^3/3}>0$，式 (2) 中括号内表达式必须为零，因此
\[
f''(\xi) + 2\xi^2 f'(\xi) + (\xi^4 + 2\xi)f(\xi)=0.
\]
因此存在 $\xi\in\mathbb{R}$ 满足要求。
\end{solution}
\begin{example}[$f(\theta) + f''(\theta) = 0$]
...
\end{example}
\begin{solution}
由方程 $y''+y=0$ 可知，若 $y$ 是其解，则两边乘以 $y'$ 得
\[
y'y''+yy'=0,
\]
即
\[
\frac{1}{2}\frac{\mathrm{d}}{\mathrm{d}x}\bigl((y')^2+y^2\bigr)=0.
\]
故 $(y')^2+y^2$ 为常数。令
\[
...
\]
假设对一切 $x\in(-\frac{\pi}{2},\frac{\pi}{2})$ 均有 $f(x)+f''(x)\neq0$，则由连续性，$f(x)+f''(x)$ 在 $(-\frac{\pi}{2},\frac{\pi}{2})$ 内恒正或恒负。若它恒负，对 $-f$ 作同一论证；以下以恒正情形写出推导。由于在 $(-\frac{\pi}{2},\frac{\pi}{2})$ 内 $\cos x>0$，由 (1) 知 $g'(x)>0$，即 $g$ 严格递增。于是
\[
...
\end{solution}
\newpage
\begin{example}[$\frac{f'\left(\xi\right)}{g'\left(\xi\right)}=\frac{f\left(\xi\right)-f\left(a\right)}{g\left(b\right)-g\left(\xi\right)}$]
...
\end{solution}
\newpage
\begin{example}[$\frac{f\left(\xi\right)}{g\left(\xi\right)}=\frac{f''\left(\xi\right)}{g''\left(\xi\right)}$]
...
\]
还需保证分式有意义。已知 $g''(x)\neq0$ 对一切 $x\in[a,b]$ 成立，故 $g''$ 在 $(a,b)$ 内恒正或恒负。若 $g''>0$，则 $g$ 为严格凸函数，且由 $g(a)=g(b)=0$ 可知 $g(x)<0\ (x\in(a,b))$；若 $g''<0$，同理有 $g(x)>0$。因此 $g(x)\neq0$ 对一切 $x\in(a,b)$ 成立，特别地 $g(\xi)\neq0$。再由 $g''(\xi)\neq0$，(2) 式化为
\[
...
\end{solution}
\newpage
\begin{example}[$f\left(\xi\right)f'\left(\xi\right)+f''\left(\xi\right)=0$]
...
\]
定义辅助函数 $g(x)=f'(x)+\frac{1}{2}f^2(x)$，则 $g(0)=0$。若能在 $(0,1]$ 内找到另一点 $x_0$ 使 $g(x_0)=0$，则对 $g$ 在 $[0,x_0]$ 上应用罗尔定理，可得存在 $\xi\in(0,1)$ 使 $g'(\xi)=0$，而 $g'(\xi)=f(\xi)f'(\xi)+f''(\xi)$。为得到第二个零点，转而考察方程 $g(x)=0$ 所对应的辅助函数。令
\[
...
若 $f$ 在 $(0,1)$ 内有零点，则取一个由相邻零点或端点围成的区间 $(\alpha,\beta)$，并设 $\alpha$ 为其中的零点。于是 $f$ 在 $(\alpha,\beta)$ 上不变号且不为零，故 $\varphi$ 在该区间内连续可导。又当 $x\to\alpha^+$ 时，$f(x)\to0$，于是 $|\varphi(x)|\to\infty$。因此 $\varphi$ 在 $(\alpha,\beta)$ 内必有极值点 $\eta$，从而 $\varphi'(\eta)=0$，即 $g(\eta)=0$。再由 $g(0)=g(\eta)=0$ 对 $g$ 应用罗尔定理，得某个 $\xi\in(0,\eta)$ 使 $g'(\xi)=0$，结论成立。

因此，无论 $f$ 在 $(0,1)$ 内是否有零点，总存在 $\xi\in(0,1)$ 使得
\[
f(\xi)f'(\xi)+f''(\xi)=0.
\]
\end{solution}
...
\section{多中值点问题}
\begin{example}[$f'\left(\lambda\right)\left(1+f'\left(\mu\right)\right)=2$]
...
\end{example}
\begin{solution}
任取 $c\in(0,1)$。由拉格朗日中值定理，在区间 $[0,c]$ 上存在 $\lambda\in(0,c)$ 使得
\[
f'(\lambda)=\frac{f(c)-f(0)}{c-0}=\frac{f(c)}{c}.
\]
在区间 $[c,1]$ 上存在 $\mu\in(c,1)$ 使得
\[
...
\end{solution}
\newpage
\begin{example}[$\sum_{i=1}^n\frac{\lambda_i}{f^{\prime}\left(x_i\right)}=1$]
...
\end{example}
\begin{solution}
取分点 $0 = y_0 < y_1 < \cdots < y_{n-1} < y_n = 1$。在每个子区间 $(y_{i-1},y_i)$ 上对 $f$ 应用拉格朗日中值定理，存在 $x_i \in (y_{i-1},y_i)$ 使得
\[
...
\]
则 (1) 化为 $\displaystyle\sum_{i=1}^n (y_i - y_{i-1}) = 1$。令
\[
t_i = \lambda_1 + \lambda_2 + \cdots + \lambda_i, \quad i=1,\dots,n-1,
\]
则 $0 < t_1 < t_2 < \cdots < t_{n-1} < 1$。由介值定理，对每个 $t_i$ 都存在 $y_i \in (0,1)$ 使得 $f(y_i)=t_i$。这些原像还可以按次序选取：取 $y_1$ 满足 $f(y_1)=t_1$；若已取到 $y_i$，则在区间 $(y_i,1]$ 上，函数 $f$ 连续且满足 $f(y_i)=t_i<t_{i+1}<1=f(1)$，故存在 $y_{i+1}>y_i$ 使得 $f(y_{i+1})=t_{i+1}$。于是
\[
0<y_1<y_2<\cdots<y_{n-1}<1,\qquad f(y_i)=t_i.
\]
因此
\[
...
\end{solution}
\newpage
\begin{example}[$\left|\sum_{j=1}^N\frac1{2^jf'\left(x_j\right)}-1\right|<\varepsilon$]
...
\]
所求的 $N$ 及两两不同的 $x_1,\dots,x_N\in(0,1)$ 便构造出来了。
\end{solution}
\newpage
\begin{example}
...
\]
而 $\frac{g(1)}{2} = \frac{\pi}{8}\int_0^1 f$，故 (1) 成立。$\theta_1,\theta_2,\theta_3$ 互不相同（$\theta_1<\theta_3<\theta_2$），证明完成。
\end{solution}
\newpage
\begin{example}
...
\]
由此得到两个不同的点 $\theta\in(0,\frac{2}{3})$ 和 $\eta\in(\frac{2}{3},1)$，满足 $f(\theta)=f(\eta)$。下证存在 $\xi\in(0,1)$ 使得
\[
...
\]
由于 $\theta<\eta$，函数 $c$ 在 $[\theta,\eta]$ 上满足 $c(\theta)=c(\eta)=0$。由罗尔中值定理，存在 $\xi\in(\theta,\eta)\subset(0,1)$ 使得 $c'(\xi)=0$。再由 $e^{g(\xi)}>0$，式 (5) 化为
\[
