...
c'(x)=2xf'(x)+(1+x^2)f''(x).
\]
注意到 $f(0)=0,\ f\!\left(\frac{1}{\sqrt{3}}\right)=\frac{\pi}{6},\ f(1)=\frac{\pi}{4}$，而 $\arctan 0=0,\ \arctan\frac{1}{\sqrt{3}}=\frac{\pi}{6},\ \arctan 1=\frac{\pi}{4}$，故令
\[
...
证明 $g''$ 在 $(0,\pi)$ 内有零点。由 $f(0)f(\pi)<0$ 知 $f(0)$ 与 $f(\pi)$ 异号且均非零。设 $f(0)>0$，则 $f(\pi)<0$；另一种情形完全相同。考虑
\[
...
\]
假设对一切 $x\in(-\frac{\pi}{2},\frac{\pi}{2})$ 均有 $f(x)+f''(x)\neq0$，则由连续性，$f(x)+f''(x)$ 在 $(-\frac{\pi}{2},\frac{\pi}{2})$ 内恒正或恒负。若它恒负，就改看 $-f$；因此只需讨论恒正的情形。由于在 $(-\frac{\pi}{2},\frac{\pi}{2})$ 内 $\cos x>0$，由 (1) 知 $g'(x)>0$，即 $g$ 严格递增。于是
\[
...
\]
定义辅助函数 $g(x)=f'(x)+\frac{1}{2}f^2(x)$，则 $g(0)=0$。若能在 $(0,1]$ 内找到另一点 $x_0$ 使 $g(x_0)=0$，则对 $g$ 在 $[0,x_0]$ 上应用罗尔定理，可得存在 $\xi\in(0,1)$ 使 $g'(\xi)=0$，而 $g'(\xi)=f(\xi)f'(\xi)+f''(\xi)$。下面寻找 $g$ 的另一个零点。考虑方程 $g(x)=0$，其通解为 $\frac{1}{f(x)}=\frac{x}{2}+C$。令
\[
...
\paragraph{情况2：$f$ 在 $(0,1)$ 内有零点。} 取一个由相邻零点或端点围成的区间 $(\alpha,\beta)$，并设 $\alpha$ 为其中的零点。则 $f$ 在 $(\alpha,\beta)$ 上不变号且不为零，故 $\varphi$ 在该区间内连续可导。又当 $x\to\alpha^+$ 时，$f(x)\to0$，于是 $|\varphi(x)|\to\infty$。因此 $\varphi$ 在 $(\alpha,\beta)$ 内必有极值点 $\eta$，从而 $\varphi'(\eta)=0$，即 $g(\eta)=0$。再由 $g(0)=g(\eta)=0$ 对 $g$ 应用罗尔定理，得某个 $\xi\in(0,\eta)$ 使 $g'(\xi)=0$，结论成立。
