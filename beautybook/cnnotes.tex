\documentclass[lang=cn,12pt,fontset=none,a4paper]{beautybook}
% ---------------------------------------------------------------------------- %
...
\beautybookstyle={
   ...
   math-font=mtpro2, % plain/mtpro2/fourier/stix (xelatex only!!)
   sidebar=on, % on/off
}
...
因此，我们得出，具有 {\rodondo neat analytic singularities} 的函数 \(\psi\) 的集合 \( S = \{ \psi = -\infty \} \) 是流形 \(M\) 的一个解析子流形。这是因为在每个邻域 \(U\) 上，\(\psi\) 可以表示为一个常数 \(c\) 乘以对全纯函数平方和的对数加上一个光滑函数，而 \(\psi = -\infty\) 的点集正好是这些全纯函数的零点集的交集，构成了一个解析子流形。

\subsection{Normal Bundle and Its Duality}

在微分几何和代数几何中，法丛（{\mont normal bundle}）和其对偶丛（{\cabin dual bundle}）是重要的概念。下面将详细解释它们的定义及如何理解这些概念。
...
\begin{itemize}
   \item $L$的Hermitian度量会影响其$l$次张量积$L^{\otimes l}$的度量;
   \item 对于$L^{\otimes l}$的曲率$\Theta(L^{\otimes l})$，我们有$\Theta(L^{\otimes l}) = l\Theta(L)$，其中$\Theta(L)$是$L$的曲率形式。因此，定理中的条件需要相应调整以考虑这个变化。
\end{itemize}
...
\begin{itemize}
      ...
      \item 定理中的其他条件也需要相应调整以适应$L^{\otimes l}$。
\end{itemize}
   

3. 证明中的调整：证明中的所有涉及$L$的部分需要替换为$L^{\otimes l}$，并且重新计算相关的L2估计和Sobolev范数。
...
\begin{itemize}
      \item L2估计：原文中使用的 L2 估计可能依赖于 $L$ 的具体性质，特别是度量 $h_L$。推广到 $L^{\otimes l}$ 时，需要重新计算这些估计，并确保 $L^{\otimes l}$ 的度量 $h_L^{\otimes l}$ 带来的变化不会破坏证明的有效性。
      
      \item Sobolev范数：原文中定义的 Sobolev 范数可能需要重新定义，以适应 $L^{\otimes l}$。尤其是在涉及到 jets 的部分，需要确认新的 Sobolev 范数能够正确估计 $L^{\otimes l}$ 的 jets。
\end{itemize}
...
\begin{itemize}
      \item L2 Extension Theorem：原文中的证明基于 Ohsawa-Takegoshi 的 L2 extension theorem 和 Manivel 的推广。需要确保这些定理在 $L^{\otimes l}$ 上仍然适用。如果有额外条件，需要进一步验证。
      
      \item Rauch Comparison Theorem：证明中使用的 Rauch 比较定理也需要重新检查，确保它在 $L^{\otimes l}$ 的情况下依然成立。
\end{itemize}
...
\begin{itemize}
      \item 证明逻辑：整个证明过程需要重新梳理，确保每一步都能推广到 $L^{\otimes l}$。特别是涉及到度量和曲率的部分，需要逐一验证。
      
      \item 统一常数和估计：原文中的统一常数和估计需要重新计算，并确保它们在新的情况下仍然适用。这包括所有涉及到 $L$ 的部分，例如 $α$ 的定义和估计等。
\end{itemize}
   

5. 额外难点
\begin{itemize}
      \item 范数和估计：你已经提到涉及到 $L$ 的范数及相关估计是一个难点。这是一个关键问题，需要细致地重新计算并验证。
      
      \item 其他潜在问题：任何涉及到具体几何性质的部分，例如喷射 (jets) 的定义和性质，可能需要重新验证。在推广过程中，需要特别关注这些细节，确保所有几何和分析性质在新的设置下仍然成立。
\end{itemize}
...
主要性质
\begin{itemize}
   \item 局部坐标描述：在局部坐标系中，$k$ 阶喷射可以表示为 $f$ 的各阶偏导数组成的向量。
   \item 光滑结构：$k$ 阶喷射丛本身是一个光滑流形，并且可以构造出一个纤维丛（fiber bundle），其中纤维是与 $k$ 阶导数有关的向量空间。
   \item 应用：喷射丛在研究偏微分方程的几何理论中非常重要。通过喷射丛，可以把微分方程的解的高阶导数转化为几何对象进行研究。
\end{itemize}

具体例子
\begin{itemize}
   \item 1-jet bundle：当 $k=1$ 时，1-jet bundle 记录了映射的点和该点的导数。对于光滑映射 $f: \mathbb{R}^n \to \mathbb{R}^m$，1-jet bundle 记录了 $f$ 在某点的值及其雅可比矩阵（Jacobian matrix）。
   \item 高阶 jet bundle：对于 $k > 1$，$k$-jet bundle 记录了高阶偏导数，能够捕捉更多的局部几何信息。
\end{itemize}
...
\begin{theorem}[][Weierstrass Division Theorem][thm:Weierstrass Division Theorem]
   Let $g(z,w)\in \calO_{n-1}[w]$ be a Weierstrass polynomial of degree $k$ in $w$. Then for any $f\in \calO_{n}$, we can write 
   \[f=g\cdot h+r\]
...
\begin{theorem}[][Inverse Function Theorem][thm:Inverse]
   Let $U,V$ be sets in $\bC^n$ with $0\in U$ and $f: U\to V$ a holomorphic map with $\scrF (f)=\Diff[f_j]{z_i}$ nonsingular at $0$. Then $f$ is one-to-one in a neiborhood of $0$, and $f^{-1}$ is holomorphic at $f(0)$.
\end{theorem}
   \begin{proof}
      As $\scrF (f)=\Diff[f_j]{z_i}$ nonsingular at $0$, i.e., not all of the $\Diff[f_j]{z_i}=0, i,j=1,\dots,n$,  thus $\det \abs{\scrF_\bR (f)}=\abs{\det(\scrF(f))}^2\neq 0$ at $0$, by ordinary inverse fuction theorem $f$ has a smooth inverse $f^{-1}$ near $0$. Now the left  thing is to prove this $f^{-1}$ is holomorphic at $f(0)$. Naturally, we have 
      \[f^{-1} \circ f (z)=z,\]
       so 
       \[
         \begin{aligned}
...
\begin{myitemize}
   \myitem 测试一下效果如何
   \myitem 测试超长文段 As $\scrF (f)=\Diff[f_j]{z_i}$ nonsingular at $0$, i.e., not all of the $\Diff[f_j]{z_i}=0, i,j=1,\dots,n$,  thus $\det \abs{\scrF_\bR (f)}=\abs{\det(\scrF(f))}^2\neq 0$ at $0$, by ordinary inverse fuction theorem $f$ has a smooth inverse $f^{-1}$ near $0$. Now the left  thing is to prove this $f^{-1}$ is holomorphic at $f(0)$. Naturally, we have 
\[f^{-1} \circ f (z)=z,\]
 so 
 \[
   \begin{aligned}
      ...
      & =\sum_k \frac{\partial f_j^{-1}}{\partial \bar{z}_k} \cdot\left(\frac{\overline{\partial f_k}}{\partial z_i}\right) \quad \text { for all } i, j
      \end{aligned}\]
   \myitem 测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何测试一下效果如何
   \myitem 初步的学习计划如下
   \begin{itemize}
      \item 第一步：学习基本概念和性质
      \item 第二步：学习基本方法和步骤
      \item 第三步：学习如何灵活运用并对原概念进行拓展
   \end{itemize}
   \myitem 学习方法
\end{myitemize}
...
$\bR,\scrR,\calR,\frakA,\rmH,\bfY$

Remember an equation:
\begin{equation}
    \remember{thm:pythagorean}{
        a^2 + b^2 = c^2
    }
\end{equation}
...
Another equation:
\begin{equation}
    a^2 + b^2 = c^2 - 2ab\cos C\label{thm:cosine}
\end{equation}

Now we recall the first equation:
\begin{equation}
    \recall{thm:pythagorean}
\end{equation}
% ---------------------------------------------------------------------------- %
\begin{Proposition}[正四棱锥与球的公共部分体积]\label{prop:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{Proposition}

\begin{Definition}[正四棱锥与球的公共部分体积]\label{defi:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{Definition}

\begin{Theorem}[正四棱锥与球的公共部分体积]\label{thm:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{Theorem}

\begin{Lemma}[正四棱锥与球的公共部分体积]\label{Lemma:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{Lemma}

\begin{Corollary}[正四棱锥与球的公共部分体积]\label{Corollary:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{Corollary}

\begin{repitition}[正四棱锥与球的公共部分体积]\label{repitition:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{repitition}

\begin{application}[正四棱锥与球的公共部分体积]\label{application:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{application}


\begin{RemarK}[正四棱锥与球的公共部分体积]\label{Remark:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{RemarK}


\begin{Example}[正四棱锥与球的公共部分体积]\label{Example:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{Example}

\begin{Problem}[正四棱锥与球的公共部分体积]\label{Problem:2}
    若正方体边长为$1$. 当点 $P$ 为正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球球心时，正方体 $A B C D-A_1 B_1 C_1 D_1$ 的内切球与正四棱雉 $P-A B C D$ 的公共部分的体积为 $\dfrac{\pi}{36}$
\end{Problem}
\begin{Proof}
测试一下
...
\chapter{学习计划：关于《Positivity in Algebraic Geometry I》的初步学习计划}
\section{当前进度与笔记}
\subsection{Ample of Nef Line Bundles}
