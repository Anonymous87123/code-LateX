% !TEX root = Notes_for_complex_geometry_by_huybrechets.tex
...
As $\alpha\in\calA^{0,q}(U)=\bigwedge^{0,q}(U)=\bC^n\otimes \bigwedge^{q}(T^* U)^{0,1}$ , set the basis of $\bigwedge^{q}(T^* U)^{0,1}$ is $\mathrm{d}\bar{z}_1,\cdots,\mathrm{d}\bar{z}_n$, then one has
\[
...
Set {\color{DarkBlue}\[\gamma:=\sum_{k \in I} g_I \,\mathrm{d}\bar{z}_{I \backslash\{k\}}\in \calA^{0,q-1}(B_{\varepsilon_k}).\]}
 Then 
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[remember picture,>=Stealth]
    % 左边部分
    \node (a1) at (90:1cm) {$E$};
    \node (a2) at (270:1cm) {$M \times \mathbb{C}^r$};
    ...
    \draw[->] (c) -- node[above, font=\bfseries\itshape,pos=0.45] {Locally} node[below, font=\bfseries\itshape,pos=0.45]{as} ++(1.8cm,0);
    
    % 右边部分
    \node (b1) at ([xshift=8cm]a1) {$\tikzmarknode{x11}{\pi^{-1}(x)} \subset \tikzmarknode{x12}{\pi^{-1}(U_i) \subset E}$};
    % 仅为 \mathbb{C}^r 和 U_i \times \mathbb{C}^r 创建 tikzmark
    \node (b2) at ([xshift=8cm]a2) {$\tikzmarknode{x21}{\mathbb{C}^r} \quad\subset\quad \tikzmarknode{x22}{U_i \times \mathbb{C}^r}$};
    \node[right] (b3) at ([xshift=4cm]b1) {$U_i \subset M (=\bigcup U_i)$};
    
    % 绘制箭头
    \draw[->] (b1) -- node[above right, pos=0.3] {$\pi$} (b3);
    \draw[->] ([xshift=0cm]x11) -- node[left] {$\psi_i(x)$} node[right] {$\cong$} ([xshift=0cm]x21);
    \draw[->] ([xshift=0cm]x12) -- node[right] {$\psi_i$} node[left] {$\cong$} ([xshift=0cm]x22);
    \draw[->] (b2.north east) -- node[below right] {$\mathrm{Pr}_1$} (b3.south west);
  \end{tikzpicture}
