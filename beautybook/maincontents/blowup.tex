...
\begin{definition}[][transgressive]
    An element $\alpha\in H^{p,q}_{\bdd}(F)$  is called transgressive if there is a representative $\alpha\in \calA^{p,q}_{\bdd}(F)$ which extends to a form $\widetilde{\alpha}\in \calA^{p,q}_{\bdd}(F)$ such that $\bd\widetilde{\alpha}=\pi^*\beta$ for some $\bd$-closed form $\beta\in \calA^{p,q+1}_{\bdd}(B)$.
\end{definition}

The following are all the settings in this paper.
\begin{enumerate}
    \item $i^*\colon \calA^{p,q}(M)\to\calA^{p,q}(N)$ is surjective, which induces a short exact sequence for the pair $(M,N)$ of complexes 
    \[
        0\to\calA^{p,\bullet}(M,N)\to \calA^{p,\bullet}(M)\stackrel{i^*}{\to}\calA^{p,\bullet}(N)\to 0
    \]
     and a long exact sequence
     \[
        \cdots\to H^{p,\bullet}_{\bdd} (M,N)\to H^{p,\bullet}_{\bdd} (M) \to H^{p,\bullet}_{\bdd} (N)\to \cdots.
     \]
     !Note that here $i\colon N\hookrightarrow M$ is an inclusion, which is naturally injective. Associating with the above condition, the exactness at $\calA^{p,\bullet}(M)$ is clear.
    \item 
\end{enumerate}
...
\begin{example}[][Blow-up with the center at a point]
    Consider the situation of line bundle $\calO(-1)$ over projective manifold $\bP^n$, which is given as the \textbf{incidence variety} 
    \[
        \begin{tikzcd}
\bP^n \ar[r,-Stealth,hook,"\pi^*"]\ar[d,-Stealth]&\calO(-1)\ar[d,-Stealth,"\sigma"]\\ 
\{0\}\ar[r,-Stealth,hook,"\operatorname*{id}"]&\bC^{n+1}
        \end{tikzcd}
    \]
    Recall that the set $\calO(-1)\subset \bP^n\times \bC^{n+1}$ that consists of all pairs $(\ell,z)\in\bP^n\times \bC^{n+1}$ with $z\in\ell$ forms in a natural way a holomorphic line bundle over $\bP^n$ by Huybrechets,Prop 2.2.6.  First, we have the projective $\pi\colon \calO(-1)\to\bP^n$, and the fibres of $\calO(-1)$ is $\pi^{-1}(\ell) , \ell\in\bP^n$. Without loss generality, let $\bP^n=\bigcup_{i=0}^n U_i$ be the standard open covering, which induces the fact that $\ell$ must belong to one of $U_i$. Here we just set $\ell\in U_i$ for conviniance. Then we have the \textit{trivialization of $\calO(-1)$ over $U_i$}, which is given by 
    \[
        \begin{aligned}
...
\end{align*}
Now we can easily see that the fibre of $\calO(-1)$ over $\ell\in\bP^n$ is naturally isomorphic to $\ell$ itself. Let us consider the other canonical projection $\sigma\colon \calO(-1)\to \bC^{n+1}$. Theirs' relations are the following diagram.
\[
    \begin{tikzcd}
        &\calO(-1)\subset \bP^n\times \bC^{n+1}\dlar[-Stealth,"\pi"]\drar[-Stealth,"\sigma"]&\\
...
Thus we construct a \textit{Blow-up} $\sigma\colon \operatorname{Bl}_0 (\bC^{n+1})\to\bC^{n+1}$ of $\bC^{n+1}$ along the zero dimensional submanifold $\{0\}$, which by definition is a holomorphic line bundle $\calO(-1)$ with the center at $0$ . Note that $E:=\operatorname{Bl}_0 (\bC^{n+1})=\bP^n$ is the exceptional divisor.
\end{example}
...
\begin{lemma}
    There exists an open neighborhood $\calU$ of $Y$ in $X$ such that $i^*\colon \calA^{p,q}(\calU)\to\calA^{p,q}(Y)$ is surjective, where $i:=\iota|_{\calU}\colon Y\hookrightarrow\calU $  is the inclusion and 
    \[
        \calA^{p,\bullet}(X,Y):=\{\alpha\in\calA^{p,\bullet}(X)\mid i^*\alpha=0\}=\ker (i^*).
    \]
\end{lemma}

\begin{lemma}[][][lem:sur]

    The pullback $\iota^*\colon \calA^{p,q}(X)\to\calA^{p,q}(Y)$ is surjective. Here $\iota\colon Y\hookrightarrow X$  is an inclusion.
\end{lemma}
Thus $j$ is an inclusion, which is naturally injective and $\iota^*$ is surjective by lemma   \ref{lem:sur}. Now we then obtains a short exact sequence for the pair $(X,Y)$ of complexes
 \[
    \begin{tikzcd}
        0\rar[-Stealth]  & \calA^{p,\bullet}(X,Y) \rar[-Stealth,"j"] & \calA^{p,\bullet}(X)\rar[-Stealth,"\iota^*"] &  \calA^{p,\bullet}(Y) \rar[-Stealth]  &0.
    \end{tikzcd}
 \]
...
\[
    \begin{tikzcd}
        0\rar[-Stealth]  & \calA^{p,\bullet}(X,Z) \dar[-Stealth,"\pi^*"]\rar[-Stealth] & \calA^{p,\bullet}(X)\dar[-Stealth,"\pi^*"]\rar[-Stealth] &  \calA^{p,\bullet}(Z) \rar[-Stealth] \dar[-Stealth,"\pi_E^*"] &0\\
        0\rar[-Stealth]  & \calA^{p,\bullet}(\widehat{X},E) \rar[-Stealth] & \calA^{p,\bullet}(\widehat{X})\rar[-Stealth] &  \calA^{p,\bullet}(E) \rar[-Stealth]  &0
    \end{tikzcd}
\]
